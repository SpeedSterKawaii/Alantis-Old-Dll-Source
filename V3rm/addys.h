// Auto Lua Type: https://github.com/Mellonyt/Roblox-Lua-Type-Scanner 
// Scanning Time: 70.000000 Secs 
// Dumping Date: Tue Feb  9 15:25:22 2021
// For version-c26bf9281ab64731 
// Dumped With Mellonyt aka Mellon Addy Dumper My Discord: mellonyt#1488 
#include <Windows.h> 
#include<cstdint> 
namespace Adresses {
	const char* name = "AtlantisClientC.dll";
	/*

	* addinfo_Decompiled_Args: int a1
	* addinfo_TypeDef: int
	* addinfo_Aob: 55 8B EC 81 EC 0 1 0 0 57
	* addinfo_ProtectedByRetcheck: false
	* addinfo_IsApartOfLua: true
	* addinfo_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_addinfo_typedef)(int a1);
	r_addinfo_typedef r_addinfo = (r_addinfo_typedef)(x(0x0138E740));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(addinfo_CCV* r_addinfo_typedef)(int a1);
	r_addinfo_typedef r_addinfo = (r_addinfo_typedef)(x(Adresses::addinfo_addr));



	*/
	std::uint32_t addinfo_addr = 0x0138E740;
#define addinfo_CCV __cdecl

	/*

	* adjuststack_Decompiled_Args: int a1
	* adjuststack_TypeDef: int
	* adjuststack_Aob: 55 8B EC 53 8B 5D 8 83
	* adjuststack_ProtectedByRetcheck: false
	* adjuststack_IsApartOfLua: true
	* adjuststack_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_adjuststack_typedef)(int a1);
	r_adjuststack_typedef r_adjuststack = (r_adjuststack_typedef)(x(0x01383C50));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(adjuststack_CCV* r_adjuststack_typedef)(int a1);
	r_adjuststack_typedef r_adjuststack = (r_adjuststack_typedef)(x(Adresses::adjuststack_addr));



	*/
	std::uint32_t adjuststack_addr = 0x01383C50;
#define adjuststack_CCV __cdecl

	/*

	* arith_Decompiled_Args: int a1, int a2, int a3
	* arith_TypeDef: int
	* arith_Aob: 55 8B EC 56 8B 75 C 57
	* arith_ProtectedByRetcheck: false
	* arith_IsApartOfLua: true
	* arith_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_arith_typedef)(int a1, int a2, int a3);
	r_arith_typedef r_arith = (r_arith_typedef)(x(0x013999B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(arith_CCV* r_arith_typedef)(int a1, int a2, int a3);
	r_arith_typedef r_arith = (r_arith_typedef)(x(Adresses::arith_addr));



	*/
	std::uint32_t arith_addr = 0x013999B0;
#define arith_CCV __cdecl

	/*

	* aux_upvalue_Decompiled_Args: int a1, int a2, int a3
	* aux_upvalue_TypeDef: int
	* aux_upvalue_Aob: 55 8B EC 8B 55 8 56
	* aux_upvalue_ProtectedByRetcheck: false
	* aux_upvalue_IsApartOfLua: true
	* aux_upvalue_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_aux_upvalue_typedef)(int a1, int a2, int a3);
	r_aux_upvalue_typedef r_aux_upvalue = (r_aux_upvalue_typedef)(x(0x01381630));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(aux_upvalue_CCV* r_aux_upvalue_typedef)(int a1, int a2, int a3);
	r_aux_upvalue_typedef r_aux_upvalue = (r_aux_upvalue_typedef)(x(Adresses::aux_upvalue_addr));



	*/
	std::uint32_t aux_upvalue_addr = 0x01381630;
#define aux_upvalue_CCV __cdecl

	/*

	* auxgetinfo_Decompiled_Args: int a1, int a2, int a3, int a4
	* auxgetinfo_TypeDef: int
	* auxgetinfo_Aob: 55 8B EC 53 56 8B 75 C BB
	* auxgetinfo_ProtectedByRetcheck: false
	* auxgetinfo_IsApartOfLua: true
	* auxgetinfo_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_auxgetinfo_typedef)(int a1, int a2, int a3, int a4);
	r_auxgetinfo_typedef r_auxgetinfo = (r_auxgetinfo_typedef)(x(0x0138E7B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(auxgetinfo_CCV* r_auxgetinfo_typedef)(int a1, int a2, int a3, int a4);
	r_auxgetinfo_typedef r_auxgetinfo = (r_auxgetinfo_typedef)(x(Adresses::auxgetinfo_addr));



	*/
	std::uint32_t auxgetinfo_addr = 0x0138E7B0;
#define auxgetinfo_CCV __cdecl

	/*

	* auxresume_Decompiled_Args: int a1, int a2, int a3
	* auxresume_TypeDef: int
	* auxresume_Aob: 55 8B EC 53 56 8B 75 C 57
	* auxresume_ProtectedByRetcheck: false
	* auxresume_IsApartOfLua: true
	* auxresume_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_auxresume_typedef)(int a1, int a2, int a3);
	r_auxresume_typedef r_auxresume = (r_auxresume_typedef)(x(0x013902B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(auxresume_CCV* r_auxresume_typedef)(int a1, int a2, int a3);
	r_auxresume_typedef r_auxresume = (r_auxresume_typedef)(x(Adresses::auxresume_addr));



	*/
	std::uint32_t auxresume_addr = 0x013902B0;
#define auxresume_CCV __cdecl

	/*

	* call_bintm_Decompiled_Args: int a1, int a2, int a3, int a4
	* call_bintm_TypeDef: int
	* call_bintm_Aob: 55 8B EC 51 53 56 8B 75 C 57
	* call_bintm_ProtectedByRetcheck: false
	* call_bintm_IsApartOfLua: true
	* call_bintm_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_call_bintm_typedef)(int a1, int a2, int a3, int a4);
	r_call_bintm_typedef r_call_bintm = (r_call_bintm_typedef)(x(0x01399220));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(call_bintm_CCV* r_call_bintm_typedef)(int a1, int a2, int a3, int a4);
	r_call_bintm_typedef r_call_bintm = (r_call_bintm_typedef)(x(Adresses::call_bintm_addr));



	*/
	std::uint32_t call_bintm_addr = 0x01399220;
#define call_bintm_CCV __cdecl

	/*

	* call_ordertm_Decompiled_Args: int a1, int a2
	* call_ordertm_TypeDef: int
	* call_ordertm_Aob: 55 8B EC 8B 45 8 53
	* call_ordertm_ProtectedByRetcheck: false
	* call_ordertm_IsApartOfLua: true
	* call_ordertm_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_call_ordertm_typedef)(int a1, int a2);
	r_call_ordertm_typedef r_call_ordertm = (r_call_ordertm_typedef)(x(0x013992D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(call_ordertm_CCV* r_call_ordertm_typedef)(int a1, int a2);
	r_call_ordertm_typedef r_call_ordertm = (r_call_ordertm_typedef)(x(Adresses::call_ordertm_addr));



	*/
	std::uint32_t call_ordertm_addr = 0x013992D0;
#define call_ordertm_CCV __cdecl

	/*

	* calltmres_Decompiled_Args: std::uint32_t Rbx_LuaState
	* calltmres_TypeDef: int
	* calltmres_Aob: 83 7E C 6 75
	* calltmres_ProtectedByRetcheck: false
	* calltmres_IsApartOfLua: true
	* calltmres_Mask: .....
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_calltmres_typedef)(std::uint32_t Rbx_LuaState);
	r_calltmres_typedef r_calltmres = (r_calltmres_typedef)(x(0x01399B10));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(calltmres_CCV* r_calltmres_typedef)(std::uint32_t Rbx_LuaState);
	r_calltmres_typedef r_calltmres = (r_calltmres_typedef)(x(Adresses::calltmres_addr));



	*/
	std::uint32_t calltmres_addr = 0x01399B10;
#define calltmres_CCV __cdecl

	/*

	* close_state_Decompiled_Args: int a1
	* close_state_TypeDef: int
	* close_state_Aob: 55 8B EC 53 8B 5D 8 56
	* close_state_ProtectedByRetcheck: false
	* close_state_IsApartOfLua: true
	* close_state_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_close_state_typedef)(int a1);
	r_close_state_typedef r_close_state = (r_close_state_typedef)(x(0x0138E1E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(close_state_CCV* r_close_state_typedef)(int a1);
	r_close_state_typedef r_close_state = (r_close_state_typedef)(x(Adresses::close_state_addr));



	*/
	std::uint32_t close_state_addr = 0x0138E1E0;
#define close_state_CCV __cdecl

	/*

	* currentline_Decompiled_Args: int a1
	* currentline_TypeDef: int
	* currentline_Aob: 55 8B EC 8B 4D C 53
	* currentline_ProtectedByRetcheck: false
	* currentline_IsApartOfLua: true
	* currentline_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_currentline_typedef)(int a1);
	r_currentline_typedef r_currentline = (r_currentline_typedef)(x(0x0138E980));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(currentline_CCV* r_currentline_typedef)(int a1);
	r_currentline_typedef r_currentline = (r_currentline_typedef)(x(Adresses::currentline_addr));



	*/
	std::uint32_t currentline_addr = 0x0138E980;
#define currentline_CCV __cdecl

	/*

	* err_geterrorstring_Decompiled_Args: int a1
	* err_geterrorstring_TypeDef: int
	* err_geterrorstring_Aob: 55 8B EC 8B 45 8 83
	* err_geterrorstring_ProtectedByRetcheck: false
	* err_geterrorstring_IsApartOfLua: false
	* err_geterrorstring_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_err_geterrorstring_typedef)(int a1);
	r_err_geterrorstring_typedef r_err_geterrorstring = (r_err_geterrorstring_typedef)(x(0x009271F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(err_geterrorstring_CCV* r_err_geterrorstring_typedef)(int a1);
	r_err_geterrorstring_typedef r_err_geterrorstring = (r_err_geterrorstring_typedef)(x(Adresses::err_geterrorstring_addr));



	*/
	std::uint32_t err_geterrorstring_addr = 0x009271F0;
#define err_geterrorstring_CCV __cdecl

	/*

	* f_call_Decompiled_Args: int a1
	* f_call_TypeDef: int
	* f_call_Aob: 55 8B EC 8B 45 C FF
	* f_call_ProtectedByRetcheck: true
	* f_call_IsApartOfLua: true
	* f_call_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_f_call_typedef)(int a1);
	r_f_call_typedef r_f_call = (r_f_call_typedef)unprotect(x(0x013816B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(f_call_CCV* r_f_call_typedef)(int a1);
	r_f_call_typedef r_f_call = (r_f_call_typedef)unprotect(x(Adresses::f_call_addr));



	*/
	std::uint32_t f_call_addr = 0x013816B0;
#define f_call_CCV __cdecl

	/*

	* f_luaopen_Decompiled_Args: int a1
	* f_luaopen_TypeDef: int
	* f_luaopen_Aob: 55 8B EC 53 8B 5D 8 8B
	* f_luaopen_ProtectedByRetcheck: false
	* f_luaopen_IsApartOfLua: true
	* f_luaopen_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_f_luaopen_typedef)(int a1);
	r_f_luaopen_typedef r_f_luaopen = (r_f_luaopen_typedef)(x(0x0138E280));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(f_luaopen_CCV* r_f_luaopen_typedef)(int a1);
	r_f_luaopen_typedef r_f_luaopen = (r_f_luaopen_typedef)(x(Adresses::f_luaopen_addr));



	*/
	std::uint32_t f_luaopen_addr = 0x0138E280;
#define f_luaopen_CCV __cdecl

	/*

	* findfirstchild_Decompiled_Args: int a1
	* findfirstchild_TypeDef: int
	* findfirstchild_Aob: 55 8B EC 83 EC 10 53 8B C1 56 57 89 45 F0 33 FF 8B 40 2C 85 C0 F 84
	* findfirstchild_ProtectedByRetcheck: false
	* findfirstchild_IsApartOfLua: false
	* findfirstchild_Mask: .......................
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_findfirstchild_typedef)(int a1);
	r_findfirstchild_typedef r_findfirstchild = (r_findfirstchild_typedef)(x(0x00679EF0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(findfirstchild_CCV* r_findfirstchild_typedef)(int a1);
	r_findfirstchild_typedef r_findfirstchild = (r_findfirstchild_typedef)(x(Adresses::findfirstchild_addr));



	*/
	std::uint32_t findfirstchild_addr = 0x00679EF0;
#define findfirstchild_CCV __thiscall

	/*

	* findfirstchildofclass_Decompiled_Args: int a1
	* findfirstchildofclass_TypeDef: int
	* findfirstchildofclass_Aob: 55 8B EC 53 56 8B 71 2C 57 85 F6 F 84
	* findfirstchildofclass_ProtectedByRetcheck: false
	* findfirstchildofclass_IsApartOfLua: false
	* findfirstchildofclass_Mask: .............
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_findfirstchildofclass_typedef)(int a1);
	r_findfirstchildofclass_typedef r_findfirstchildofclass = (r_findfirstchildofclass_typedef)(x(0x0067A0C0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(findfirstchildofclass_CCV* r_findfirstchildofclass_typedef)(int a1);
	r_findfirstchildofclass_typedef r_findfirstchildofclass = (r_findfirstchildofclass_typedef)(x(Adresses::findfirstchildofclass_addr));



	*/
	std::uint32_t findfirstchildofclass_addr = 0x0067A0C0;
#define findfirstchildofclass_CCV __thiscall

	/*

	* findlocal_Decompiled_Args: int a1, int a2, int a3
	* findlocal_TypeDef: int
	* findlocal_Aob: 55 8B EC 83 EC C 8B
	* findlocal_ProtectedByRetcheck: false
	* findlocal_IsApartOfLua: true
	* findlocal_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_findlocal_typedef)(int a1, int a2, int a3);
	r_findlocal_typedef r_findlocal = (r_findlocal_typedef)(x(0x013962F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(findlocal_CCV* r_findlocal_typedef)(int a1, int a2, int a3);
	r_findlocal_typedef r_findlocal = (r_findlocal_typedef)(x(Adresses::findlocal_addr));



	*/
	std::uint32_t findlocal_addr = 0x013962F0;
#define findlocal_CCV __cdecl

	/*

	* fireclickdetector_Decompiled_Args: int a1
	* fireclickdetector_TypeDef: int
	* fireclickdetector_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* fireclickdetector_ProtectedByRetcheck: false
	* fireclickdetector_IsApartOfLua: false
	* fireclickdetector_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_fireclickdetector_typedef)(int a1);
	r_fireclickdetector_typedef r_fireclickdetector = (r_fireclickdetector_typedef)(x(0x00C3EBE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(fireclickdetector_CCV* r_fireclickdetector_typedef)(int a1);
	r_fireclickdetector_typedef r_fireclickdetector = (r_fireclickdetector_typedef)(x(Adresses::fireclickdetector_addr));



	*/
	std::uint32_t fireclickdetector_addr = 0x00C3EBE0;
#define fireclickdetector_CCV __thiscall

	/*

	* flog_getvalue_Decompiled_Args: int a1, int a2, int a3
	* flog_getvalue_TypeDef: bool
	* flog_getvalue_Aob: 55 8B EC 83 EC 1C 53 8B 5D 8 56
	* flog_getvalue_ProtectedByRetcheck: false
	* flog_getvalue_IsApartOfLua: false
	* flog_getvalue_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_flog_getvalue_typedef)(int a1, int a2, int a3);
	r_flog_getvalue_typedef r_flog_getvalue = (r_flog_getvalue_typedef)(x(0x01574C50));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(flog_getvalue_CCV* r_flog_getvalue_typedef)(int a1, int a2, int a3);
	r_flog_getvalue_typedef r_flog_getvalue = (r_flog_getvalue_typedef)(x(Adresses::flog_getvalue_addr));



	*/
	std::uint32_t flog_getvalue_addr = 0x01574C50;
#define flog_getvalue_CCV __cdecl

	/*

	* flog_setvalue_Decompiled_Args: int a1, const char* a2, int a3
	* flog_setvalue_TypeDef: int
	* flog_setvalue_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* flog_setvalue_ProtectedByRetcheck: false
	* flog_setvalue_IsApartOfLua: false
	* flog_setvalue_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_flog_setvalue_typedef)(int a1, const char* a2, int a3);
	r_flog_setvalue_typedef r_flog_setvalue = (r_flog_setvalue_typedef)(x(0x01575410));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(flog_setvalue_CCV* r_flog_setvalue_typedef)(int a1, const char* a2, int a3);
	r_flog_setvalue_typedef r_flog_setvalue = (r_flog_setvalue_typedef)(x(Adresses::flog_setvalue_addr));



	*/
	std::uint32_t flog_setvalue_addr = 0x01575410;
#define flog_setvalue_CCV __cdecl

	/*

	* getdatamodel_Decompiled_Args: int a1
	* getdatamodel_TypeDef: int
	* getdatamodel_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* getdatamodel_ProtectedByRetcheck: false
	* getdatamodel_IsApartOfLua: false
	* getdatamodel_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_getdatamodel_typedef)(int a1);
	r_getdatamodel_typedef r_getdatamodel = (r_getdatamodel_typedef)(x(0x00F89F60));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(getdatamodel_CCV* r_getdatamodel_typedef)(int a1);
	r_getdatamodel_typedef r_getdatamodel = (r_getdatamodel_typedef)(x(Adresses::g15771E0etdatamodel_addr));



	*/
	std::uint32_t getdatamodel_addr = 0x00F89F60;
#define getdatamodel_CCV __thiscall

	/*

	* getdatamodel2_Decompiled_Args: int
	* getdatamodel2_TypeDef: int
	* getdatamodel2_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* getdatamodel2_ProtectedByRetcheck: false
	* getdatamodel2_IsApartOfLua: false
	* getdatamodel2_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_getdatamodel2_typedef)(int);
	r_getdatamodel2_typedef r_getdatamodel2 = (r_getdatamodel2_typedef)(x(0x00F89E10));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(getdatamodel2_CCV* r_getdatamodel2_typedef)(int);
	r_getdatamodel2_typedef r_getdatamodel2 = (r_getdatamodel2_typedef)(x(Adresses::getdatamodel2_addr));



	*/
	std::uint32_t getdatamodel2_addr = 0x00F89E10;
#define getdatamodel2_CCV __cdecl

	/*

	* getfunc_Decompiled_Args: int a1, int a2
	* getfunc_TypeDef: int
	* getfunc_Aob: 55 8B EC 81 EC 24 1 0 0 56
	* getfunc_ProtectedByRetcheck: false
	* getfunc_IsApartOfLua: true
	* getfunc_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_getfunc_typedef)(int a1, int a2);
	r_getfunc_typedef r_getfunc = (r_getfunc_typedef)(x(0x0138FAC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(getfunc_CCV* r_getfunc_typedef)(int a1, int a2);
	r_getfunc_typedef r_getfunc = (r_getfunc_typedef)(x(Adresses::getfunc_addr));



	*/
	std::uint32_t getfunc_addr = 0x0138FAC0;
#define getfunc_CCV __cdecl

	/*

	* getjobsbyname_Decompiled_Args: int
	* getjobsbyname_TypeDef: int
	* getjobsbyname_Aob: 55 8B EC E8 E8 FD FF FF FF 75 C 8B
	* getjobsbyname_ProtectedByRetcheck: false
	* getjobsbyname_IsApartOfLua: false
	* getjobsbyname_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_getjobsbyname_typedef)(int);
	r_getjobsbyname_typedef r_getjobsbyname = (r_getjobsbyname_typedef)(x(0x010163E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(getjobsbyname_CCV* r_getjobsbyname_typedef)(int);
	r_getjobsbyname_typedef r_getjobsbyname = (r_getjobsbyname_typedef)(x(Adresses::getjobsbyname_addr));



	*/
	std::uint32_t getjobsbyname_addr = 0x010163E0;
#define getjobsbyname_CCV __cdecl

	/*

	* getnamecallmethod_Decompiled_Args: int a1
	* getnamecallmethod_TypeDef: int
	* getnamecallmethod_Aob: 55 8B EC 56 57 8B 7D 8 8B
	* getnamecallmethod_ProtectedByRetcheck: false
	* getnamecallmethod_IsApartOfLua: true
	* getnamecallmethod_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_getnamecallmethod_typedef)(int a1);
	r_getnamecallmethod_typedef r_getnamecallmethod = (r_getnamecallmethod_typedef)(x(0x01383CC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(getnamecallmethod_CCV* r_getnamecallmethod_typedef)(int a1);
	r_getnamecallmethod_typedef r_getnamecallmethod = (r_getnamecallmethod_typedef)(x(Adresses::getnamecallmethod_addr));



	*/
	std::uint32_t getnamecallmethod_addr = 0x01383CC0;
#define getnamecallmethod_CCV __cdecl

	/*

	* grabscriptcontext_Decompiled_Args: int a1
	* grabscriptcontext_TypeDef: int
	* grabscriptcontext_Aob: 55 8B EC 51 53 56 8B F1 57 8B 7D C 8B
	* grabscriptcontext_ProtectedByRetcheck: false
	* grabscriptcontext_IsApartOfLua: false
	* grabscriptcontext_Mask: .............
	* Usage (Not 100% Accurate):

	typedef int(__stdcall* r_grabscriptcontext_typedef)(int a1);
	r_grabscriptcontext_typedef r_grabscriptcontext = (r_grabscriptcontext_typedef)(x(0x005E4A80));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(grabscriptcontext_CCV* r_grabscriptcontext_typedef)(int a1);
	r_grabscriptcontext_typedef r_grabscriptcontext = (r_grabscriptcontext_typedef)(x(Adresses::grabscriptcontext_addr));



	*/
	std::uint32_t grabscriptcontext_addr = 0x005E4A80;
#define grabscriptcontext_CCV __stdcall

	/*

	* grabglobalstateindex_Decompiled_Args: int
	* grabglobalstateindex_TypeDef: int
	* grabglobalstateindex_Aob: 55 8B EC 56 57 6A 5 FF
	* grabglobalstateindex_ProtectedByRetcheck: false
	* grabglobalstateindex_IsApartOfLua: false
	* grabglobalstateindex_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_grabglobalstateindex_typedef)(int);
	r_grabglobalstateindex_typedef r_grabglobalstateindex = (r_grabglobalstateindex_typedef)(x(0x00788FA0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(grabglobalstateindex_CCV* r_grabglobalstateindex_typedef)(int);
	r_grabglobalstateindex_typedef r_grabglobalstateindex = (r_grabglobalstateindex_typedef)(x(Adresses::grabglobalstateindex_addr));



	*/
	std::uint32_t grabglobalstateindex_addr = 0x00788FA0;
#define grabglobalstateindex_CCV __thiscall

	/*

	* growci_Decompiled_Args: int a1
	* growci_TypeDef: int
	* growci_Aob: 55 8B EC 56 8B 75 8 8B
	* growci_ProtectedByRetcheck: false
	* growci_IsApartOfLua: true
	* growci_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_growci_typedef)(int a1);
	r_growci_typedef r_growci = (r_growci_typedef)(x(0x013848E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(growci_CCV* r_growci_typedef)(int a1);
	r_growci_typedef r_growci = (r_growci_typedef)(x(Adresses::growci_addr));



	*/
	std::uint32_t growci_addr = 0x013848E0;
#define growci_CCV __cdecl

	/*

	* hashnum_Decompiled_Args: int a1, int a2, int a3
	* hashnum_TypeDef: int
	* hashnum_Aob: 55 8B EC 8B 4D 10 81 E1 FF FF FF 7F 8B C1 C1 E8 12 33 45 C 53
	* hashnum_ProtectedByRetcheck: false
	* hashnum_IsApartOfLua: true
	* hashnum_Mask: .....................
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_hashnum_typedef)(int a1, int a2, int a3);
	r_hashnum_typedef r_hashnum = (r_hashnum_typedef)(x(0x01397DD0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(hashnum_CCV* r_hashnum_typedef)(int a1, int a2, int a3);
	r_hashnum_typedef r_hashnum = (r_hashnum_typedef)(x(Adresses::hashnum_addr));



	*/
	std::uint32_t hashnum_addr = 0x01397DD0;
#define hashnum_CCV __cdecl

	/*

	* index2adr_Decompiled_Args: std::uint32_t Rbx_LuaState
	* index2adr_TypeDef: int
	* index2adr_Aob: 81 FA F0 D8 FF FF 7E 9 C1
	* index2adr_ProtectedByRetcheck: false
	* index2adr_IsApartOfLua: true
	* index2adr_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_index2adr_typedef)(std::uint32_t Rbx_LuaState);
	r_index2adr_typedef r_index2adr = (r_index2adr_typedef)(x(0x01381700));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(index2adr_CCV* r_index2adr_typedef)(std::uint32_t Rbx_LuaState);
	r_index2adr_typedef r_index2adr = (r_index2adr_typedef)(x(Adresses::index2adr_addr));



	*/
	std::uint32_t index2adr_addr = 0x01381700;
#define index2adr_CCV __thiscall

	/*

	* luaopen_base_Decompiled_Args: int a1
	* luaopen_base_TypeDef: int
	* luaopen_base_Aob: 55 8B EC 56 8B 75 8 56
	* luaopen_base_ProtectedByRetcheck: false
	* luaopen_base_IsApartOfLua: true
	* luaopen_base_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaopen_base_typedef)(int a1);
	r_luaopen_base_typedef r_luaopen_base = (r_luaopen_base_typedef)(x(0x0138FEE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaopen_base_CCV* r_luaopen_base_typedef)(int a1);
	r_luaopen_base_typedef r_luaopen_base = (r_luaopen_base_typedef)(x(Adresses::luaopen_base_addr));



	*/
	std::uint32_t luaopen_base_addr = 0x0138FEE0;
#define luaopen_base_CCV __cdecl

	/*

	* luaopen_table_Decompiled_Args: int a1
	* luaopen_table_TypeDef: int
	* luaopen_table_Aob: 55 8B EC 56 8B 75 8 68
	* luaopen_table_ProtectedByRetcheck: false
	* luaopen_table_IsApartOfLua: true
	* luaopen_table_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaopen_table_typedef)(int a1);
	r_luaopen_table_typedef r_luaopen_table = (r_luaopen_table_typedef)(x(0x01390740));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaopen_table_CCV* r_luaopen_table_typedef)(int a1);
	r_luaopen_table_typedef r_luaopen_table = (r_luaopen_table_typedef)(x(Adresses::luaopen_table_addr));



	*/
	std::uint32_t luaopen_table_addr = 0x01390740;
#define luaopen_table_CCV __cdecl

	/*

	* luaopen_bit32_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luaopen_bit32_TypeDef: int
	* luaopen_bit32_Aob: 55 8B EC 68 10 9D 71 2 68
	* luaopen_bit32_ProtectedByRetcheck: false
	* luaopen_bit32_IsApartOfLua: true
	* luaopen_bit32_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaopen_bit32_typedef)(std::uint32_t Rbx_LuaState);
	r_luaopen_bit32_typedef r_luaopen_bit32 = (r_luaopen_bit32_typedef)(x(0x01394F90));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaopen_bit32_CCV* r_luaopen_bit32_typedef)(std::uint32_t Rbx_LuaState);
	r_luaopen_bit32_typedef r_luaopen_bit32 = (r_luaopen_bit32_typedef)(x(Adresses::luaopen_bit32_addr));



	*/
	std::uint32_t luaopen_bit32_addr = 0x01394F90;
#define luaopen_bit32_CCV __cdecl

	/*

	* lua_call_Decompiled_Args: int a1, int a2, int a3
	* lua_call_TypeDef: int
	* lua_call_Aob: 55 8B EC 8B 4D C 57
	* lua_call_ProtectedByRetcheck: true
	* lua_call_IsApartOfLua: true
	* lua_call_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_call_typedef)(int a1, int a2, int a3);
	r_lua_call_typedef r_lua_call = (r_lua_call_typedef)unprotect(x(0x01381800));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_call_CCV* r_lua_call_typedef)(int a1, int a2, int a3);
	r_lua_call_typedef r_lua_call = (r_lua_call_typedef)unprotect(x(Adresses::lua_call_addr));



	*/
	std::uint32_t lua_call_addr = 0x01381800;
#define lua_call_CCV __cdecl

	/*

	* lua_checkstack_Decompiled_Args: int a1, int a2
	* lua_checkstack_TypeDef: int
	* lua_checkstack_Aob: 55 8B EC 8B 4D C 56
	* lua_checkstack_ProtectedByRetcheck: false
	* lua_checkstack_IsApartOfLua: true
	* lua_checkstack_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_checkstack_typedef)(int a1, int a2);
	r_lua_checkstack_typedef r_lua_checkstack = (r_lua_checkstack_typedef)(x(0x01381870));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_checkstack_CCV* r_lua_checkstack_typedef)(int a1, int a2);
	r_lua_checkstack_typedef r_lua_checkstack = (r_lua_checkstack_typedef)(x(Adresses::lua_checkstack_addr));



	*/
	std::uint32_t lua_checkstack_addr = 0x01381870;
#define lua_checkstack_CCV __cdecl

	/*

	* lua_close_Decompiled_Args: int a1
	* lua_close_TypeDef: int
	* lua_close_Aob: 55 8B EC 8B 4D 8 83
	* lua_close_ProtectedByRetcheck: false
	* lua_close_IsApartOfLua: true
	* lua_close_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_close_typedef)(int a1);
	r_lua_close_typedef r_lua_close = (r_lua_close_typedef)(x(0x0138E410));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_close_CCV* r_lua_close_typedef)(int a1);
	r_lua_close_typedef r_lua_close = (r_lua_close_typedef)(x(Adresses::lua_close_addr));



	*/
	std::uint32_t lua_close_addr = 0x0138E410;
#define lua_close_CCV __cdecl

	/*

	* lua_concat_Decompiled_Args: int a1, int a2
	* lua_concat_TypeDef: int
	* lua_concat_Aob: 55 8B EC 56 8B 75 C 57
	* lua_concat_ProtectedByRetcheck: true
	* lua_concat_IsApartOfLua: true
	* lua_concat_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_concat_typedef)(int a1, int a2);
	r_lua_concat_typedef r_lua_concat = (r_lua_concat_typedef)unprotect(x(0x01381910));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_concat_CCV* r_lua_concat_typedef)(int a1, int a2);
	r_lua_concat_typedef r_lua_concat = (r_lua_concat_typedef)unprotect(x(Adresses::lua_concat_addr));



	*/
	std::uint32_t lua_concat_addr = 0x01381910;
#define lua_concat_CCV __cdecl

	/*

	* lua_createtable_Decompiled_Args: int a1
	* lua_createtable_TypeDef: int
	* lua_createtable_Aob: 55 8B EC 57 8B 7D 8 8B
	* lua_createtable_ProtectedByRetcheck: true
	* lua_createtable_IsApartOfLua: true
	* lua_createtable_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_createtable_typedef)(int a1);
	r_lua_createtable_typedef r_lua_createtable = (r_lua_createtable_typedef)unprotect(x(0x013819C0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_createtable_CCV* r_lua_createtable_typedef)(int a1);
	r_lua_createtable_typedef r_lua_createtable = (r_lua_createtable_typedef)unprotect(x(Adresses::lua_createtable_addr));



	*/
	std::uint32_t lua_createtable_addr = 0x013819C0;
#define lua_createtable_CCV __cdecl

	/*

	* lua_error_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lua_error_TypeDef: int
	* lua_error_Aob: 55 8B EC 6A 2 FF 75
	* lua_error_ProtectedByRetcheck: false
	* lua_error_IsApartOfLua: true
	* lua_error_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_error_typedef)(std::uint32_t Rbx_LuaState);
	r_lua_error_typedef r_lua_error = (r_lua_error_typedef)(x(0x01381B00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_error_CCV* r_lua_error_typedef)(std::uint32_t Rbx_LuaState);
	r_lua_error_typedef r_lua_error = (r_lua_error_typedef)(x(Adresses::lua_error_addr));



	*/
	std::uint32_t lua_error_addr = 0x01381B00;
#define lua_error_CCV __cdecl

	/*

	* lua_freeobj_Decompiled_Args: int a1
	* lua_freeobj_TypeDef: int
	* lua_freeobj_Aob: 55 8B EC 51 53 57 8B 7D 8 8B
	* lua_freeobj_ProtectedByRetcheck: false
	* lua_freeobj_IsApartOfLua: true
	* lua_freeobj_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_freeobj_typedef)(int a1);
	r_lua_freeobj_typedef r_lua_freeobj = (r_lua_freeobj_typedef)(x(0x01396700));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_freeobj_CCV* r_lua_freeobj_typedef)(int a1);
	r_lua_freeobj_typedef r_lua_freeobj = (r_lua_freeobj_typedef)(x(Adresses::lua_freeobj_addr));



	*/
	std::uint32_t lua_freeobj_addr = 0x01396700;
#define lua_freeobj_CCV __cdecl

	/*

	* lua_gc_Decompiled_Args: int a1, int a2, int a3
	* lua_gc_TypeDef: int
	* lua_gc_Aob: 55 8B EC 53 56 57 8B 7D 8 33
	* lua_gc_ProtectedByRetcheck: false
	* lua_gc_IsApartOfLua: true
	* lua_gc_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_gc_typedef)(int a1, int a2, int a3);
	r_lua_gc_typedef r_lua_gc = (r_lua_gc_typedef)(x(0x01381B10));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_gc_CCV* r_lua_gc_typedef)(int a1, int a2, int a3);
	r_lua_gc_typedef r_lua_gc = (r_lua_gc_typedef)(x(Adresses::lua_gc_addr));



	*/
	std::uint32_t lua_gc_addr = 0x01381B10;
#define lua_gc_CCV __cdecl

	/*

	* lua_getargument_Decompiled_Args: int a1, int a2, int a3
	* lua_getargument_TypeDef: int
	* lua_getargument_Aob: 55 8B EC 8B 45 C 53
	* lua_getargument_ProtectedByRetcheck: true
	* lua_getargument_IsApartOfLua: true
	* lua_getargument_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_getargument_typedef)(int a1, int a2, int a3);
	r_lua_getargument_typedef r_lua_getargument = (r_lua_getargument_typedef)unprotect(x(0x0138EDE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getargument_CCV* r_lua_getargument_typedef)(int a1, int a2, int a3);
	r_lua_getargument_typedef r_lua_getargument = (r_lua_getargument_typedef)unprotect(x(Adresses::lua_getargument_addr));



	*/
	std::uint32_t lua_getargument_addr = 0x0138EDE0;
#define lua_getargument_CCV __cdecl

	/*

	* lua_getfenv_Decompiled_Args: int a1, int a2
	* lua_getfenv_TypeDef: int
	* lua_getfenv_Aob: 55 8B EC 8B 55 C 56
	* lua_getfenv_ProtectedByRetcheck: true
	* lua_getfenv_IsApartOfLua: true
	* lua_getfenv_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_getfenv_typedef)(int a1, int a2);
	r_lua_getfenv_typedef r_lua_getfenv = (r_lua_getfenv_typedef)unprotect(x(0x01381C30));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getfenv_CCV* r_lua_getfenv_typedef)(int a1, int a2);
	r_lua_getfenv_typedef r_lua_getfenv = (r_lua_getfenv_typedef)unprotect(x(Adresses::lua_getfenv_addr));



	*/
	std::uint32_t lua_getfenv_addr = 0x01381C30;
#define lua_getfenv_CCV __cdecl

	/*

	* lua_getfield_Decompiled_Args: const char* a1
	* lua_getfield_TypeDef: int
	* lua_getfield_Aob: 55 8B EC 83 EC 10 56 8B F1 57 85 D2 7E 18 8B 46 8 BF
	* lua_getfield_ProtectedByRetcheck: true
	* lua_getfield_IsApartOfLua: true
	* lua_getfield_Mask: ..................
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_lua_getfield_typedef)(const char* a1);
	r_lua_getfield_typedef r_lua_getfield = (r_lua_getfield_typedef)unprotect(x(0x01381CD0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getfield_CCV* r_lua_getfield_typedef)(const char* a1);
	r_lua_getfield_typedef r_lua_getfield = (r_lua_getfield_typedef)unprotect(x(Adresses::lua_getfield_addr));



	*/
	std::uint32_t lua_getfield_addr = 0x01381CD0;
#define lua_getfield_CCV __fastcall

	/*

	* lua_getinfo_Decompiled_Args: int a1, int a2, int a3
	* lua_getinfo_TypeDef: int
	* lua_getinfo_Aob: 55 8B EC 51 8B 55 10 53 8B 5D C 56
	* lua_getinfo_ProtectedByRetcheck: true
	* lua_getinfo_IsApartOfLua: true
	* lua_getinfo_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_getinfo_typedef)(int a1, int a2, int a3);
	r_lua_getinfo_typedef r_lua_getinfo = (r_lua_getinfo_typedef)unprotect(x(0x0138EEA0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getinfo_CCV* r_lua_getinfo_typedef)(int a1, int a2, int a3);
	r_lua_getinfo_typedef r_lua_getinfo = (r_lua_getinfo_typedef)unprotect(x(Adresses::lua_getinfo_addr));



	*/
	std::uint32_t lua_getinfo_addr = 0x0138EEA0;
#define lua_getinfo_CCV __cdecl

	/*

	* lua_getlocal_Decompiled_Args: int a1, int a2
	* lua_getlocal_TypeDef: int
	* lua_getlocal_Aob: 55 8B EC 8B 45 C 53
	* lua_getlocal_ProtectedByRetcheck: true
	* lua_getlocal_IsApartOfLua: true
	* lua_getlocal_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_getlocal_typedef)(int a1, int a2);
	r_lua_getlocal_typedef r_lua_getlocal = (r_lua_getlocal_typedef)unprotect(x(0x0138EF70));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getlocal_CCV* r_lua_getlocal_typedef)(int a1, int a2);
	r_lua_getlocal_typedef r_lua_getlocal = (r_lua_getlocal_typedef)unprotect(x(Adresses::lua_getlocal_addr));



	*/
	std::uint32_t lua_getlocal_addr = 0x0138EF70;
#define lua_getlocal_CCV __thiscall

	/*

	* lua_getmetatable_Decompiled_Args: int a1, int a2
	* lua_getmetatable_TypeDef: int
	* lua_getmetatable_Aob: 55 8B EC 8B 55 C 56
	* lua_getmetatable_ProtectedByRetcheck: false
	* lua_getmetatable_IsApartOfLua: true
	* lua_getmetatable_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__stdcall* r_lua_getmetatable_typedef)(int a1, int a2);
	r_lua_getmetatable_typedef r_lua_getmetatable = (r_lua_getmetatable_typedef)(x(0x01381D70));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getmetatable_CCV* r_lua_getmetatable_typedef)(int a1, int a2);
	r_lua_getmetatable_typedef r_lua_getmetatable = (r_lua_getmetatable_typedef)(x(Adresses::lua_getmetatable_addr));



	*/
	std::uint32_t lua_getmetatable_addr = 0x01381D70;
#define lua_getmetatable_CCV __stdcall

	/*

	* lua_getstack_Decompiled_Args: int a1, int a2, int a3
	* lua_getstack_TypeDef: int
	* lua_getstack_Aob: 55 8B EC 8B 4D C B8
	* lua_getstack_ProtectedByRetcheck: false
	* lua_getstack_IsApartOfLua: true
	* lua_getstack_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_getstack_typedef)(int a1, int a2, int a3);
	r_lua_getstack_typedef r_lua_getstack = (r_lua_getstack_typedef)(x(0x0138F030));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getstack_CCV* r_lua_getstack_typedef)(int a1, int a2, int a3);
	r_lua_getstack_typedef r_lua_getstack = (r_lua_getstack_typedef)(x(Adresses::lua_getstack_addr));



	*/
	std::uint32_t lua_getstack_addr = 0x0138F030;
#define lua_getstack_CCV __cdecl

	/*

	* lua_gettable_Decompiled_Args: int a1, int a2
	* lua_gettable_TypeDef: int
	* lua_gettable_Aob: 55 8B EC 8B 55 C 56
	* lua_gettable_ProtectedByRetcheck: true
	* lua_gettable_IsApartOfLua: true
	* lua_gettable_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_gettable_typedef)(int a1, int a2);
	r_lua_gettable_typedef r_lua_gettable = (r_lua_gettable_typedef)unprotect(x(0x01381E30));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_gettable_CCV* r_lua_gettable_typedef)(int a1, int a2);
	r_lua_gettable_typedef r_lua_gettable = (r_lua_gettable_typedef)unprotect(x(Adresses::lua_gettable_addr));



	*/
	std::uint32_t lua_gettable_addr = 0x01381E30;
#define lua_gettable_CCV __cdecl

	/*

	* lua_gettop_Decompiled_Args: int a1
	* lua_gettop_TypeDef: int
	* lua_gettop_Aob: 55 8B EC 8B 4D 8 8B
	* lua_gettop_ProtectedByRetcheck: false
	* lua_gettop_IsApartOfLua: true
	* lua_gettop_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_gettop_typedef)(int a1);
	r_lua_gettop_typedef r_lua_gettop = (r_lua_gettop_typedef)(x(0x01381EA0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_gettop_CCV* r_lua_gettop_typedef)(int a1);
	r_lua_gettop_typedef r_lua_gettop = (r_lua_gettop_typedef)(x(Adresses::lua_gettop_addr));



	*/
	std::uint32_t lua_gettop_addr = 0x01381EA0;
#define lua_gettop_CCV __cdecl

	/*

	* lua_getupvalue_Decompiled_Args: int a1, int a2
	* lua_getupvalue_TypeDef: int
	* lua_getupvalue_Aob: 55 8B EC 8B 55 C 56
	* lua_getupvalue_ProtectedByRetcheck: true
	* lua_getupvalue_IsApartOfLua: true
	* lua_getupvalue_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_getupvalue_typedef)(int a1, int a2);
	r_lua_getupvalue_typedef r_lua_getupvalue = (r_lua_getupvalue_typedef)unprotect(x(0x01381EC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_getupvalue_CCV* r_lua_getupvalue_typedef)(int a1, int a2);
	r_lua_getupvalue_typedef r_lua_getupvalue = (r_lua_getupvalue_typedef)unprotect(x(Adresses::lua_getupvalue_addr));



	*/
	std::uint32_t lua_getupvalue_addr = 0x01381EC0;
#define lua_getupvalue_CCV __thiscall

	/*

	* lua_insert_Decompiled_Args: int a1, int a2
	* lua_insert_TypeDef: int
	* lua_insert_Aob: 55 8B EC 8B 55 C 56
	* lua_insert_ProtectedByRetcheck: true
	* lua_insert_IsApartOfLua: true
	* lua_insert_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_insert_typedef)(int a1, int a2);
	r_lua_insert_typedef r_lua_insert = (r_lua_insert_typedef)unprotect(x(0x01381F50));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_insert_CCV* r_lua_insert_typedef)(int a1, int a2);
	r_lua_insert_typedef r_lua_insert = (r_lua_insert_typedef)unprotect(x(Adresses::lua_insert_addr));



	*/
	std::uint32_t lua_insert_addr = 0x01381F50;
#define lua_insert_CCV __cdecl

	/*

	* lua_iscfunction_Decompiled_Args: int a1, int a2
	* lua_iscfunction_TypeDef: int
	* lua_iscfunction_Aob: 55 8B EC 8B 55 C 8B
	* lua_iscfunction_ProtectedByRetcheck: false
	* lua_iscfunction_IsApartOfLua: true
	* lua_iscfunction_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_iscfunction_typedef)(int a1, int a2);
	r_lua_iscfunction_typedef r_lua_iscfunction = (r_lua_iscfunction_typedef)(x(0x01382020));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_iscfunction_CCV* r_lua_iscfunction_typedef)(int a1, int a2);
	r_lua_iscfunction_typedef r_lua_iscfunction = (r_lua_iscfunction_typedef)(x(Adresses::lua_iscfunction_addr));



	*/
	std::uint32_t lua_iscfunction_addr = 0x01382020;
#define lua_iscfunction_CCV __cdecl

	/*

	* lua_isnumber_Decompiled_Args: int a1, int a2
	* lua_isnumber_TypeDef: int
	* lua_isnumber_Aob: 55 8B EC 8B 55 C 83
	* lua_isnumber_ProtectedByRetcheck: false
	* lua_isnumber_IsApartOfLua: true
	* lua_isnumber_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_isnumber_typedef)(int a1, int a2);
	r_lua_isnumber_typedef r_lua_isnumber = (r_lua_isnumber_typedef)(x(0x01382070));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_isnumber_CCV* r_lua_isnumber_typedef)(int a1, int a2);
	r_lua_isnumber_typedef r_lua_isnumber = (r_lua_isnumber_typedef)(x(Adresses::lua_isnumber_addr));



	*/
	std::uint32_t lua_isnumber_addr = 0x01382070;
#define lua_isnumber_CCV __cdecl

	/*

	* lua_isstring_Decompiled_Args: int a1, int a2
	* lua_isstring_TypeDef: int
	* lua_isstring_Aob: 55 8B EC 8B 55 C 8B
	* lua_isstring_ProtectedByRetcheck: false
	* lua_isstring_IsApartOfLua: true
	* lua_isstring_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_isstring_typedef)(int a1, int a2);
	r_lua_isstring_typedef r_lua_isstring = (r_lua_isstring_typedef)(x(0x013820D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_isstring_CCV* r_lua_isstring_typedef)(int a1, int a2);
	r_lua_isstring_typedef r_lua_isstring = (r_lua_isstring_typedef)(x(Adresses::lua_isstring_addr));



	*/
	std::uint32_t lua_isstring_addr = 0x013820D0;
#define lua_isstring_CCV __cdecl

	/*

	* lua_isuserdata_Decompiled_Args: int a1, int a2
	* lua_isuserdata_TypeDef: int
	* lua_isuserdata_Aob: 55 8B EC 8B 55 C 8B
	* lua_isuserdata_ProtectedByRetcheck: false
	* lua_isuserdata_IsApartOfLua: true
	* lua_isuserdata_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_isuserdata_typedef)(int a1, int a2);
	r_lua_isuserdata_typedef r_lua_isuserdata = (r_lua_isuserdata_typedef)(x(0x01381FD0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_isuserdata_CCV* r_lua_isuserdata_typedef)(int a1, int a2);
	r_lua_isuserdata_typedef r_lua_isuserdata = (r_lua_isuserdata_typedef)(x(Adresses::lua_isuserdata_addr));



	*/
	std::uint32_t lua_isuserdata_addr = 0x01381FD0;
#define lua_isuserdata_CCV __cdecl

	/*

	* lua_lessthan_Decompiled_Args: int a1, int a2, int a3
	* lua_lessthan_TypeDef: int
	* lua_lessthan_Aob: 55 8B EC 8B 55 C 53
	* lua_lessthan_ProtectedByRetcheck: true
	* lua_lessthan_IsApartOfLua: true
	* lua_lessthan_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_lessthan_typedef)(int a1, int a2, int a3);
	r_lua_lessthan_typedef r_lua_lessthan = (r_lua_lessthan_typedef)unprotect(x(0x01382120));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_lessthan_CCV* r_lua_lessthan_typedef)(int a1, int a2, int a3);
	r_lua_lessthan_typedef r_lua_lessthan = (r_lua_lessthan_typedef)unprotect(x(Adresses::lua_lessthan_addr));



	*/
	std::uint32_t lua_lessthan_addr = 0x01382120;
#define lua_lessthan_CCV __cdecl

	/*

	* lua_newstate_Decompiled_Args: int a1, int a2
	* lua_newstate_TypeDef: int
	* lua_newstate_Aob: 55 8B EC 83 EC 8 53
	* lua_newstate_ProtectedByRetcheck: false
	* lua_newstate_IsApartOfLua: true
	* lua_newstate_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_newstate_typedef)(int a1, int a2);
	r_lua_newstate_typedef r_lua_newstate = (r_lua_newstate_typedef)(x(0x0138E440));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_newstate_CCV* r_lua_newstate_typedef)(int a1, int a2);
	r_lua_newstate_typedef r_lua_newstate = (r_lua_newstate_typedef)(x(Adresses::lua_newstate_addr));



	*/
	std::uint32_t lua_newstate_addr = 0x0138E440;
#define lua_newstate_CCV __cdecl

	/*

	* lua_newthread_Decompiled_Args: int a1
	* lua_newthread_TypeDef: int
	* lua_newthread_Aob: 55 8B EC 53 56 8B 75 8 8B
	* lua_newthread_ProtectedByRetcheck: true
	* lua_newthread_IsApartOfLua: true
	* lua_newthread_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_newthread_typedef)(int a1);
	r_lua_newthread_typedef r_lua_newthread = (r_lua_newthread_typedef)unprotect(x(0x013821D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_newthread_CCV* r_lua_newthread_typedef)(int a1);
	r_lua_newthread_typedef r_lua_newthread = (r_lua_newthread_typedef)unprotect(x(Adresses::lua_newthread_addr));



	*/
	std::uint32_t lua_newthread_addr = 0x013821D0;
#define lua_newthread_CCV __cdecl

	/*

	* lua_newuserdata_Decompiled_Args: int a1
	* lua_newuserdata_TypeDef: int
	* lua_newuserdata_Aob: 55 8B EC 56 8B 75 8 8B
	* lua_newuserdata_ProtectedByRetcheck: true
	* lua_newuserdata_IsApartOfLua: true
	* lua_newuserdata_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_lua_newuserdata_typedef)(int a1);
	r_lua_newuserdata_typedef r_lua_newuserdata = (r_lua_newuserdata_typedef)unprotect(x(0x01382260));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_newuserdata_CCV* r_lua_newuserdata_typedef)(int a1);
	r_lua_newuserdata_typedef r_lua_newuserdata = (r_lua_newuserdata_typedef)unprotect(x(Adresses::lua_newuserdata_addr));



	*/
	std::uint32_t lua_newuserdata_addr = 0x01382260;
#define lua_newuserdata_CCV __fastcall

	/*

	* lua_next_Decompiled_Args: int a1, int a2
	* lua_next_TypeDef: int
	* lua_next_Aob: 55 8B EC 8B 55 C 56
	* lua_next_ProtectedByRetcheck: true
	* lua_next_IsApartOfLua: true
	* lua_next_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_next_typedef)(int a1, int a2);
	r_lua_next_typedef r_lua_next = (r_lua_next_typedef)unprotect(x(0x013822E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_next_CCV* r_lua_next_typedef)(int a1, int a2);
	r_lua_next_typedef r_lua_next = (r_lua_next_typedef)unprotect(x(Adresses::lua_next_addr));



	*/
	std::uint32_t lua_next_addr = 0x013822E0;
#define lua_next_CCV __cdecl

	/*

	* lua_objlen_Decompiled_Args: int a1, int a2
	* lua_objlen_TypeDef: int
	* lua_objlen_Aob: 55 8B EC 8B 55 C 56
	* lua_objlen_ProtectedByRetcheck: false
	* lua_objlen_IsApartOfLua: true
	* lua_objlen_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_objlen_typedef)(int a1, int a2);
	r_lua_objlen_typedef r_lua_objlen = (r_lua_objlen_typedef)(x(0x01382370));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_objlen_CCV* r_lua_objlen_typedef)(int a1, int a2);
	r_lua_objlen_typedef r_lua_objlen = (r_lua_objlen_typedef)(x(Adresses::lua_objlen_addr));



	*/
	std::uint32_t lua_objlen_addr = 0x01382370;
#define lua_objlen_CCV __cdecl

	/*

	* lua_pcall_Decompiled_Args: int a1, int a2, int a3, int a4
	* lua_pcall_TypeDef: int
	* lua_pcall_Aob: 55 8B EC 8B 55 14 83 EC 8 53
	* lua_pcall_ProtectedByRetcheck: true
	* lua_pcall_IsApartOfLua: true
	* lua_pcall_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_pcall_typedef)(int a1, int a2, int a3, int a4);
	r_lua_pcall_typedef r_lua_pcall = (r_lua_pcall_typedef)unprotect(x(0x01382440));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pcall_CCV* r_lua_pcall_typedef)(int a1, int a2, int a3, int a4);
	r_lua_pcall_typedef r_lua_pcall = (r_lua_pcall_typedef)unprotect(x(Adresses::lua_pcall_addr));



	*/
	std::uint32_t lua_pcall_addr = 0x01382440;
#define lua_pcall_CCV __cdecl

	/*

	* lua_pushboolean_Decompiled_Args: int a1, int a2
	* lua_pushboolean_TypeDef: int
	* lua_pushboolean_Aob: 55 8B EC 8B 55 8 33
	* lua_pushboolean_ProtectedByRetcheck: true
	* lua_pushboolean_IsApartOfLua: true
	* lua_pushboolean_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_pushboolean_typedef)(int a1, int a2);
	r_lua_pushboolean_typedef r_lua_pushboolean = (r_lua_pushboolean_typedef)unprotect(x(0x01382500));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushboolean_CCV* r_lua_pushboolean_typedef)(int a1, int a2);
	r_lua_pushboolean_typedef r_lua_pushboolean = (r_lua_pushboolean_typedef)unprotect(x(Adresses::lua_pushboolean_addr));



	*/
	std::uint32_t lua_pushboolean_addr = 0x01382500;
#define lua_pushboolean_CCV __thiscall

	/*

	* lua_pushcclosure_Decompiled_Args: int a1, int a2, int a3, int a4, int a5
	* lua_pushcclosure_TypeDef: int
	* lua_pushcclosure_Aob: 55 8B EC 56 8B 75 8 8B
	* lua_pushcclosure_ProtectedByRetcheck: true
	* lua_pushcclosure_IsApartOfLua: true
	* lua_pushcclosure_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_pushcclosure_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_lua_pushcclosure_typedef r_lua_pushcclosure = (r_lua_pushcclosure_typedef)unprotect(x(0x01382550));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushcclosure_CCV* r_lua_pushcclosure_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_lua_pushcclosure_typedef r_lua_pushcclosure = (r_lua_pushcclosure_typedef)unprotect(x(Adresses::lua_pushcclosure_addr));



	*/
	std::uint32_t lua_pushcclosure_addr = 0x01382550;
#define lua_pushcclosure_CCV __cdecl

	/*

	* lua_pushfstring_Decompiled_Args: int a1, int a2
	* lua_pushfstring_TypeDef: int
	* lua_pushfstring_Aob: 55 8B EC 56 8B 75 8 8B
	* lua_pushfstring_ProtectedByRetcheck: false
	* lua_pushfstring_IsApartOfLua: true
	* lua_pushfstring_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_pushfstring_typedef)(int a1, int a2);
	r_lua_pushfstring_typedef r_lua_pushfstring = (r_lua_pushfstring_typedef)(x(0x01382630));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushfstring_CCV* r_lua_pushfstring_typedef)(int a1, int a2);
	r_lua_pushfstring_typedef r_lua_pushfstring = (r_lua_pushfstring_typedef)(x(Adresses::lua_pushfstring_addr));



	*/
	std::uint32_t lua_pushfstring_addr = 0x01382630;
#define lua_pushfstring_CCV __cdecl

	/*

	* lua_pushinteger_Decompiled_Args: int a1, int a2
	* lua_pushinteger_TypeDef: int
	* lua_pushinteger_Aob: 55 8B EC 66 F 6E
	* lua_pushinteger_ProtectedByRetcheck: true
	* lua_pushinteger_IsApartOfLua: true
	* lua_pushinteger_Mask: ......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_pushinteger_typedef)(int a1, int a2);
	r_lua_pushinteger_typedef r_lua_pushinteger = (r_lua_pushinteger_typedef)unprotect(x(0x013826A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushinteger_CCV* r_lua_pushinteger_typedef)(int a1, int a2);
	r_lua_pushinteger_typedef r_lua_pushinteger = (r_lua_pushinteger_typedef)unprotect(x(Adresses::lua_pushinteger_addr));



	*/
	std::uint32_t lua_pushinteger_addr = 0x013826A0;
#define lua_pushinteger_CCV __thiscall

	/*

	* lua_pushlightuserdata_Decompiled_Args: int a1, int a2
	* lua_pushlightuserdata_TypeDef: int
	* lua_pushlightuserdata_Aob: 55 8B EC 8B 55 8 8B
	* lua_pushlightuserdata_ProtectedByRetcheck: true
	* lua_pushlightuserdata_IsApartOfLua: true
	* lua_pushlightuserdata_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_pushlightuserdata_typedef)(int a1, int a2);
	r_lua_pushlightuserdata_typedef r_lua_pushlightuserdata = (r_lua_pushlightuserdata_typedef)unprotect(x(0x01382700));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushlightuserdata_CCV* r_lua_pushlightuserdata_typedef)(int a1, int a2);
	r_lua_pushlightuserdata_typedef r_lua_pushlightuserdata = (r_lua_pushlightuserdata_typedef)unprotect(x(Adresses::lua_pushlightuserdata_addr));



	*/
	std::uint32_t lua_pushlightuserdata_addr = 0x01382700;
#define lua_pushlightuserdata_CCV __cdecl

	/*

	* lua_pushlstring_Decompiled_Args: int a1, int a2
	* lua_pushlstring_TypeDef: int
	* lua_pushlstring_Aob: 55 8B EC 57 8B 7D 8 8B
	* lua_pushlstring_ProtectedByRetcheck: true
	* lua_pushlstring_IsApartOfLua: true
	* lua_pushlstring_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_pushlstring_typedef)(int a1, int a2);
	r_lua_pushlstring_typedef r_lua_pushlstring = (r_lua_pushlstring_typedef)unprotect(x(0x01382750));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushlstring_CCV* r_lua_pushlstring_typedef)(int a1, int a2);
	r_lua_pushlstring_typedef r_lua_pushlstring = (r_lua_pushlstring_typedef)unprotect(x(Adresses::lua_pushlstring_addr));



	*/
	std::uint32_t lua_pushlstring_addr = 0x01382750;
#define lua_pushlstring_CCV __thiscall

	/*

	* lua_pushnil_Decompiled_Args: int a1
	* lua_pushnil_TypeDef: int
	* lua_pushnil_Aob: 55 8B EC 8B 55 8 8B
	* lua_pushnil_ProtectedByRetcheck: true
	* lua_pushnil_IsApartOfLua: true
	* lua_pushnil_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_pushnil_typedef)(int a1);
	r_lua_pushnil_typedef r_lua_pushnil = (r_lua_pushnil_typedef)unprotect(x(0x013827D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushnil_CCV* r_lua_pushnil_typedef)(int a1);
	r_lua_pushnil_typedef r_lua_pushnil = (r_lua_pushnil_typedef)unprotect(x(Adresses::lua_pushnil_addr));



	*/
	std::uint32_t lua_pushnil_addr = 0x013827D0;
#define lua_pushnil_CCV __cdecl

	/*

	* lua_pushnumber_Decompiled_Args: int a1
	* lua_pushnumber_TypeDef: int
	* lua_pushnumber_Aob: 55 8B EC F 28
	* lua_pushnumber_ProtectedByRetcheck: true
	* lua_pushnumber_IsApartOfLua: true
	* lua_pushnumber_Mask: .....
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_lua_pushnumber_typedef)(int a1);
	r_lua_pushnumber_typedef r_lua_pushnumber = (r_lua_pushnumber_typedef)unprotect(x(0x01382820));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushnumber_CCV* r_lua_pushnumber_typedef)(int a1);
	r_lua_pushnumber_typedef r_lua_pushnumber = (r_lua_pushnumber_typedef)unprotect(x(Adresses::lua_pushnumber_addr));



	*/
	std::uint32_t lua_pushnumber_addr = 0x01382820;
#define lua_pushnumber_CCV __fastcall

	/*

	* lua_pushstring_Decompiled_Args: int a1, const char* a2
	* lua_pushstring_TypeDef: int
	* lua_pushstring_Aob: 55 8B EC 51 8B C2 89 45 FC 53 8B D9 85 C0 75 8 8B
	* lua_pushstring_ProtectedByRetcheck: true
	* lua_pushstring_IsApartOfLua: true
	* lua_pushstring_Mask: .................
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_lua_pushstring_typedef)(int a1, const char* a2);
	r_lua_pushstring_typedef r_lua_pushstring = (r_lua_pushstring_typedef)unprotect(x(0x01382880));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushstring_CCV* r_lua_pushstring_typedef)(int a1, const char* a2);
	r_lua_pushstring_typedef r_lua_pushstring = (r_lua_pushstring_typedef)unprotect(x(Adresses::lua_pushstring_addr));



	*/
	std::uint32_t lua_pushstring_addr = 0x01382880;
#define lua_pushstring_CCV __fastcall

	/*

	* lua_pushthread_Decompiled_Args: int a1
	* lua_pushthread_TypeDef: int
	* lua_pushthread_Aob: 55 8B EC 56 8B 75 8 8B
	* lua_pushthread_ProtectedByRetcheck: true
	* lua_pushthread_IsApartOfLua: true
	* lua_pushthread_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_pushthread_typedef)(int a1);
	r_lua_pushthread_typedef r_lua_pushthread = (r_lua_pushthread_typedef)unprotect(x(0x01382920));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushthread_CCV* r_lua_pushthread_typedef)(int a1);
	r_lua_pushthread_typedef r_lua_pushthread = (r_lua_pushthread_typedef)unprotect(x(Adresses::lua_pushthread_addr));



	*/
	std::uint32_t lua_pushthread_addr = 0x01382920;
#define lua_pushthread_CCV __cdecl

	/*

	* lua_pushvalue_Decompiled_Args: int a1, int a2
	* lua_pushvalue_TypeDef: int
	* lua_pushvalue_Aob: 55 8B EC 8B 55 C 56
	* lua_pushvalue_ProtectedByRetcheck: true
	* lua_pushvalue_IsApartOfLua: true
	* lua_pushvalue_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_pushvalue_typedef)(int a1, int a2);
	r_lua_pushvalue_typedef r_lua_pushvalue = (r_lua_pushvalue_typedef)unprotect(x(0x013829F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushvalue_CCV* r_lua_pushvalue_typedef)(int a1, int a2);
	r_lua_pushvalue_typedef r_lua_pushvalue = (r_lua_pushvalue_typedef)unprotect(x(Adresses::lua_pushvalue_addr));



	*/
	std::uint32_t lua_pushvalue_addr = 0x013829F0;
#define lua_pushvalue_CCV __cdecl

	/*

	* lua_pushvfstring_Decompiled_Args: int a1
	* lua_pushvfstring_TypeDef: int
	* lua_pushvfstring_Aob: 55 8B EC 56 8B 75 8 8B
	* lua_pushvfstring_ProtectedByRetcheck: false
	* lua_pushvfstring_IsApartOfLua: true
	* lua_pushvfstring_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_lua_pushvfstring_typedef)(int a1);
	r_lua_pushvfstring_typedef r_lua_pushvfstring = (r_lua_pushvfstring_typedef)(x(0x01382A60));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_pushvfstring_CCV* r_lua_pushvfstring_typedef)(int a1);
	r_lua_pushvfstring_typedef r_lua_pushvfstring = (r_lua_pushvfstring_typedef)(x(Adresses::lua_pushvfstring_addr));



	*/
	std::uint32_t lua_pushvfstring_addr = 0x01382A60;
#define lua_pushvfstring_CCV __fastcall

	/*

	* lua_rawequal_Decompiled_Args: int a1, int a2, int a3
	* lua_rawequal_TypeDef: int
	* lua_rawequal_Aob: 55 8B EC 8B 55 C 53
	* lua_rawequal_ProtectedByRetcheck: false
	* lua_rawequal_IsApartOfLua: true
	* lua_rawequal_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_rawequal_typedef)(int a1, int a2, int a3);
	r_lua_rawequal_typedef r_lua_rawequal = (r_lua_rawequal_typedef)(x(0x01382B40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_rawequal_CCV* r_lua_rawequal_typedef)(int a1, int a2, int a3);
	r_lua_rawequal_typedef r_lua_rawequal = (r_lua_rawequal_typedef)(x(Adresses::lua_rawequal_addr));



	*/
	std::uint32_t lua_rawequal_addr = 0x01382B40;
#define lua_rawequal_CCV __cdecl

	/*

	* lua_rawget_Decompiled_Args: int a1, int a2
	* lua_rawget_TypeDef: int
	* lua_rawget_Aob: 55 8B EC 8B 55 C 56
	* lua_rawget_ProtectedByRetcheck: true
	* lua_rawget_IsApartOfLua: true
	* lua_rawget_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_rawget_typedef)(int a1, int a2);
	r_lua_rawget_typedef r_lua_rawget = (r_lua_rawget_typedef)unprotect(x(0x01382BC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_rawget_CCV* r_lua_rawget_typedef)(int a1, int a2);
	r_lua_rawget_typedef r_lua_rawget = (r_lua_rawget_typedef)unprotect(x(Adresses::lua_rawget_addr));



	*/
	std::uint32_t lua_rawget_addr = 0x01382BC0;
#define lua_rawget_CCV __cdecl

	/*

	* lua_rawgeti_Decompiled_Args: int a1, int a2
	* lua_rawgeti_TypeDef: int
	* lua_rawgeti_Aob: 55 8B EC 8B 55 C 56
	* lua_rawgeti_ProtectedByRetcheck: true
	* lua_rawgeti_IsApartOfLua: true
	* lua_rawgeti_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_rawgeti_typedef)(int a1, int a2);
	r_lua_rawgeti_typedef r_lua_rawgeti = (r_lua_rawgeti_typedef)unprotect(x(0x01382CE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_rawgeti_CCV* r_lua_rawgeti_typedef)(int a1, int a2);
	r_lua_rawgeti_typedef r_lua_rawgeti = (r_lua_rawgeti_typedef)unprotect(x(Adresses::lua_rawgeti_addr));



	*/
	std::uint32_t lua_rawgeti_addr = 0x01382CE0;
#define lua_rawgeti_CCV __cdecl

	/*

	* lua_rawset_Decompiled_Args: int a1, int a2
	* lua_rawset_TypeDef: int
	* lua_rawset_Aob: 55 8B EC 8B 55 C 53
	* lua_rawset_ProtectedByRetcheck: true
	* lua_rawset_IsApartOfLua: true
	* lua_rawset_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_rawset_typedef)(int a1, int a2);
	r_lua_rawset_typedef r_lua_rawset = (r_lua_rawset_typedef)unprotect(x(0x01382D60));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_rawset_CCV* r_lua_rawset_typedef)(int a1, int a2);
	r_lua_rawset_typedef r_lua_rawset = (r_lua_rawset_typedef)unprotect(x(Adresses::lua_rawset_addr));



	*/
	std::uint32_t lua_rawset_addr = 0x01382D60;
#define lua_rawset_CCV __cdecl

	/*

	* lua_rawseti_Decompiled_Args: int a1, int a2
	* lua_rawseti_TypeDef: int
	* lua_rawseti_Aob: 55 8B EC 8B 55 C 53
	* lua_rawseti_ProtectedByRetcheck: true
	* lua_rawseti_IsApartOfLua: true
	* lua_rawseti_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_rawseti_typedef)(int a1, int a2);
	r_lua_rawseti_typedef r_lua_rawseti = (r_lua_rawseti_typedef)unprotect(x(0x01382E30));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_rawseti_CCV* r_lua_rawseti_typedef)(int a1, int a2);
	r_lua_rawseti_typedef r_lua_rawseti = (r_lua_rawseti_typedef)unprotect(x(Adresses::lua_rawseti_addr));



	*/
	std::uint32_t lua_rawseti_addr = 0x01382E30;
#define lua_rawseti_CCV __cdecl

	/*

	* lua_rawvalue_Decompiled_Args: int a1, int a2
	* lua_rawvalue_TypeDef: int
	* lua_rawvalue_Aob: 55 8B EC 8B 55 C 8B
	* lua_rawvalue_ProtectedByRetcheck: false
	* lua_rawvalue_IsApartOfLua: true
	* lua_rawvalue_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_rawvalue_typedef)(int a1, int a2);
	r_lua_rawvalue_typedef r_lua_rawvalue = (r_lua_rawvalue_typedef)(x(0x013817B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_rawvalue_CCV* r_lua_rawvalue_typedef)(int a1, int a2);
	r_lua_rawvalue_typedef r_lua_rawvalue = (r_lua_rawvalue_typedef)(x(Adresses::lua_rawvalue_addr));



	*/
	std::uint32_t lua_rawvalue_addr = 0x013817B0;
#define lua_rawvalue_CCV __cdecl

	/*

	* lua_remove_Decompiled_Args: int a1, int a2
	* lua_remove_TypeDef: int
	* lua_remove_Aob: 55 8B EC 8B 55 C 56
	* lua_remove_ProtectedByRetcheck: true
	* lua_remove_IsApartOfLua: true
	* lua_remove_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_remove_typedef)(int a1, int a2);
	r_lua_remove_typedef r_lua_remove = (r_lua_remove_typedef)unprotect(x(0x01382F00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_remove_CCV* r_lua_remove_typedef)(int a1, int a2);
	r_lua_remove_typedef r_lua_remove = (r_lua_remove_typedef)unprotect(x(Adresses::lua_remove_addr));



	*/
	std::uint32_t lua_remove_addr = 0x01382F00;
#define lua_remove_CCV __cdecl

	/*

	* lua_replace_Decompiled_Args: int a1, int a2
	* lua_replace_TypeDef: int
	* lua_replace_Aob: 55 8B EC 56 8B 75 8 57
	* lua_replace_ProtectedByRetcheck: true
	* lua_replace_IsApartOfLua: true
	* lua_replace_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_replace_typedef)(int a1, int a2);
	r_lua_replace_typedef r_lua_replace = (r_lua_replace_typedef)unprotect(x(0x01382F90));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_replace_CCV* r_lua_replace_typedef)(int a1, int a2);
	r_lua_replace_typedef r_lua_replace = (r_lua_replace_typedef)unprotect(x(Adresses::lua_replace_addr));



	*/
	std::uint32_t lua_replace_addr = 0x01382F90;
#define lua_replace_CCV __cdecl

	/*

	* lua_resume_Decompiled_Args: int a1, int a2, int a3
	* lua_resume_TypeDef: int
	* lua_resume_Aob: 55 8B EC 56 8B 75 8 8A
	* lua_resume_ProtectedByRetcheck: true
	* lua_resume_IsApartOfLua: true
	* lua_resume_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_resume_typedef)(int a1, int a2, int a3);
	r_lua_resume_typedef r_lua_resume = (r_lua_resume_typedef)unprotect(x(0x01384E60));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_resume_CCV* r_lua_resume_typedef)(int a1, int a2, int a3);
	r_lua_resume_typedef r_lua_resume = (r_lua_resume_typedef)unprotect(x(Adresses::lua_resume_addr));



	*/
	std::uint32_t lua_resume_addr = 0x01384E60;
#define lua_resume_CCV __cdecl

	/*

	* lua_setfenv_Decompiled_Args: int a1, int a2
	* lua_setfenv_TypeDef: int
	* lua_setfenv_Aob: 55 8B EC 8B 55 C 53
	* lua_setfenv_ProtectedByRetcheck: false
	* lua_setfenv_IsApartOfLua: true
	* lua_setfenv_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_setfenv_typedef)(int a1, int a2);
	r_lua_setfenv_typedef r_lua_setfenv = (r_lua_setfenv_typedef)(x(0x01383090));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_setfenv_CCV* r_lua_setfenv_typedef)(int a1, int a2);
	r_lua_setfenv_typedef r_lua_setfenv = (r_lua_setfenv_typedef)(x(Adresses::lua_setfenv_addr));



	*/
	std::uint32_t lua_setfenv_addr = 0x01383090;
#define lua_setfenv_CCV __cdecl

	/*

	* lua_setfield_Decompiled_Args: int a1, int a2, const char* a3
	* lua_setfield_TypeDef: int
	* lua_setfield_Aob: 55 8B EC 8B 55 C 83
	* lua_setfield_ProtectedByRetcheck: true
	* lua_setfield_IsApartOfLua: true
	* lua_setfield_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_setfield_typedef)(int a1, int a2, const char* a3);
	r_lua_setfield_typedef r_lua_setfield = (r_lua_setfield_typedef)unprotect(x(0x01383160));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_setfield_CCV* r_lua_setfield_typedef)(int a1, int a2, const char* a3);
	r_lua_setfield_typedef r_lua_setfield = (r_lua_setfield_typedef)unprotect(x(Adresses::lua_setfield_addr));



	*/
	std::uint32_t lua_setfield_addr = 0x01383160;
#define lua_setfield_CCV __cdecl

	/*

	* lua_setlocal_Decompiled_Args: int a1, int a2
	* lua_setlocal_TypeDef: int
	* lua_setlocal_Aob: 55 8B EC 8B 45 C 53
	* lua_setlocal_ProtectedByRetcheck: true
	* lua_setlocal_IsApartOfLua: true
	* lua_setlocal_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_setlocal_typedef)(int a1, int a2);
	r_lua_setlocal_typedef r_lua_setlocal = (r_lua_setlocal_typedef)unprotect(x(0x0138F0C0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_setlocal_CCV* r_lua_setlocal_typedef)(int a1, int a2);
	r_lua_setlocal_typedef r_lua_setlocal = (r_lua_setlocal_typedef)unprotect(x(Adresses::lua_setlocal_addr));



	*/
	std::uint32_t lua_setlocal_addr = 0x0138F0C0;
#define lua_setlocal_CCV __thiscall

	/*

	* lua_setmetatable_Decompiled_Args: int a1, int a2
	* lua_setmetatable_TypeDef: int
	* lua_setmetatable_Aob: 55 8B EC 8B 55 C 56
	* lua_setmetatable_ProtectedByRetcheck: true
	* lua_setmetatable_IsApartOfLua: true
	* lua_setmetatable_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_setmetatable_typedef)(int a1, int a2);
	r_lua_setmetatable_typedef r_lua_setmetatable = (r_lua_setmetatable_typedef)unprotect(x(0x01383210));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_setmetatable_CCV* r_lua_setmetatable_typedef)(int a1, int a2);
	r_lua_setmetatable_typedef r_lua_setmetatable = (r_lua_setmetatable_typedef)unprotect(x(Adresses::lua_setmetatable_addr));



	*/
	std::uint32_t lua_setmetatable_addr = 0x01383210;
#define lua_setmetatable_CCV __cdecl

	/*

	* lua_setreadonly_Decompiled_Args: int a1, int a2, int a3
	* lua_setreadonly_TypeDef: int
	* lua_setreadonly_Aob: 55 8B EC 8B 55 C 56
	* lua_setreadonly_ProtectedByRetcheck: true
	* lua_setreadonly_IsApartOfLua: true
	* lua_setreadonly_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_setreadonly_typedef)(int a1, int a2, int a3);
	r_lua_setreadonly_typedef r_lua_setreadonly = (r_lua_setreadonly_typedef)unprotect(x(0x01383320));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_setreadonly_CCV* r_lua_setreadonly_typedef)(int a1, int a2, int a3);
	r_lua_setreadonly_typedef r_lua_setreadonly = (r_lua_setreadonly_typedef)unprotect(x(Adresses::lua_setreadonly_addr));



	*/
	std::uint32_t lua_setreadonly_addr = 0x01383320;
#define lua_setreadonly_CCV __thiscall

	/*

	* lua_setsafeenv_Decompiled_Args: int a1, int a2, int a3
	* lua_setsafeenv_TypeDef: int
	* lua_setsafeenv_Aob: 55 8B EC 8B 55 C 56
	* lua_setsafeenv_ProtectedByRetcheck: true
	* lua_setsafeenv_IsApartOfLua: true
	* lua_setsafeenv_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_setsafeenv_typedef)(int a1, int a2, int a3);
	r_lua_setsafeenv_typedef r_lua_setsafeenv = (r_lua_setsafeenv_typedef)unprotect(x(0x01383390));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_setsafeenv_CCV* r_lua_setsafeenv_typedef)(int a1, int a2, int a3);
	r_lua_setsafeenv_typedef r_lua_setsafeenv = (r_lua_setsafeenv_typedef)unprotect(x(Adresses::lua_setsafeenv_addr));



	*/
	std::uint32_t lua_setsafeenv_addr = 0x01383390;
#define lua_setsafeenv_CCV __thiscall

	/*

	* lua_settable_Decompiled_Args: int a1, int a2
	* lua_settable_TypeDef: int
	* lua_settable_Aob: 55 8B EC 8B 55 C 56
	* lua_settable_ProtectedByRetcheck: true
	* lua_settable_IsApartOfLua: true
	* lua_settable_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_settable_typedef)(int a1, int a2);
	r_lua_settable_typedef r_lua_settable = (r_lua_settable_typedef)unprotect(x(0x01383400));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_settable_CCV* r_lua_settable_typedef)(int a1, int a2);
	r_lua_settable_typedef r_lua_settable = (r_lua_settable_typedef)unprotect(x(Adresses::lua_settable_addr));



	*/
	std::uint32_t lua_settable_addr = 0x01383400;
#define lua_settable_CCV __cdecl

	/*

	* lua_settop_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lua_settop_TypeDef: int
	* lua_settop_Aob: 55 8B EC 56 57 8B FA 8B F1 C1 E7 4 85
	* lua_settop_ProtectedByRetcheck: true
	* lua_settop_IsApartOfLua: true
	* lua_settop_Mask: .............
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_lua_settop_typedef)(std::uint32_t Rbx_LuaState);
	r_lua_settop_typedef r_lua_settop = (r_lua_settop_typedef)unprotect(x(0x01383480));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_settop_CCV* r_lua_settop_typedef)(std::uint32_t Rbx_LuaState);
	r_lua_settop_typedef r_lua_settop = (r_lua_settop_typedef)unprotect(x(Adresses::lua_settop_addr));



	*/
	std::uint32_t lua_settop_addr = 0x01383480;
#define lua_settop_CCV __fastcall

	/*

	* lua_setupvalue_Decompiled_Args: int a1, int a2
	* lua_setupvalue_TypeDef: int
	* lua_setupvalue_Aob: 55 8B EC 8B 55 C 53
	* lua_setupvalue_ProtectedByRetcheck: true
	* lua_setupvalue_IsApartOfLua: true
	* lua_setupvalue_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_lua_setupvalue_typedef)(int a1, int a2);
	r_lua_setupvalue_typedef r_lua_setupvalue = (r_lua_setupvalue_typedef)unprotect(x(0x01383500));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_setupvalue_CCV* r_lua_setupvalue_typedef)(int a1, int a2);
	r_lua_setupvalue_typedef r_lua_setupvalue = (r_lua_setupvalue_typedef)unprotect(x(Adresses::lua_setupvalue_addr));



	*/
	std::uint32_t lua_setupvalue_addr = 0x01383500;
#define lua_setupvalue_CCV __thiscall

	/*

	* lua_toboolean_Decompiled_Args: int a1, int a2
	* lua_toboolean_TypeDef: bool
	* lua_toboolean_Aob: 55 8B EC 8B 55 C 8B
	* lua_toboolean_ProtectedByRetcheck: false
	* lua_toboolean_IsApartOfLua: true
	* lua_toboolean_Mask: .......
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_lua_toboolean_typedef)(int a1, int a2);
	r_lua_toboolean_typedef r_lua_toboolean = (r_lua_toboolean_typedef)(x(0x013835B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(lua_toboolean_CCV* r_lua_toboolean_typedef)(int a1, int a2);
	r_lua_toboolean_typedef r_lua_toboolean = (r_lua_toboolean_typedef)(x(Adresses::lua_toboolean_addr));



	*/
	std::uint32_t lua_toboolean_addr = 0x013835B0;
#define lua_toboolean_CCV __cdecl

	/*

	* lua_tointeger_Decompiled_Args: int a1, int a2, int a3
	* lua_tointeger_TypeDef: int
	* lua_tointeger_Aob: 55 8B EC 8B 55 C 83
	* lua_tointeger_ProtectedByRetcheck: false
	* lua_tointeger_IsApartOfLua: true
	* lua_tointeger_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_tointeger_typedef)(int a1, int a2, int a3);
	r_lua_tointeger_typedef r_lua_tointeger = (r_lua_tointeger_typedef)(x(0x01383600));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_tointeger_CCV* r_lua_tointeger_typedef)(int a1, int a2, int a3);
	r_lua_tointeger_typedef r_lua_tointeger = (r_lua_tointeger_typedef)(x(Adresses::lua_tointeger_addr));



	*/
	std::uint32_t lua_tointeger_addr = 0x01383600;
#define lua_tointeger_CCV __cdecl

	/*

	* lua_tolstring_Decompiled_Args: int a1, int a2, int a3
	* lua_tolstring_TypeDef: int
	* lua_tolstring_Aob: 55 8B EC 53 8B 5D C 56
	* lua_tolstring_ProtectedByRetcheck: true
	* lua_tolstring_IsApartOfLua: true
	* lua_tolstring_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_tolstring_typedef)(int a1, int a2, int a3);
	r_lua_tolstring_typedef r_lua_tolstring = (r_lua_tolstring_typedef)unprotect(x(0x01383680));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_tolstring_CCV* r_lua_tolstring_typedef)(int a1, int a2, int a3);
	r_lua_tolstring_typedef r_lua_tolstring = (r_lua_tolstring_typedef)unprotect(x(Adresses::lua_tolstring_addr));



	*/
	std::uint32_t lua_tolstring_addr = 0x01383680;
#define lua_tolstring_CCV __cdecl

	/*

	* lua_tonumber_Decompiled_Args: int a1, int a2, int a3
	* lua_tonumber_TypeDef: int
	* lua_tonumber_Aob: 55 8B EC 8B 55 C 83
	* lua_tonumber_ProtectedByRetcheck: false
	* lua_tonumber_IsApartOfLua: true
	* lua_tonumber_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_tonumber_typedef)(int a1, int a2, int a3);
	r_lua_tonumber_typedef r_lua_tonumber = (r_lua_tonumber_typedef)(x(0x013837B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_tonumber_CCV* r_lua_tonumber_typedef)(int a1, int a2, int a3);
	r_lua_tonumber_typedef r_lua_tonumber = (r_lua_tonumber_typedef)(x(Adresses::lua_tonumber_addr));



	*/
	std::uint32_t lua_tonumber_addr = 0x013837B0;
#define lua_tonumber_CCV __cdecl

	/*

	* lua_topointer_Decompiled_Args: int a1, int a2
	* lua_topointer_TypeDef: int
	* lua_topointer_Aob: 55 8B EC 53 8B 5D 8 56
	* lua_topointer_ProtectedByRetcheck: false
	* lua_topointer_IsApartOfLua: true
	* lua_topointer_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_topointer_typedef)(int a1, int a2);
	r_lua_topointer_typedef r_lua_topointer = (r_lua_topointer_typedef)(x(0x01383830));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_topointer_CCV* r_lua_topointer_typedef)(int a1, int a2);
	r_lua_topointer_typedef r_lua_topointer = (r_lua_topointer_typedef)(x(Adresses::lua_topointer_addr));



	*/
	std::uint32_t lua_topointer_addr = 0x01383830;
#define lua_topointer_CCV __cdecl

	/*

	* lua_tostring_Decompiled_Args: int a1, int a2
	* lua_tostring_TypeDef: int
	* lua_tostring_Aob: 55 8B EC 8B 55 C 8B
	* lua_tostring_ProtectedByRetcheck: false
	* lua_tostring_IsApartOfLua: true
	* lua_tostring_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_tostring_typedef)(int a1, int a2);
	r_lua_tostring_typedef r_lua_tostring = (r_lua_tostring_typedef)(x(0x013838F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_tostring_CCV* r_lua_tostring_typedef)(int a1, int a2);
	r_lua_tostring_typedef r_lua_tostring = (r_lua_tostring_typedef)(x(Adresses::lua_tostring_addr));



	*/
	std::uint32_t lua_tostring_addr = 0x013838F0;
#define lua_tostring_CCV __cdecl

	/*

	* lua_tothread_Decompiled_Args: int a1, int a2
	* lua_tothread_TypeDef: int
	* lua_tothread_Aob: 55 8B EC 8B 55 C 8B
	* lua_tothread_ProtectedByRetcheck: false
	* lua_tothread_IsApartOfLua: true
	* lua_tothread_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_tothread_typedef)(int a1, int a2);
	r_lua_tothread_typedef r_lua_tothread = (r_lua_tothread_typedef)(x(0x01383970));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_tothread_CCV* r_lua_tothread_typedef)(int a1, int a2);
	r_lua_tothread_typedef r_lua_tothread = (r_lua_tothread_typedef)(x(Adresses::lua_tothread_addr));



	*/
	std::uint32_t lua_tothread_addr = 0x01383970;
#define lua_tothread_CCV __cdecl

	/*

	* lua_tounsignedx_Decompiled_Args: int a1, int a2, int a3
	* lua_tounsignedx_TypeDef: int
	* lua_tounsignedx_Aob: 55 8B EC 8B 55 C 83
	* lua_tounsignedx_ProtectedByRetcheck: false
	* lua_tounsignedx_IsApartOfLua: true
	* lua_tounsignedx_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_tounsignedx_typedef)(int a1, int a2, int a3);
	r_lua_tounsignedx_typedef r_lua_tounsignedx = (r_lua_tounsignedx_typedef)(x(0x013839B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_tounsignedx_CCV* r_lua_tounsignedx_typedef)(int a1, int a2, int a3);
	r_lua_tounsignedx_typedef r_lua_tounsignedx = (r_lua_tounsignedx_typedef)(x(Adresses::lua_tounsignedx_addr));



	*/
	std::uint32_t lua_tounsignedx_addr = 0x013839B0;
#define lua_tounsignedx_CCV __cdecl

	/*

	* lua_touserdata_Decompiled_Args: int a1, int a2
	* lua_touserdata_TypeDef: int
	* lua_touserdata_Aob: 55 8B EC 8B 55 C 8B
	* lua_touserdata_ProtectedByRetcheck: false
	* lua_touserdata_IsApartOfLua: true
	* lua_touserdata_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_touserdata_typedef)(int a1, int a2);
	r_lua_touserdata_typedef r_lua_touserdata = (r_lua_touserdata_typedef)(x(0x01383A40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_touserdata_CCV* r_lua_touserdata_typedef)(int a1, int a2);
	r_lua_touserdata_typedef r_lua_touserdata = (r_lua_touserdata_typedef)(x(Adresses::lua_touserdata_addr));



	*/
	std::uint32_t lua_touserdata_addr = 0x01383A40;
#define lua_touserdata_CCV __cdecl

	/*

	* lua_type_Decompiled_Args: int a1, int a2
	* lua_type_TypeDef: int
	* lua_type_Aob: 55 8B EC 8B 55 C 8B
	* lua_type_ProtectedByRetcheck: false
	* lua_type_IsApartOfLua: true
	* lua_type_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_type_typedef)(int a1, int a2);
	r_lua_type_typedef r_lua_type = (r_lua_type_typedef)(x(0x01383B30));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_type_CCV* r_lua_type_typedef)(int a1, int a2);
	r_lua_type_typedef r_lua_type = (r_lua_type_typedef)(x(Adresses::lua_type_addr));



	*/
	std::uint32_t lua_type_addr = 0x01383B30;
#define lua_type_CCV __cdecl

	/*

	* lua_typename_Decompiled_Args: int a1
	* lua_typename_TypeDef: int
	* lua_typename_Aob: 55 8B EC 8B 45 C 83
	* lua_typename_ProtectedByRetcheck: false
	* lua_typename_IsApartOfLua: true
	* lua_typename_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_typename_typedef)(int a1);
	r_lua_typename_typedef r_lua_typename = (r_lua_typename_typedef)(x(0x01383B70));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_typename_CCV* r_lua_typename_typedef)(int a1);
	r_lua_typename_typedef r_lua_typename = (r_lua_typename_typedef)(x(Adresses::lua_typename_addr));



	*/
	std::uint32_t lua_typename_addr = 0x01383B70;
#define lua_typename_CCV __cdecl

	/*

	* lua_xmove_Decompiled_Args: int a1, int a2, int a3
	* lua_xmove_TypeDef: int
	* lua_xmove_Aob: 55 8B EC 56 8B 75 C 57
	* lua_xmove_ProtectedByRetcheck: true
	* lua_xmove_IsApartOfLua: true
	* lua_xmove_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lua_xmove_typedef)(int a1, int a2, int a3);
	r_lua_xmove_typedef r_lua_xmove = (r_lua_xmove_typedef)unprotect(x(0x01383BD0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lua_xmove_CCV* r_lua_xmove_typedef)(int a1, int a2, int a3);
	r_lua_xmove_typedef r_lua_xmove = (r_lua_xmove_typedef)unprotect(x(Adresses::lua_xmove_addr));



	*/
	std::uint32_t lua_xmove_addr = 0x01383BD0;
#define lua_xmove_CCV __cdecl

	/*

	* lua_yeild_Decompiled_Args: int a1, int a2
	* lua_yeild_TypeDef: bool
	* lua_yeild_Aob: 55 8B EC 8B 55 8 66
	* lua_yeild_ProtectedByRetcheck: true
	* lua_yeild_IsApartOfLua: true
	* lua_yeild_Mask: .......
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_lua_yeild_typedef)(int a1, int a2);
	r_lua_yeild_typedef r_lua_yeild = (r_lua_yeild_typedef)unprotect(x(0x013850C0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(lua_yeild_CCV* r_lua_yeild_typedef)(int a1, int a2);
	r_lua_yeild_typedef r_lua_yeild = (r_lua_yeild_typedef)unprotect(x(Adresses::lua_yeild_addr));



	*/
	std::uint32_t lua_yeild_addr = 0x013850C0;
#define lua_yeild_CCV __cdecl

	/*

	* luaa_pushobject_Decompiled_Args: int a1, int a2
	* luaa_pushobject_TypeDef: int
	* luaa_pushobject_Aob: 55 8B EC 8B 55 8 8B
	* luaa_pushobject_ProtectedByRetcheck: false
	* luaa_pushobject_IsApartOfLua: true
	* luaa_pushobject_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaa_pushobject_typedef)(int a1, int a2);
	r_luaa_pushobject_typedef r_luaa_pushobject = (r_luaa_pushobject_typedef)(x(0x01381790));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaa_pushobject_CCV* r_luaa_pushobject_typedef)(int a1, int a2);
	r_luaa_pushobject_typedef r_luaa_pushobject = (r_luaa_pushobject_typedef)(x(Adresses::luaa_pushobject_addr));



	*/
	std::uint32_t luaa_pushobject_addr = 0x01381790;
#define luaa_pushobject_CCV __cdecl

	/*

	* luab_auxwrap_Decompiled_Args: int a1
	* luab_auxwrap_TypeDef: int
	* luab_auxwrap_Aob: 55 8B EC 56 57 8B 7D 8 68
	* luab_auxwrap_ProtectedByRetcheck: false
	* luab_auxwrap_IsApartOfLua: true
	* luab_auxwrap_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luab_auxwrap_typedef)(int a1);
	r_luab_auxwrap_typedef r_luab_auxwrap = (r_luab_auxwrap_typedef)(x(0x01390470));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luab_auxwrap_CCV* r_luab_auxwrap_typedef)(int a1);
	r_luab_auxwrap_typedef r_luab_auxwrap = (r_luab_auxwrap_typedef)(x(Adresses::luab_auxwrap_addr));



	*/
	std::uint32_t luab_auxwrap_addr = 0x01390470;
#define luab_auxwrap_CCV __cdecl

	/*

	* luac_barrierback_Decompiled_Args: int a1, int a2
	* luac_barrierback_TypeDef: int
	* luac_barrierback_Aob: 55 8B EC 8B 45 8 8B
	* luac_barrierback_ProtectedByRetcheck: false
	* luac_barrierback_IsApartOfLua: true
	* luac_barrierback_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luac_barrierback_typedef)(int a1, int a2);
	r_luac_barrierback_typedef r_luac_barrierback = (r_luac_barrierback_typedef)(x(0x013966A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luac_barrierback_CCV* r_luac_barrierback_typedef)(int a1, int a2);
	r_luac_barrierback_typedef r_luac_barrierback = (r_luac_barrierback_typedef)(x(Adresses::luac_barrierback_addr));



	*/
	std::uint32_t luac_barrierback_addr = 0x013966A0;
#define luac_barrierback_CCV __cdecl

	/*

	* luac_barrierf_Decompiled_Args: int a1, int a2
	* luac_barrierf_TypeDef: int
	* luac_barrierf_Aob: 55 8B EC 8B 45 8 83
	* luac_barrierf_ProtectedByRetcheck: false
	* luac_barrierf_IsApartOfLua: true
	* luac_barrierf_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_luac_barrierf_typedef)(int a1, int a2);
	r_luac_barrierf_typedef r_luac_barrierf = (r_luac_barrierf_typedef)(x(0x013966C0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luac_barrierf_CCV* r_luac_barrierf_typedef)(int a1, int a2);
	r_luac_barrierf_typedef r_luac_barrierf = (r_luac_barrierf_typedef)(x(Adresses::luac_barrierf_addr));



	*/
	std::uint32_t luac_barrierf_addr = 0x013966C0;
#define luac_barrierf_CCV __thiscall

	/*

	* luac_fullgc_Decompiled_Args: int a1
	* luac_fullgc_TypeDef: int
	* luac_fullgc_Aob: 55 8B EC 53 56 57 8B 7D 8 8B
	* luac_fullgc_ProtectedByRetcheck: false
	* luac_fullgc_IsApartOfLua: true
	* luac_fullgc_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luac_fullgc_typedef)(int a1);
	r_luac_fullgc_typedef r_luac_fullgc = (r_luac_fullgc_typedef)(x(0x013967F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luac_fullgc_CCV* r_luac_fullgc_typedef)(int a1);
	r_luac_fullgc_typedef r_luac_fullgc = (r_luac_fullgc_typedef)(x(Adresses::luac_fullgc_addr));



	*/
	std::uint32_t luac_fullgc_addr = 0x013967F0;
#define luac_fullgc_CCV __cdecl

	/*

	* luac_link_Decompiled_Args: int a1, int a2, int a3
	* luac_link_TypeDef: int
	* luac_link_Aob: 55 8B EC 8B 45 8 8B
	* luac_link_ProtectedByRetcheck: false
	* luac_link_IsApartOfLua: true
	* luac_link_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_luac_link_typedef)(int a1, int a2, int a3);
	r_luac_link_typedef r_luac_link = (r_luac_link_typedef)(x(0x013968D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luac_link_CCV* r_luac_link_typedef)(int a1, int a2, int a3);
	r_luac_link_typedef r_luac_link = (r_luac_link_typedef)(x(Adresses::luac_link_addr));



	*/
	std::uint32_t luac_link_addr = 0x013968D0;
#define luac_link_CCV __thiscall

	/*

	* luac_linkupval_Decompiled_Args: int a1, int a2
	* luac_linkupval_TypeDef: bool
	* luac_linkupval_Aob: 55 8B EC 8B 55 C 56
	* luac_linkupval_ProtectedByRetcheck: false
	* luac_linkupval_IsApartOfLua: true
	* luac_linkupval_Mask: .......
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_luac_linkupval_typedef)(int a1, int a2);
	r_luac_linkupval_typedef r_luac_linkupval = (r_luac_linkupval_typedef)(x(0x01396900));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(luac_linkupval_CCV* r_luac_linkupval_typedef)(int a1, int a2);
	r_luac_linkupval_typedef r_luac_linkupval = (r_luac_linkupval_typedef)(x(Adresses::luac_linkupval_addr));



	*/
	std::uint32_t luac_linkupval_addr = 0x01396900;
#define luac_linkupval_CCV __cdecl

	/*

	* luac_step_Decompiled_Args: int a1
	* luac_step_TypeDef: int
	* luac_step_Aob: 55 8B EC 83 EC 14 53 56 8B 75 8 B8
	* luac_step_ProtectedByRetcheck: false
	* luac_step_IsApartOfLua: true
	* luac_step_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luac_step_typedef)(int a1);
	r_luac_step_typedef r_luac_step = (r_luac_step_typedef)(x(0x01396980));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luac_step_CCV* r_luac_step_typedef)(int a1);
	r_luac_step_typedef r_luac_step = (r_luac_step_typedef)(x(Adresses::luac_step_addr));



	*/
	std::uint32_t luac_step_addr = 0x01396980;
#define luac_step_CCV __cdecl

	/*

	* luad_call_Decompiled_Args: int a1
	* luad_call_TypeDef: int
	* luad_call_Aob: 55 8B EC 56 8B 75 8 BA
	* luad_call_ProtectedByRetcheck: false
	* luad_call_IsApartOfLua: true
	* luad_call_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_luad_call_typedef)(int a1);
	r_luad_call_typedef r_luad_call = (r_luad_call_typedef)(x(0x01384840));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_call_CCV* r_luad_call_typedef)(int a1);
	r_luad_call_typedef r_luad_call = (r_luad_call_typedef)(x(Adresses::luad_call_addr));



	*/
	std::uint32_t luad_call_addr = 0x01384840;
#define luad_call_CCV __fastcall

	/*

	* luad_growstack_Decompiled_Args: int a1, int a2
	* luad_growstack_TypeDef: int
	* luad_growstack_Aob: 55 8B EC 56 8B 75 C 57
	* luad_growstack_ProtectedByRetcheck: false
	* luad_growstack_IsApartOfLua: true
	* luad_growstack_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luad_growstack_typedef)(int a1, int a2);
	r_luad_growstack_typedef r_luad_growstack = (r_luad_growstack_typedef)(x(0x01384940));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_growstack_CCV* r_luad_growstack_typedef)(int a1, int a2);
	r_luad_growstack_typedef r_luad_growstack = (r_luad_growstack_typedef)(x(Adresses::luad_growstack_addr));



	*/
	std::uint32_t luad_growstack_addr = 0x01384940;
#define luad_growstack_CCV __cdecl

	/*

	* luad_pcall_Decompiled_Args: int a1, int a2, int a3
	* luad_pcall_TypeDef: int
	* luad_pcall_Aob: 55 8B EC 83 EC 8 53
	* luad_pcall_ProtectedByRetcheck: false
	* luad_pcall_IsApartOfLua: true
	* luad_pcall_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luad_pcall_typedef)(int a1, int a2, int a3);
	r_luad_pcall_typedef r_luad_pcall = (r_luad_pcall_typedef)(x(0x01384970));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_pcall_CCV* r_luad_pcall_typedef)(int a1, int a2, int a3);
	r_luad_pcall_typedef r_luad_pcall = (r_luad_pcall_typedef)(x(Adresses::luad_pcall_addr));



	*/
	std::uint32_t luad_pcall_addr = 0x01384970;
#define luad_pcall_CCV __cdecl

	/*

	* luad_poscall_Decompiled_Args: int a1, int a2
	* luad_poscall_TypeDef: int
	* luad_poscall_Aob: 55 8B EC 83 EC 8 53
	* luad_poscall_ProtectedByRetcheck: false
	* luad_poscall_IsApartOfLua: true
	* luad_poscall_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luad_poscall_typedef)(int a1, int a2);
	r_luad_poscall_typedef r_luad_poscall = (r_luad_poscall_typedef)(x(0x0138DD00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_poscall_CCV* r_luad_poscall_typedef)(int a1, int a2);
	r_luad_poscall_typedef r_luad_poscall = (r_luad_poscall_typedef)(x(Adresses::luad_poscall_addr));



	*/
	std::uint32_t luad_poscall_addr = 0x0138DD00;
#define luad_poscall_CCV __cdecl

	/*

	* luad_precall_Decompiled_Args: int a1, int a2, int a3
	* luad_precall_TypeDef: int
	* luad_precall_Aob: 55 8B EC 53 56 8B 75 C 57
	* luad_precall_ProtectedByRetcheck: false
	* luad_precall_IsApartOfLua: true
	* luad_precall_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luad_precall_typedef)(int a1, int a2, int a3);
	r_luad_precall_typedef r_luad_precall = (r_luad_precall_typedef)(x(0x0138DD80));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_precall_CCV* r_luad_precall_typedef)(int a1, int a2, int a3);
	r_luad_precall_typedef r_luad_precall = (r_luad_precall_typedef)(x(Adresses::luad_precall_addr));



	*/
	std::uint32_t luad_precall_addr = 0x0138DD80;
#define luad_precall_CCV __cdecl

	/*

	* luad_rawrunprotected_Decompiled_Args: int a1, int a2
	* luad_rawrunprotected_TypeDef: int
	* luad_rawrunprotected_Aob: 55 8B EC 6A FF 68 D0 2B 1D 2 64
	* luad_rawrunprotected_ProtectedByRetcheck: false
	* luad_rawrunprotected_IsApartOfLua: true
	* luad_rawrunprotected_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_luad_rawrunprotected_typedef)(int a1, int a2);
	r_luad_rawrunprotected_typedef r_luad_rawrunprotected = (r_luad_rawrunprotected_typedef)(x(0x01384AA0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_rawrunprotected_CCV* r_luad_rawrunprotected_typedef)(int a1, int a2);
	r_luad_rawrunprotected_typedef r_luad_rawrunprotected = (r_luad_rawrunprotected_typedef)(x(Adresses::luad_rawrunprotected_addr));



	*/
	std::uint32_t luad_rawrunprotected_addr = 0x01384AA0;
#define luad_rawrunprotected_CCV __thiscall

	/*

	* luad_reallocci_Decompiled_Args: int a1, int a2
	* luad_reallocci_TypeDef: int
	* luad_reallocci_Aob: 55 8B EC 8B 45 C 53
	* luad_reallocci_ProtectedByRetcheck: false
	* luad_reallocci_IsApartOfLua: true
	* luad_reallocci_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luad_reallocci_typedef)(int a1, int a2);
	r_luad_reallocci_typedef r_luad_reallocci = (r_luad_reallocci_typedef)(x(0x01384BE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_reallocci_CCV* r_luad_reallocci_typedef)(int a1, int a2);
	r_luad_reallocci_typedef r_luad_reallocci = (r_luad_reallocci_typedef)(x(Adresses::luad_reallocci_addr));



	*/
	std::uint32_t luad_reallocci_addr = 0x01384BE0;
#define luad_reallocci_CCV __cdecl

	/*

	* luad_reallocstack_Decompiled_Args: int a1, int a2
	* luad_reallocstack_TypeDef: int
	* luad_reallocstack_Aob: 55 8B EC 8B 45 C 53
	* luad_reallocstack_ProtectedByRetcheck: false
	* luad_reallocstack_IsApartOfLua: true
	* luad_reallocstack_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luad_reallocstack_typedef)(int a1, int a2);
	r_luad_reallocstack_typedef r_luad_reallocstack = (r_luad_reallocstack_typedef)(x(0x01384C60));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_reallocstack_CCV* r_luad_reallocstack_typedef)(int a1, int a2);
	r_luad_reallocstack_typedef r_luad_reallocstack = (r_luad_reallocstack_typedef)(x(Adresses::luad_reallocstack_addr));



	*/
	std::uint32_t luad_reallocstack_addr = 0x01384C60;
#define luad_reallocstack_CCV __cdecl

	/*

	* luad_seterrorObj_Decompiled_Args: int a1, int a2, int a3
	* luad_seterrorObj_TypeDef: int
	* luad_seterrorObj_Aob: 55 8B EC 8B 45 C 56
	* luad_seterrorObj_ProtectedByRetcheck: false
	* luad_seterrorObj_IsApartOfLua: true
	* luad_seterrorObj_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luad_seterrorObj_typedef)(int a1, int a2, int a3);
	r_luad_seterrorObj_typedef r_luad_seterrorObj = (r_luad_seterrorObj_typedef)(x(0x01384D40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_seterrorObj_CCV* r_luad_seterrorObj_typedef)(int a1, int a2, int a3);
	r_luad_seterrorObj_typedef r_luad_seterrorObj = (r_luad_seterrorObj_typedef)(x(Adresses::luad_seterrorObj_addr));



	*/
	std::uint32_t luad_seterrorObj_addr = 0x01384D40;
#define luad_seterrorObj_CCV __cdecl

	/*

	* luad_throw_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luad_throw_TypeDef: int
	* luad_throw_Aob: 55 8B EC 83 EC 14 8D 4D EC FF 75 C FF
	* luad_throw_ProtectedByRetcheck: false
	* luad_throw_IsApartOfLua: true
	* luad_throw_Mask: .............
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_luad_throw_typedef)(std::uint32_t Rbx_LuaState);
	r_luad_throw_typedef r_luad_throw = (r_luad_throw_typedef)(x(0x01384DB0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luad_throw_CCV* r_luad_throw_typedef)(std::uint32_t Rbx_LuaState);
	r_luad_throw_typedef r_luad_throw = (r_luad_throw_typedef)(x(Adresses::luad_throw_addr));



	*/
	std::uint32_t luad_throw_addr = 0x01384DB0;
#define luad_throw_CCV __fastcall

	/*

	* luae_freethread_Decompiled_Args: int a1, int a2
	* luae_freethread_TypeDef: int
	* luae_freethread_Aob: 55 8B EC 56 8B 75 C 57
	* luae_freethread_ProtectedByRetcheck: false
	* luae_freethread_IsApartOfLua: true
	* luae_freethread_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luae_freethread_typedef)(int a1, int a2);
	r_luae_freethread_typedef r_luae_freethread = (r_luae_freethread_typedef)(x(0x0138E310));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luae_freethread_CCV* r_luae_freethread_typedef)(int a1, int a2);
	r_luae_freethread_typedef r_luae_freethread = (r_luae_freethread_typedef)(x(Adresses::luae_freethread_addr));



	*/
	std::uint32_t luae_freethread_addr = 0x0138E310;
#define luae_freethread_CCV __cdecl

	/*

	* luae_newthread_Decompiled_Args: int a1
	* luae_newthread_TypeDef: int
	* luae_newthread_Aob: 55 8B EC 56 57 8B 7D 8 6A
	* luae_newthread_ProtectedByRetcheck: false
	* luae_newthread_IsApartOfLua: true
	* luae_newthread_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luae_newthread_typedef)(int a1);
	r_luae_newthread_typedef r_luae_newthread = (r_luae_newthread_typedef)(x(0x0138E380));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luae_newthread_CCV* r_luae_newthread_typedef)(int a1);
	r_luae_newthread_typedef r_luae_newthread = (r_luae_newthread_typedef)(x(Adresses::luae_newthread_addr));



	*/
	std::uint32_t luae_newthread_addr = 0x0138E380;
#define luae_newthread_CCV __cdecl

	/*

	* luaf_close_Decompiled_Args: int a1, int a2
	* luaf_close_TypeDef: int
	* luaf_close_Aob: 55 8B EC 53 57 8B 7D 8 8B
	* luaf_close_ProtectedByRetcheck: false
	* luaf_close_IsApartOfLua: true
	* luaf_close_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaf_close_typedef)(int a1, int a2);
	r_luaf_close_typedef r_luaf_close = (r_luaf_close_typedef)(x(0x013960A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaf_close_CCV* r_luaf_close_typedef)(int a1, int a2);
	r_luaf_close_typedef r_luaf_close = (r_luaf_close_typedef)(x(Adresses::luaf_close_addr));



	*/
	std::uint32_t luaf_close_addr = 0x013960A0;
#define luaf_close_CCV __cdecl

	/*

	* luaf_findupval_Decompiled_Args: int a1, int a2
	* luaf_findupval_TypeDef: bool
	* luaf_findupval_Aob: 55 8B EC 8B 55 8 53
	* luaf_findupval_ProtectedByRetcheck: false
	* luaf_findupval_IsApartOfLua: true
	* luaf_findupval_Mask: .......
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_luaf_findupval_typedef)(int a1, int a2);
	r_luaf_findupval_typedef r_luaf_findupval = (r_luaf_findupval_typedef)(x(0x01396130));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(luaf_findupval_CCV* r_luaf_findupval_typedef)(int a1, int a2);
	r_luaf_findupval_typedef r_luaf_findupval = (r_luaf_findupval_typedef)(x(Adresses::luaf_findupval_addr));



	*/
	std::uint32_t luaf_findupval_addr = 0x01396130;
#define luaf_findupval_CCV __cdecl

	/*

	* luaf_freeproto_Decompiled_Args: int a1, int a2
	* luaf_freeproto_TypeDef: int
	* luaf_freeproto_Aob: 55 8B EC 56 8B 75 C 57
	* luaf_freeproto_ProtectedByRetcheck: false
	* luaf_freeproto_IsApartOfLua: true
	* luaf_freeproto_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaf_freeproto_typedef)(int a1, int a2);
	r_luaf_freeproto_typedef r_luaf_freeproto = (r_luaf_freeproto_typedef)(x(0x01396200));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaf_freeproto_CCV* r_luaf_freeproto_typedef)(int a1, int a2);
	r_luaf_freeproto_typedef r_luaf_freeproto = (r_luaf_freeproto_typedef)(x(Adresses::luaf_freeproto_addr));



	*/
	std::uint32_t luaf_freeproto_addr = 0x01396200;
#define luaf_freeproto_CCV __cdecl

	/*

	* luaf_newcclosure_Decompiled_Args: int a1, int a2
	* luaf_newcclosure_TypeDef: int
	* luaf_newcclosure_Aob: 55 8B EC 53 8B 5D C 8B
	* luaf_newcclosure_ProtectedByRetcheck: false
	* luaf_newcclosure_IsApartOfLua: true
	* luaf_newcclosure_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaf_newcclosure_typedef)(int a1, int a2);
	r_luaf_newcclosure_typedef r_luaf_newcclosure = (r_luaf_newcclosure_typedef)(x(0x01396360));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaf_newcclosure_CCV* r_luaf_newcclosure_typedef)(int a1, int a2);
	r_luaf_newcclosure_typedef r_luaf_newcclosure = (r_luaf_newcclosure_typedef)(x(Adresses::luaf_newcclosure_addr));



	*/
	std::uint32_t luaf_newcclosure_addr = 0x01396360;
#define luaf_newcclosure_CCV __cdecl

	/*

	* luaf_newlclosure_Decompiled_Args: int a1, int a2, int a3
	* luaf_newlclosure_TypeDef: int
	* luaf_newlclosure_Aob: 55 8B EC 53 8B 5D C 57
	* luaf_newlclosure_ProtectedByRetcheck: false
	* luaf_newlclosure_IsApartOfLua: true
	* luaf_newlclosure_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaf_newlclosure_typedef)(int a1, int a2, int a3);
	r_luaf_newlclosure_typedef r_luaf_newlclosure = (r_luaf_newlclosure_typedef)(x(0x013963B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaf_newlclosure_CCV* r_luaf_newlclosure_typedef)(int a1, int a2, int a3);
	r_luaf_newlclosure_typedef r_luaf_newlclosure = (r_luaf_newlclosure_typedef)(x(Adresses::luaf_newlclosure_addr));



	*/
	std::uint32_t luaf_newlclosure_addr = 0x013963B0;
#define luaf_newlclosure_CCV __cdecl

	/*

	* luaf_newproto_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luaf_newproto_TypeDef: int
	* luaf_newproto_Aob: 55 8B EC 57 6A 54 FF 75 8 E8
	* luaf_newproto_ProtectedByRetcheck: false
	* luaf_newproto_IsApartOfLua: true
	* luaf_newproto_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaf_newproto_typedef)(std::uint32_t Rbx_LuaState);
	r_luaf_newproto_typedef r_luaf_newproto = (r_luaf_newproto_typedef)(x(0x01396420));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaf_newproto_CCV* r_luaf_newproto_typedef)(std::uint32_t Rbx_LuaState);
	r_luaf_newproto_typedef r_luaf_newproto = (r_luaf_newproto_typedef)(x(Adresses::luaf_newproto_addr));



	*/
	std::uint32_t luaf_newproto_addr = 0x01396420;
#define luaf_newproto_CCV __cdecl

	/*

	* luag_aritherror_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luag_aritherror_TypeDef: int
	* luag_aritherror_Aob: 55 8B EC 56 8B 75 8 57
	* luag_aritherror_ProtectedByRetcheck: false
	* luag_aritherror_IsApartOfLua: true
	* luag_aritherror_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_aritherror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_aritherror_typedef r_luag_aritherror = (r_luag_aritherror_typedef)(x(0x0138EA20));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_aritherror_CCV* r_luag_aritherror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_aritherror_typedef r_luag_aritherror = (r_luag_aritherror_typedef)(x(Adresses::luag_aritherror_addr));



	*/
	std::uint32_t luag_aritherror_addr = 0x0138EA20;
#define luag_aritherror_CCV __cdecl

	/*

	* luag_breakpoint_Decompiled_Args: int a1, int a2, int a3
	* luag_breakpoint_TypeDef: int
	* luag_breakpoint_Aob: 55 8B EC 53 8B 5D 10 56 57 8B 7D C 8B
	* luag_breakpoint_ProtectedByRetcheck: false
	* luag_breakpoint_IsApartOfLua: true
	* luag_breakpoint_Mask: .............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_breakpoint_typedef)(int a1, int a2, int a3);
	r_luag_breakpoint_typedef r_luag_breakpoint = (r_luag_breakpoint_typedef)(x(0x0138EA90));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_breakpoint_CCV* r_luag_breakpoint_typedef)(int a1, int a2, int a3);
	r_luag_breakpoint_typedef r_luag_breakpoint = (r_luag_breakpoint_typedef)(x(Adresses::luag_breakpoint_addr));



	*/
	std::uint32_t luag_breakpoint_addr = 0x0138EA90;
#define luag_breakpoint_CCV __cdecl

	/*

	* luag_concaterror_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luag_concaterror_TypeDef: int
	* luag_concaterror_Aob: 55 8B EC 51 56 57 FF 75 C 8B
	* luag_concaterror_ProtectedByRetcheck: false
	* luag_concaterror_IsApartOfLua: true
	* luag_concaterror_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_concaterror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_concaterror_typedef r_luag_concaterror = (r_luag_concaterror_typedef)(x(0x0138EB90));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_concaterror_CCV* r_luag_concaterror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_concaterror_typedef r_luag_concaterror = (r_luag_concaterror_typedef)(x(Adresses::luag_concaterror_addr));



	*/
	std::uint32_t luag_concaterror_addr = 0x0138EB90;
#define luag_concaterror_CCV __cdecl

	/*

	* luag_forerrorl_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luag_forerrorl_TypeDef: int
	* luag_forerrorl_Aob: 55 8B EC 51 FF 75 C FF
	* luag_forerrorl_ProtectedByRetcheck: false
	* luag_forerrorl_IsApartOfLua: true
	* luag_forerrorl_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_forerrorl_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_forerrorl_typedef r_luag_forerrorl = (r_luag_forerrorl_typedef)(x(0x0138EBD0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_forerrorl_CCV* r_luag_forerrorl_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_forerrorl_typedef r_luag_forerrorl = (r_luag_forerrorl_typedef)(x(Adresses::luag_forerrorl_addr));



	*/
	std::uint32_t luag_forerrorl_addr = 0x0138EBD0;
#define luag_forerrorl_CCV __cdecl

	/*

	* luag_getline_Decompiled_Args: int a1, int a2
	* luag_getline_TypeDef: int
	* luag_getline_Aob: 55 8B EC 53 57 8B 7D 8 8B
	* luag_getline_ProtectedByRetcheck: false
	* luag_getline_IsApartOfLua: true
	* luag_getline_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_getline_typedef)(int a1, int a2);
	r_luag_getline_typedef r_luag_getline = (r_luag_getline_typedef)(x(0x0138EC00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_getline_CCV* r_luag_getline_typedef)(int a1, int a2);
	r_luag_getline_typedef r_luag_getline = (r_luag_getline_typedef)(x(Adresses::luag_getline_addr));



	*/
	std::uint32_t luag_getline_addr = 0x0138EC00;
#define luag_getline_CCV __cdecl

	/*

	* luag_indexerror_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luag_indexerror_TypeDef: int
	* luag_indexerror_Aob: 55 8B EC 53 56 8B 75 8 57
	* luag_indexerror_ProtectedByRetcheck: false
	* luag_indexerror_IsApartOfLua: true
	* luag_indexerror_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_indexerror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_indexerror_typedef r_luag_indexerror = (r_luag_indexerror_typedef)(x(0x0138EC40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_indexerror_CCV* r_luag_indexerror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_indexerror_typedef r_luag_indexerror = (r_luag_indexerror_typedef)(x(Adresses::luag_indexerror_addr));



	*/
	std::uint32_t luag_indexerror_addr = 0x0138EC40;
#define luag_indexerror_CCV __cdecl

	/*

	* luag_ordererror_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luag_ordererror_TypeDef: int
	* luag_ordererror_Aob: 55 8B EC 51 56 8B 75 8 57
	* luag_ordererror_ProtectedByRetcheck: false
	* luag_ordererror_IsApartOfLua: true
	* luag_ordererror_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_ordererror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_ordererror_typedef r_luag_ordererror = (r_luag_ordererror_typedef)(x(0x0138ECF0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_ordererror_CCV* r_luag_ordererror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_ordererror_typedef r_luag_ordererror = (r_luag_ordererror_typedef)(x(Adresses::luag_ordererror_addr));



	*/
	std::uint32_t luag_ordererror_addr = 0x0138ECF0;
#define luag_ordererror_CCV __cdecl

	/*

	* luag_runerror_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luag_runerror_TypeDef: int
	* luag_runerror_Aob: 55 8B EC 56 8B 75 8 8D
	* luag_runerror_ProtectedByRetcheck: false
	* luag_runerror_IsApartOfLua: true
	* luag_runerror_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_runerror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_runerror_typedef r_luag_runerror = (r_luag_runerror_typedef)(x(0x0138ED80));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_runerror_CCV* r_luag_runerror_typedef)(std::uint32_t Rbx_LuaState);
	r_luag_runerror_typedef r_luag_runerror = (r_luag_runerror_typedef)(x(Adresses::luag_runerror_addr));



	*/
	std::uint32_t luag_runerror_addr = 0x0138ED80;
#define luag_runerror_CCV __cdecl

	/*

	* luag_typeerror_Decompiled_Args: int a1, int a2, int a3
	* luag_typeerror_TypeDef: int
	* luag_typeerror_Aob: 55 8B EC 8B 4D C 56
	* luag_typeerror_ProtectedByRetcheck: false
	* luag_typeerror_IsApartOfLua: true
	* luag_typeerror_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luag_typeerror_typedef)(int a1, int a2, int a3);
	r_luag_typeerror_typedef r_luag_typeerror = (r_luag_typeerror_typedef)(x(0x01399FF0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luag_typeerror_CCV* r_luag_typeerror_typedef)(int a1, int a2, int a3);
	r_luag_typeerror_typedef r_luag_typeerror = (r_luag_typeerror_typedef)(x(Adresses::luag_typeerror_addr));



	*/
	std::uint32_t luag_typeerror_addr = 0x01399FF0;
#define luag_typeerror_CCV __cdecl

	/*

	* luah_clone_Decompiled_Args: int a1, int a2
	* luah_clone_TypeDef: int
	* luah_clone_Aob: 55 8B EC 53 56 57 8B 7D 8 6A
	* luah_clone_ProtectedByRetcheck: false
	* luah_clone_IsApartOfLua: true
	* luah_clone_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_clone_typedef)(int a1, int a2);
	r_luah_clone_typedef r_luah_clone = (r_luah_clone_typedef)(x(0x01397EC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_clone_CCV* r_luah_clone_typedef)(int a1, int a2);
	r_luah_clone_typedef r_luah_clone = (r_luah_clone_typedef)(x(Adresses::luah_clone_addr));



	*/
	std::uint32_t luah_clone_addr = 0x01397EC0;
#define luah_clone_CCV __cdecl

	/*

	* luah_free_Decompiled_Args: int a1, int a2
	* luah_free_TypeDef: int
	* luah_free_Aob: 55 8B EC 56 8B 75 C 8B
	* luah_free_ProtectedByRetcheck: false
	* luah_free_IsApartOfLua: true
	* luah_free_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_free_typedef)(int a1, int a2);
	r_luah_free_typedef r_luah_free = (r_luah_free_typedef)(x(0x01397FE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_free_CCV* r_luah_free_typedef)(int a1, int a2);
	r_luah_free_typedef r_luah_free = (r_luah_free_typedef)(x(Adresses::luah_free_addr));



	*/
	std::uint32_t luah_free_addr = 0x01397FE0;
#define luah_free_CCV __cdecl

	/*

	* luah_get_Decompiled_Args: int a1
	* luah_get_TypeDef: int
	* luah_get_Aob: 55 8B EC 56 57 8B 7D C 8B
	* luah_get_ProtectedByRetcheck: false
	* luah_get_IsApartOfLua: true
	* luah_get_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_get_typedef)(int a1);
	r_luah_get_typedef r_luah_get = (r_luah_get_typedef)(x(0x01398040));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_get_CCV* r_luah_get_typedef)(int a1);
	r_luah_get_typedef r_luah_get = (r_luah_get_typedef)(x(Adresses::luah_get_addr));



	*/
	std::uint32_t luah_get_addr = 0x01398040;
#define luah_get_CCV __cdecl

	/*

	* luah_getn_Decompiled_Args: int a1
	* luah_getn_TypeDef: int
	* luah_getn_Aob: 55 8B EC 53 56 57 8B 7D 8 8B
	* luah_getn_ProtectedByRetcheck: false
	* luah_getn_IsApartOfLua: true
	* luah_getn_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_getn_typedef)(int a1);
	r_luah_getn_typedef r_luah_getn = (r_luah_getn_typedef)(x(0x013980F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_getn_CCV* r_luah_getn_typedef)(int a1);
	r_luah_getn_typedef r_luah_getn = (r_luah_getn_typedef)(x(Adresses::luah_getn_addr));



	*/
	std::uint32_t luah_getn_addr = 0x013980F0;
#define luah_getn_CCV __cdecl

	/*

	* luah_getnum_Decompiled_Args: int a1, int a2
	* luah_getnum_TypeDef: int
	* luah_getnum_Aob: 55 8B EC 8B 4D C 83
	* luah_getnum_ProtectedByRetcheck: false
	* luah_getnum_IsApartOfLua: true
	* luah_getnum_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_getnum_typedef)(int a1, int a2);
	r_luah_getnum_typedef r_luah_getnum = (r_luah_getnum_typedef)(x(0x01398170));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_getnum_CCV* r_luah_getnum_typedef)(int a1, int a2);
	r_luah_getnum_typedef r_luah_getnum = (r_luah_getnum_typedef)(x(Adresses::luah_getnum_addr));



	*/
	std::uint32_t luah_getnum_addr = 0x01398170;
#define luah_getnum_CCV __cdecl

	/*

	* luah_getstr_Decompiled_Args: int a1, int a2
	* luah_getstr_TypeDef: int
	* luah_getstr_Aob: 55 8B EC 8B 45 8 53
	* luah_getstr_ProtectedByRetcheck: false
	* luah_getstr_IsApartOfLua: true
	* luah_getstr_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_getstr_typedef)(int a1, int a2);
	r_luah_getstr_typedef r_luah_getstr = (r_luah_getstr_typedef)(x(0x01398250));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_getstr_CCV* r_luah_getstr_typedef)(int a1, int a2);
	r_luah_getstr_typedef r_luah_getstr = (r_luah_getstr_typedef)(x(Adresses::luah_getstr_addr));



	*/
	std::uint32_t luah_getstr_addr = 0x01398250;
#define luah_getstr_CCV __cdecl

	/*

	* luah_new_Decompiled_Args: int a1
	* luah_new_TypeDef: int
	* luah_new_Aob: 55 8B EC 56 57 8B 7D 8 6A
	* luah_new_ProtectedByRetcheck: false
	* luah_new_IsApartOfLua: true
	* luah_new_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_new_typedef)(int a1);
	r_luah_new_typedef r_luah_new = (r_luah_new_typedef)(x(0x013982C0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_new_CCV* r_luah_new_typedef)(int a1);
	r_luah_new_typedef r_luah_new = (r_luah_new_typedef)(x(Adresses::luah_new_addr));



	*/
	std::uint32_t luah_new_addr = 0x013982C0;
#define luah_new_CCV __cdecl

	/*

	* luah_next_Decompiled_Args: int a1, int a2
	* luah_next_TypeDef: int
	* luah_next_Aob: 55 8B EC 83 EC C 56
	* luah_next_ProtectedByRetcheck: false
	* luah_next_IsApartOfLua: true
	* luah_next_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_next_typedef)(int a1, int a2);
	r_luah_next_typedef r_luah_next = (r_luah_next_typedef)(x(0x01398330));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_next_CCV* r_luah_next_typedef)(int a1, int a2);
	r_luah_next_typedef r_luah_next = (r_luah_next_typedef)(x(Adresses::luah_next_addr));



	*/
	std::uint32_t luah_next_addr = 0x01398330;
#define luah_next_CCV __cdecl

	/*

	* luah_resizearray_Decompiled_Args: int a1
	* luah_resizearray_TypeDef: int
	* luah_resizearray_Aob: 55 8B EC 8B 55 C 8B
	* luah_resizearray_ProtectedByRetcheck: false
	* luah_resizearray_IsApartOfLua: true
	* luah_resizearray_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_resizearray_typedef)(int a1);
	r_luah_resizearray_typedef r_luah_resizearray = (r_luah_resizearray_typedef)(x(0x01398500));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_resizearray_CCV* r_luah_resizearray_typedef)(int a1);
	r_luah_resizearray_typedef r_luah_resizearray = (r_luah_resizearray_typedef)(x(Adresses::luah_resizearray_addr));



	*/
	std::uint32_t luah_resizearray_addr = 0x01398500;
#define luah_resizearray_CCV __cdecl

	/*

	* luah_set_Decompiled_Args: int a1, int a2
	* luah_set_TypeDef: int
	* luah_set_Aob: 55 8B EC 56 57 8B 7D 10 8B 47 C 83
	* luah_set_ProtectedByRetcheck: false
	* luah_set_IsApartOfLua: true
	* luah_set_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_set_typedef)(int a1, int a2);
	r_luah_set_typedef r_luah_set = (r_luah_set_typedef)(x(0x01398550));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_set_CCV* r_luah_set_typedef)(int a1, int a2);
	r_luah_set_typedef r_luah_set = (r_luah_set_typedef)(x(Adresses::luah_set_addr));



	*/
	std::uint32_t luah_set_addr = 0x01398550;
#define luah_set_CCV __cdecl

	/*

	* luah_setnum_Decompiled_Args: int a1, int a2
	* luah_setnum_TypeDef: int
	* luah_setnum_Aob: 55 8B EC 83 EC 10 56 8B 75 10 57 8B 7D C 8D
	* luah_setnum_ProtectedByRetcheck: false
	* luah_setnum_IsApartOfLua: true
	* luah_setnum_Mask: ...............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_setnum_typedef)(int a1, int a2);
	r_luah_setnum_typedef r_luah_setnum = (r_luah_setnum_typedef)(x(0x01398670));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_setnum_CCV* r_luah_setnum_typedef)(int a1, int a2);
	r_luah_setnum_typedef r_luah_setnum = (r_luah_setnum_typedef)(x(Adresses::luah_setnum_addr));



	*/
	std::uint32_t luah_setnum_addr = 0x01398670;
#define luah_setnum_CCV __cdecl

	/*

	* luah_setstr_Decompiled_Args: int a1
	* luah_setstr_TypeDef: int
	* luah_setstr_Aob: 55 8B EC 83 EC 10 56 8B 75 10 56 FF 75 C E8
	* luah_setstr_ProtectedByRetcheck: false
	* luah_setstr_IsApartOfLua: true
	* luah_setstr_Mask: ...............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luah_setstr_typedef)(int a1);
	r_luah_setstr_typedef r_luah_setstr = (r_luah_setstr_typedef)(x(0x013986E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luah_setstr_CCV* r_luah_setstr_typedef)(int a1);
	r_luah_setstr_typedef r_luah_setstr = (r_luah_setstr_typedef)(x(Adresses::luah_setstr_addr));



	*/
	std::uint32_t luah_setstr_addr = 0x013986E0;
#define luah_setstr_CCV __cdecl

	/*

	* lual_addlstring_Decompiled_Args: int a1, int a2, int a3
	* lual_addlstring_TypeDef: int
	* lual_addlstring_Aob: 55 8B EC 53 8B 5D 10 85 DB 74 5B 8B 55 C 56
	* lual_addlstring_ProtectedByRetcheck: false
	* lual_addlstring_IsApartOfLua: true
	* lual_addlstring_Mask: ...............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_addlstring_typedef)(int a1, int a2, int a3);
	r_lual_addlstring_typedef r_lual_addlstring = (r_lual_addlstring_typedef)(x(0x01383D40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_addlstring_CCV* r_lual_addlstring_typedef)(int a1, int a2, int a3);
	r_lual_addlstring_typedef r_lual_addlstring = (r_lual_addlstring_typedef)(x(Adresses::lual_addlstring_addr));



	*/
	std::uint32_t lual_addlstring_addr = 0x01383D40;
#define lual_addlstring_CCV __cdecl

	/*

	* lual_addvalue_Decompiled_Args: int a1
	* lual_addvalue_TypeDef: int
	* lual_addvalue_Aob: 55 8B EC 53 56 57 8B 7D 8 8D
	* lual_addvalue_ProtectedByRetcheck: false
	* lual_addvalue_IsApartOfLua: true
	* lual_addvalue_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_addvalue_typedef)(int a1);
	r_lual_addvalue_typedef r_lual_addvalue = (r_lual_addvalue_typedef)(x(0x01383DB0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_addvalue_CCV* r_lual_addvalue_typedef)(int a1);
	r_lual_addvalue_typedef r_lual_addvalue = (r_lual_addvalue_typedef)(x(Adresses::lual_addvalue_addr));



	*/
	std::uint32_t lual_addvalue_addr = 0x01383DB0;
#define lual_addvalue_CCV __cdecl

	/*

	* lual_argerror_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_argerror_TypeDef: int
	* lual_argerror_Aob: 55 8B EC 51 56 8B 75 8 56
	* lual_argerror_ProtectedByRetcheck: false
	* lual_argerror_IsApartOfLua: true
	* lual_argerror_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_argerror_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_argerror_typedef r_lual_argerror = (r_lual_argerror_typedef)(x(0x01383E40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_argerror_CCV* r_lual_argerror_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_argerror_typedef r_lual_argerror = (r_lual_argerror_typedef)(x(Adresses::lual_argerror_addr));



	*/
	std::uint32_t lual_argerror_addr = 0x01383E40;
#define lual_argerror_CCV __cdecl

	/*

	* lual_buffinit_Decompiled_Args: int a1, int a2
	* lual_buffinit_TypeDef: int
	* lual_buffinit_Aob: 55 8B EC 8B 4D C 8B
	* lual_buffinit_ProtectedByRetcheck: false
	* lual_buffinit_IsApartOfLua: true
	* lual_buffinit_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_buffinit_typedef)(int a1, int a2);
	r_lual_buffinit_typedef r_lual_buffinit = (r_lual_buffinit_typedef)(x(0x01383E90));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_buffinit_CCV* r_lual_buffinit_typedef)(int a1, int a2);
	r_lual_buffinit_typedef r_lual_buffinit = (r_lual_buffinit_typedef)(x(Adresses::lual_buffinit_addr));



	*/
	std::uint32_t lual_buffinit_addr = 0x01383E90;
#define lual_buffinit_CCV __cdecl

	/*

	* lual_checkany_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_checkany_TypeDef: int
	* lual_checkany_Aob: 55 8B EC 51 FF 75 C FF
	* lual_checkany_ProtectedByRetcheck: false
	* lual_checkany_IsApartOfLua: true
	* lual_checkany_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_checkany_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkany_typedef r_lual_checkany = (r_lual_checkany_typedef)(x(0x01383F40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_checkany_CCV* r_lual_checkany_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkany_typedef r_lual_checkany = (r_lual_checkany_typedef)(x(Adresses::lual_checkany_addr));



	*/
	std::uint32_t lual_checkany_addr = 0x01383F40;
#define lual_checkany_CCV __cdecl

	/*

	* lual_checkinteger_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_checkinteger_TypeDef: int
	* lual_checkinteger_Aob: 55 8B EC 51 8D 45 FC 50 FF 75 C FF
	* lual_checkinteger_ProtectedByRetcheck: false
	* lual_checkinteger_IsApartOfLua: true
	* lual_checkinteger_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_checkinteger_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkinteger_typedef r_lual_checkinteger = (r_lual_checkinteger_typedef)(x(0x01383F80));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_checkinteger_CCV* r_lual_checkinteger_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkinteger_typedef r_lual_checkinteger = (r_lual_checkinteger_typedef)(x(Adresses::lual_checkinteger_addr));



	*/
	std::uint32_t lual_checkinteger_addr = 0x01383F80;
#define lual_checkinteger_CCV __cdecl

	/*

	* lual_checklstring_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_checklstring_TypeDef: int
	* lual_checklstring_Aob: 55 8B EC FF 75 10 FF 75 C FF
	* lual_checklstring_ProtectedByRetcheck: false
	* lual_checklstring_IsApartOfLua: true
	* lual_checklstring_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_checklstring_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checklstring_typedef r_lual_checklstring = (r_lual_checklstring_typedef)(x(0x01383FB0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_checklstring_CCV* r_lual_checklstring_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checklstring_typedef r_lual_checklstring = (r_lual_checklstring_typedef)(x(Adresses::lual_checklstring_addr));



	*/
	std::uint32_t lual_checklstring_addr = 0x01383FB0;
#define lual_checklstring_CCV __cdecl

	/*

	* lual_checknumber_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_checknumber_TypeDef: int
	* lual_checknumber_Aob: 55 8B EC 51 8D 45 FC 50 FF 75 C FF
	* lual_checknumber_ProtectedByRetcheck: false
	* lual_checknumber_IsApartOfLua: true
	* lual_checknumber_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_checknumber_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checknumber_typedef r_lual_checknumber = (r_lual_checknumber_typedef)(x(0x01383FE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_checknumber_CCV* r_lual_checknumber_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checknumber_typedef r_lual_checknumber = (r_lual_checknumber_typedef)(x(Adresses::lual_checknumber_addr));



	*/
	std::uint32_t lual_checknumber_addr = 0x01383FE0;
#define lual_checknumber_CCV __cdecl

	/*

	* lual_checkstack_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_checkstack_TypeDef: int
	* lual_checkstack_Aob: 55 8B EC 51 FF 75 C FF
	* lual_checkstack_ProtectedByRetcheck: false
	* lual_checkstack_IsApartOfLua: true
	* lual_checkstack_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_checkstack_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkstack_typedef r_lual_checkstack = (r_lual_checkstack_typedef)(x(0x01384010));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_checkstack_CCV* r_lual_checkstack_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkstack_typedef r_lual_checkstack = (r_lual_checkstack_typedef)(x(Adresses::lual_checkstack_addr));



	*/
	std::uint32_t lual_checkstack_addr = 0x01384010;
#define lual_checkstack_CCV __cdecl

	/*

	* lual_checktype_Decompiled_Args: int a1
	* lual_checktype_TypeDef: int
	* lual_checktype_Aob: 55 8B EC FF 75 C FF
	* lual_checktype_ProtectedByRetcheck: false
	* lual_checktype_IsApartOfLua: true
	* lual_checktype_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_checktype_typedef)(int a1);
	r_lual_checktype_typedef r_lual_checktype = (r_lual_checktype_typedef)(x(0x01384050));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_checktype_CCV* r_lual_checktype_typedef)(int a1);
	r_lual_checktype_typedef r_lual_checktype = (r_lual_checktype_typedef)(x(Adresses::lual_checktype_addr));



	*/
	std::uint32_t lual_checktype_addr = 0x01384050;
#define lual_checktype_CCV __cdecl

	/*

	* lual_checkunsigned_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_checkunsigned_TypeDef: int
	* lual_checkunsigned_Aob: 55 8B EC 51 8D 45 FC 50 FF 75 C FF
	* lual_checkunsigned_ProtectedByRetcheck: false
	* lual_checkunsigned_IsApartOfLua: true
	* lual_checkunsigned_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_checkunsigned_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkunsigned_typedef r_lual_checkunsigned = (r_lual_checkunsigned_typedef)(x(0x01384080));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_checkunsigned_CCV* r_lual_checkunsigned_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_checkunsigned_typedef r_lual_checkunsigned = (r_lual_checkunsigned_typedef)(x(Adresses::lual_checkunsigned_addr));



	*/
	std::uint32_t lual_checkunsigned_addr = 0x01384080;
#define lual_checkunsigned_CCV __cdecl

	/*

	* lual_error_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_error_TypeDef: int
	* lual_error_Aob: 55 8B EC 56 8B 75 8 6A
	* lual_error_ProtectedByRetcheck: false
	* lual_error_IsApartOfLua: true
	* lual_error_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_error_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_error_typedef r_lual_error = (r_lual_error_typedef)(x(0x013840B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_error_CCV* r_lual_error_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_error_typedef r_lual_error = (r_lual_error_typedef)(x(Adresses::lual_error_addr));



	*/
	std::uint32_t lual_error_addr = 0x013840B0;
#define lual_error_CCV __cdecl

	/*

	* lual_findtable_Decompiled_Args: int a1, int a2, int a3
	* lual_findtable_TypeDef: int
	* lual_findtable_Aob: 55 8B EC 53 56 8B 75 8 57
	* lual_findtable_ProtectedByRetcheck: false
	* lual_findtable_IsApartOfLua: true
	* lual_findtable_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_findtable_typedef)(int a1, int a2, int a3);
	r_lual_findtable_typedef r_lual_findtable = (r_lual_findtable_typedef)(x(0x013840E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_findtable_CCV* r_lual_findtable_typedef)(int a1, int a2, int a3);
	r_lual_findtable_typedef r_lual_findtable = (r_lual_findtable_typedef)(x(Adresses::lual_findtable_addr));



	*/
	std::uint32_t lual_findtable_addr = 0x013840E0;
#define lual_findtable_CCV __cdecl

	/*

	* lual_getmetafield_Decompiled_Args: int a1, int a2
	* lual_getmetafield_TypeDef: int
	* lual_getmetafield_Aob: 55 8B EC 56 FF 75 C 8B
	* lual_getmetafield_ProtectedByRetcheck: false
	* lual_getmetafield_IsApartOfLua: true
	* lual_getmetafield_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_getmetafield_typedef)(int a1, int a2);
	r_lual_getmetafield_typedef r_lual_getmetafield = (r_lual_getmetafield_typedef)(x(0x013841D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_getmetafield_CCV* r_lual_getmetafield_typedef)(int a1, int a2);
	r_lual_getmetafield_typedef r_lual_getmetafield = (r_lual_getmetafield_typedef)(x(Adresses::lual_getmetafield_addr));



	*/
	std::uint32_t lual_getmetafield_addr = 0x013841D0;
#define lual_getmetafield_CCV __cdecl

	/*

	* lual_newmetatable_Decompiled_Args: int a1
	* lual_newmetatable_TypeDef: int
	* lual_newmetatable_Aob: 55 8B EC 56 FF 75 C 8B
	* lual_newmetatable_ProtectedByRetcheck: false
	* lual_newmetatable_IsApartOfLua: true
	* lual_newmetatable_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_newmetatable_typedef)(int a1);
	r_lual_newmetatable_typedef r_lual_newmetatable = (r_lual_newmetatable_typedef)(x(0x01384230));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_newmetatable_CCV* r_lual_newmetatable_typedef)(int a1);
	r_lual_newmetatable_typedef r_lual_newmetatable = (r_lual_newmetatable_typedef)(x(Adresses::lual_newmetatable_addr));



	*/
	std::uint32_t lual_newmetatable_addr = 0x01384230;
#define lual_newmetatable_CCV __cdecl

	/*

	* lual_optinteger_Decompiled_Args: int a1, int a2, int a3
	* lual_optinteger_TypeDef: int
	* lual_optinteger_Aob: 55 8B EC 51 56 8B 75 C 57
	* lual_optinteger_ProtectedByRetcheck: false
	* lual_optinteger_IsApartOfLua: true
	* lual_optinteger_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_optinteger_typedef)(int a1, int a2, int a3);
	r_lual_optinteger_typedef r_lual_optinteger = (r_lual_optinteger_typedef)(x(0x013842A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_optinteger_CCV* r_lual_optinteger_typedef)(int a1, int a2, int a3);
	r_lual_optinteger_typedef r_lual_optinteger = (r_lual_optinteger_typedef)(x(Adresses::lual_optinteger_addr));



	*/
	std::uint32_t lual_optinteger_addr = 0x013842A0;
#define lual_optinteger_CCV __cdecl

	/*

	* lual_optlstring_Decompiled_Args: int a1, int a2, int a3, const char* a4
	* lual_optlstring_TypeDef: int
	* lual_optlstring_Aob: 55 8B EC 56 8B 75 C 57
	* lual_optlstring_ProtectedByRetcheck: false
	* lual_optlstring_IsApartOfLua: true
	* lual_optlstring_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_optlstring_typedef)(int a1, int a2, int a3, const char* a4);
	r_lual_optlstring_typedef r_lual_optlstring = (r_lual_optlstring_typedef)(x(0x013842F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_optlstring_CCV* r_lual_optlstring_typedef)(int a1, int a2, int a3, const char* a4);
	r_lual_optlstring_typedef r_lual_optlstring = (r_lual_optlstring_typedef)(x(Adresses::lual_optlstring_addr));



	*/
	std::uint32_t lual_optlstring_addr = 0x013842F0;
#define lual_optlstring_CCV __cdecl

	/*

	* lual_prepbuffer_Decompiled_Args: int a1
	* lual_prepbuffer_TypeDef: int
	* lual_prepbuffer_Aob: 55 8B EC 56 8B 75 8 57
	* lual_prepbuffer_ProtectedByRetcheck: false
	* lual_prepbuffer_IsApartOfLua: true
	* lual_prepbuffer_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_prepbuffer_typedef)(int a1);
	r_lual_prepbuffer_typedef r_lual_prepbuffer = (r_lual_prepbuffer_typedef)(x(0x013843D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_prepbuffer_CCV* r_lual_prepbuffer_typedef)(int a1);
	r_lual_prepbuffer_typedef r_lual_prepbuffer = (r_lual_prepbuffer_typedef)(x(Adresses::lual_prepbuffer_addr));



	*/
	std::uint32_t lual_prepbuffer_addr = 0x013843D0;
#define lual_prepbuffer_CCV __cdecl

	/*

	* lual_pushresult_Decompiled_Args: int a1
	* lual_pushresult_TypeDef: int
	* lual_pushresult_Aob: 55 8B EC 57 8B 7D 8 8B
	* lual_pushresult_ProtectedByRetcheck: false
	* lual_pushresult_IsApartOfLua: true
	* lual_pushresult_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_pushresult_typedef)(int a1);
	r_lual_pushresult_typedef r_lual_pushresult = (r_lual_pushresult_typedef)(x(0x01384410));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_pushresult_CCV* r_lual_pushresult_typedef)(int a1);
	r_lual_pushresult_typedef r_lual_pushresult = (r_lual_pushresult_typedef)(x(Adresses::lual_pushresult_addr));



	*/
	std::uint32_t lual_pushresult_addr = 0x01384410;
#define lual_pushresult_CCV __cdecl

	/*

	* lual_ref_Decompiled_Args: int a1, int a2
	* lual_ref_TypeDef: int
	* lual_ref_Aob: 55 8B EC 56 8B 75 8 57
	* lual_ref_ProtectedByRetcheck: false
	* lual_ref_IsApartOfLua: true
	* lual_ref_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_ref_typedef)(int a1, int a2);
	r_lual_ref_typedef r_lual_ref = (r_lual_ref_typedef)(x(0x01384450));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_ref_CCV* r_lual_ref_typedef)(int a1, int a2);
	r_lual_ref_typedef r_lual_ref = (r_lual_ref_typedef)(x(Adresses::lual_ref_addr));



	*/
	std::uint32_t lual_ref_addr = 0x01384450;
#define lual_ref_CCV __cdecl

	/*

	* lual_register_Decompiled_Args: int a1, int a2, int a3
	* lual_register_TypeDef: int
	* lual_register_Aob: 55 8B EC 83 7D C 0 56
	* lual_register_ProtectedByRetcheck: false
	* lual_register_IsApartOfLua: true
	* lual_register_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_register_typedef)(int a1, int a2, int a3);
	r_lual_register_typedef r_lual_register = (r_lual_register_typedef)(x(0x01384510));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_register_CCV* r_lual_register_typedef)(int a1, int a2, int a3);
	r_lual_register_typedef r_lual_register = (r_lual_register_typedef)(x(Adresses::lual_register_addr));



	*/
	std::uint32_t lual_register_addr = 0x01384510;
#define lual_register_CCV __cdecl

	/*

	* lual_typeerror_Decompiled_Args: std::uint32_t Rbx_LuaState
	* lual_typeerror_TypeDef: int
	* lual_typeerror_Aob: 55 8B EC 51 53 56 8B 75 8 57
	* lual_typeerror_ProtectedByRetcheck: false
	* lual_typeerror_IsApartOfLua: true
	* lual_typeerror_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_typeerror_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_typeerror_typedef r_lual_typeerror = (r_lual_typeerror_typedef)(x(0x01384600));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_typeerror_CCV* r_lual_typeerror_typedef)(std::uint32_t Rbx_LuaState);
	r_lual_typeerror_typedef r_lual_typeerror = (r_lual_typeerror_typedef)(x(Adresses::lual_typeerror_addr));



	*/
	std::uint32_t lual_typeerror_addr = 0x01384600;
#define lual_typeerror_CCV __cdecl

	/*

	* lual_unref_Decompiled_Args: int a1, int a2, int a3
	* lual_unref_TypeDef: int
	* lual_unref_Aob: 55 8B EC 53 8B 5D 10 85 DB 78 47 56 8B 75 C 57
	* lual_unref_ProtectedByRetcheck: false
	* lual_unref_IsApartOfLua: true
	* lual_unref_Mask: ................
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_unref_typedef)(int a1, int a2, int a3);
	r_lual_unref_typedef r_lual_unref = (r_lual_unref_typedef)(x(0x013846A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_unref_CCV* r_lual_unref_typedef)(int a1, int a2, int a3);
	r_lual_unref_typedef r_lual_unref = (r_lual_unref_typedef)(x(Adresses::lual_unref_addr));



	*/
	std::uint32_t lual_unref_addr = 0x013846A0;
#define lual_unref_CCV __cdecl

	/*

	* lual_where_Decompiled_Args: int a1
	* lual_where_TypeDef: int
	* lual_where_Aob: 55 8B EC 81 EC 24 1 0 0 8D
	* lual_where_ProtectedByRetcheck: false
	* lual_where_IsApartOfLua: true
	* lual_where_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_lual_where_typedef)(int a1);
	r_lual_where_typedef r_lual_where = (r_lual_where_typedef)(x(0x01384700));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(lual_where_CCV* r_lual_where_typedef)(int a1);
	r_lual_where_typedef r_lual_where = (r_lual_where_typedef)(x(Adresses::lual_where_addr));



	*/
	std::uint32_t lual_where_addr = 0x01384700;
#define lual_where_CCV __cdecl

	/*

	* luam_realloc_Decompiled_Args: int a1, int a2, int a3, int a4
	* luam_realloc_TypeDef: int
	* luam_realloc_Aob: 55 8B EC 8B 4D 8 53
	* luam_realloc_ProtectedByRetcheck: false
	* luam_realloc_IsApartOfLua: true
	* luam_realloc_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luam_realloc_typedef)(int a1, int a2, int a3, int a4);
	r_luam_realloc_typedef r_luam_realloc = (r_luam_realloc_typedef)(x(0x0139A450));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luam_realloc_CCV* r_luam_realloc_typedef)(int a1, int a2, int a3, int a4);
	r_luam_realloc_typedef r_luam_realloc = (r_luam_realloc_typedef)(x(Adresses::luam_realloc_addr));



	*/
	std::uint32_t luam_realloc_addr = 0x0139A450;
#define luam_realloc_CCV __cdecl

	/*

	* luam_realloc__Decompiled_Args: int a1, int a2
	* luam_realloc__TypeDef: int
	* luam_realloc__Aob: 55 8B EC 53 8B 5D 8 56
	* luam_realloc__ProtectedByRetcheck: false
	* luam_realloc__IsApartOfLua: true
	* luam_realloc__Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luam_realloc__typedef)(int a1, int a2);
	r_luam_realloc__typedef r_luam_realloc_ = (r_luam_realloc__typedef)(x(0x0139A310));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luam_realloc__CCV* r_luam_realloc__typedef)(int a1, int a2);
	r_luam_realloc__typedef r_luam_realloc_ = (r_luam_realloc__typedef)(x(Adresses::luam_realloc__addr));



	*/
	std::uint32_t luam_realloc__addr = 0x0139A310;
#define luam_realloc__CCV __cdecl

	/*

	* luam_toobig_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luam_toobig_TypeDef: int
	* luam_toobig_Aob: 55 8B EC 51 C7 45 FC 4E D5 89 2 8B
	* luam_toobig_ProtectedByRetcheck: false
	* luam_toobig_IsApartOfLua: true
	* luam_toobig_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luam_toobig_typedef)(std::uint32_t Rbx_LuaState);
	r_luam_toobig_typedef r_luam_toobig = (r_luam_toobig_typedef)(x(0x0139A560));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luam_toobig_CCV* r_luam_toobig_typedef)(std::uint32_t Rbx_LuaState);
	r_luam_toobig_typedef r_luam_toobig = (r_luam_toobig_typedef)(x(Adresses::luam_toobig_addr));



	*/
	std::uint32_t luam_toobig_addr = 0x0139A560;
#define luam_toobig_CCV __cdecl

	/*

	* luao_chunkid_Decompiled_Args: int a1, int a2, const char* a3
	* luao_chunkid_TypeDef: int
	* luao_chunkid_Aob: 55 8B EC 53 8B 5D C 56
	* luao_chunkid_ProtectedByRetcheck: false
	* luao_chunkid_IsApartOfLua: true
	* luao_chunkid_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luao_chunkid_typedef)(int a1, int a2, const char* a3);
	r_luao_chunkid_typedef r_luao_chunkid = (r_luao_chunkid_typedef)(x(0x013853B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luao_chunkid_CCV* r_luao_chunkid_typedef)(int a1, int a2, const char* a3);
	r_luao_chunkid_typedef r_luao_chunkid = (r_luao_chunkid_typedef)(x(Adresses::luao_chunkid_addr));



	*/
	std::uint32_t luao_chunkid_addr = 0x013853B0;
#define luao_chunkid_CCV __cdecl

	/*

	* luao_pushfstring_Decompiled_Args: int a1, int a2
	* luao_pushfstring_TypeDef: int
	* luao_pushfstring_Aob: 55 8B EC 81 EC 0 2 0 0 8D
	* luao_pushfstring_ProtectedByRetcheck: false
	* luao_pushfstring_IsApartOfLua: true
	* luao_pushfstring_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luao_pushfstring_typedef)(int a1, int a2);
	r_luao_pushfstring_typedef r_luao_pushfstring = (r_luao_pushfstring_typedef)(x(0x01385530));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luao_pushfstring_CCV* r_luao_pushfstring_typedef)(int a1, int a2);
	r_luao_pushfstring_typedef r_luao_pushfstring = (r_luao_pushfstring_typedef)(x(Adresses::luao_pushfstring_addr));



	*/
	std::uint32_t luao_pushfstring_addr = 0x01385530;
#define luao_pushfstring_CCV __cdecl

	/*

	* luao_pushvfstring_Decompiled_Args: int a1
	* luao_pushvfstring_TypeDef: int
	* luao_pushvfstring_Aob: 55 8B EC 81 EC 0 2 0 0 8D
	* luao_pushvfstring_ProtectedByRetcheck: false
	* luao_pushvfstring_IsApartOfLua: true
	* luao_pushvfstring_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luao_pushvfstring_typedef)(int a1);
	r_luao_pushvfstring_typedef r_luao_pushvfstring = (r_luao_pushvfstring_typedef)(x(0x013855D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luao_pushvfstring_CCV* r_luao_pushvfstring_typedef)(int a1);
	r_luao_pushvfstring_typedef r_luao_pushvfstring = (r_luao_pushvfstring_typedef)(x(Adresses::luao_pushvfstring_addr));



	*/
	std::uint32_t luao_pushvfstring_addr = 0x013855D0;
#define luao_pushvfstring_CCV __cdecl

	/*

	* luao_rawequalKey_Decompiled_Args: int a1, int a2
	* luao_rawequalKey_TypeDef: int
	* luao_rawequalKey_Aob: 55 8B EC 8B 4D 8 8B
	* luao_rawequalKey_ProtectedByRetcheck: false
	* luao_rawequalKey_IsApartOfLua: true
	* luao_rawequalKey_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luao_rawequalKey_typedef)(int a1, int a2);
	r_luao_rawequalKey_typedef r_luao_rawequalKey = (r_luao_rawequalKey_typedef)(x(0x01385670));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luao_rawequalKey_CCV* r_luao_rawequalKey_typedef)(int a1, int a2);
	r_luao_rawequalKey_typedef r_luao_rawequalKey = (r_luao_rawequalKey_typedef)(x(Adresses::luao_rawequalKey_addr));



	*/
	std::uint32_t luao_rawequalKey_addr = 0x01385670;
#define luao_rawequalKey_CCV __cdecl

	/*

	* luao_rawequalobj_Decompiled_Args: int a1, int a2
	* luao_rawequalobj_TypeDef: int
	* luao_rawequalobj_Aob: 55 8B EC 8B 4D 8 8B
	* luao_rawequalobj_ProtectedByRetcheck: false
	* luao_rawequalobj_IsApartOfLua: true
	* luao_rawequalobj_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luao_rawequalobj_typedef)(int a1, int a2);
	r_luao_rawequalobj_typedef r_luao_rawequalobj = (r_luao_rawequalobj_typedef)(x(0x013856F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luao_rawequalobj_CCV* r_luao_rawequalobj_typedef)(int a1, int a2);
	r_luao_rawequalobj_typedef r_luao_rawequalobj = (r_luao_rawequalobj_typedef)(x(Adresses::luao_rawequalobj_addr));



	*/
	std::uint32_t luao_rawequalobj_addr = 0x013856F0;
#define luao_rawequalobj_CCV __cdecl

	/*

	* luao_str2d_Decompiled_Args: int a1, int a2
	* luao_str2d_TypeDef: int
	* luao_str2d_Aob: 55 8B EC 51 56 8B 75 8 8D
	* luao_str2d_ProtectedByRetcheck: false
	* luao_str2d_IsApartOfLua: true
	* luao_str2d_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luao_str2d_typedef)(int a1, int a2);
	r_luao_str2d_typedef r_luao_str2d = (r_luao_str2d_typedef)(x(0x01385770));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luao_str2d_CCV* r_luao_str2d_typedef)(int a1, int a2);
	r_luao_str2d_typedef r_luao_str2d = (r_luao_str2d_typedef)(x(Adresses::luao_str2d_addr));



	*/
	std::uint32_t luao_str2d_addr = 0x01385770;
#define luao_str2d_CCV __cdecl

	/*

	* luas_newlstr_Decompiled_Args: int a1, int a2, int a3
	* luas_newlstr_TypeDef: int
	* luas_newlstr_Aob: 55 8B EC 83 EC 8 8B
	* luas_newlstr_ProtectedByRetcheck: false
	* luas_newlstr_IsApartOfLua: true
	* luas_newlstr_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_luas_newlstr_typedef)(int a1, int a2, int a3);
	r_luas_newlstr_typedef r_luas_newlstr = (r_luas_newlstr_typedef)(x(0x013979B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luas_newlstr_CCV* r_luas_newlstr_typedef)(int a1, int a2, int a3);
	r_luas_newlstr_typedef r_luas_newlstr = (r_luas_newlstr_typedef)(x(Adresses::luas_newlstr_addr));



	*/
	std::uint32_t luas_newlstr_addr = 0x013979B0;
#define luas_newlstr_CCV __fastcall

	/*

	* luas_newudata_Decompiled_Args: int a1, int a2
	* luas_newudata_TypeDef: int
	* luas_newudata_Aob: 55 8B EC 53 8B 5D C 57
	* luas_newudata_ProtectedByRetcheck: false
	* luas_newudata_IsApartOfLua: true
	* luas_newudata_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luas_newudata_typedef)(int a1, int a2);
	r_luas_newudata_typedef r_luas_newudata = (r_luas_newudata_typedef)(x(0x01397AD0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luas_newudata_CCV* r_luas_newudata_typedef)(int a1, int a2);
	r_luas_newudata_typedef r_luas_newudata = (r_luas_newudata_typedef)(x(Adresses::luas_newudata_addr));



	*/
	std::uint32_t luas_newudata_addr = 0x01397AD0;
#define luas_newudata_CCV __cdecl

	/*

	* luas_resize_Decompiled_Args: int a1, int a2
	* luas_resize_TypeDef: int
	* luas_resize_Aob: 55 8B EC 8B 4D 8 83
	* luas_resize_ProtectedByRetcheck: false
	* luas_resize_IsApartOfLua: true
	* luas_resize_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luas_resize_typedef)(int a1, int a2);
	r_luas_resize_typedef r_luas_resize = (r_luas_resize_typedef)(x(0x01397B20));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luas_resize_CCV* r_luas_resize_typedef)(int a1, int a2);
	r_luas_resize_typedef r_luas_resize = (r_luas_resize_typedef)(x(Adresses::luas_resize_addr));



	*/
	std::uint32_t luas_resize_addr = 0x01397B20;
#define luas_resize_CCV __cdecl

	/*

	* luat_gettm_Decompiled_Args: int a1, int a2
	* luat_gettm_TypeDef: int
	* luat_gettm_Aob: 55 8B EC 56 FF 75 10 8B 75 8 56
	* luat_gettm_ProtectedByRetcheck: false
	* luat_gettm_IsApartOfLua: true
	* luat_gettm_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luat_gettm_typedef)(int a1, int a2);
	r_luat_gettm_typedef r_luat_gettm = (r_luat_gettm_typedef)(x(0x01399FC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luat_gettm_CCV* r_luat_gettm_typedef)(int a1, int a2);
	r_luat_gettm_typedef r_luat_gettm = (r_luat_gettm_typedef)(x(Adresses::luat_gettm_addr));



	*/
	std::uint32_t luat_gettm_addr = 0x01399FC0;
#define luat_gettm_CCV __cdecl

	/*

	* luat_objtypename_Decompiled_Args: int a1, int a2
	* luat_objtypename_TypeDef: int
	* luat_objtypename_Aob: 55 8B EC 53 8B 5D 8 56
	* luat_objtypename_ProtectedByRetcheck: false
	* luat_objtypename_IsApartOfLua: true
	* luat_objtypename_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luat_objtypename_typedef)(int a1, int a2);
	r_luat_objtypename_typedef r_luat_objtypename = (r_luat_objtypename_typedef)(x(0x0139A120));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luat_objtypename_CCV* r_luat_objtypename_typedef)(int a1, int a2);
	r_luat_objtypename_typedef r_luat_objtypename = (r_luat_objtypename_typedef)(x(Adresses::luat_objtypename_addr));



	*/
	std::uint32_t luat_objtypename_addr = 0x0139A120;
#define luat_objtypename_CCV __cdecl

	/*

	* luau_backtrace_Decompiled_Args: int a1, int a2, const char* a3
	* luau_backtrace_TypeDef: int
	* luau_backtrace_Aob: 55 8B EC 6A FF 68 F9 2B 1D 2 64
	* luau_backtrace_ProtectedByRetcheck: false
	* luau_backtrace_IsApartOfLua: true
	* luau_backtrace_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_backtrace_typedef)(int a1, int a2, const char* a3);
	r_luau_backtrace_typedef r_luau_backtrace = (r_luau_backtrace_typedef)(x(0x0138CB30));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_backtrace_CCV* r_luau_backtrace_typedef)(int a1, int a2, const char* a3);
	r_luau_backtrace_typedef r_luau_backtrace = (r_luau_backtrace_typedef)(x(Adresses::luau_backtrace_addr));



	*/
	std::uint32_t luau_backtrace_addr = 0x0138CB30;
#define luau_backtrace_CCV __cdecl

	/*

	* luau_betavm_Decompiled_Args: int a1
	* luau_betavm_TypeDef: bool
	* luau_betavm_Aob: 55 8B EC 83 E4 F0 81 EC B8 0 0 0 56
	* luau_betavm_ProtectedByRetcheck: false
	* luau_betavm_IsApartOfLua: true
	* luau_betavm_Mask: .............
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_luau_betavm_typedef)(int a1);
	r_luau_betavm_typedef r_luau_betavm = (r_luau_betavm_typedef)(x(0x013891B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(luau_betavm_CCV* r_luau_betavm_typedef)(int a1);
	r_luau_betavm_typedef r_luau_betavm = (r_luau_betavm_typedef)(x(Adresses::luau_betavm_addr));



	*/
	std::uint32_t luau_betavm_addr = 0x013891B0;
#define luau_betavm_CCV __cdecl

	/*

	* luau_callhook_Decompiled_Args: int a1, int a2, int a3
	* luau_callhook_TypeDef: int
	* luau_callhook_Aob: 55 8B EC 81 EC 24 1 0 0 53
	* luau_callhook_ProtectedByRetcheck: true
	* luau_callhook_IsApartOfLua: true
	* luau_callhook_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_callhook_typedef)(int a1, int a2, int a3);
	r_luau_callhook_typedef r_luau_callhook = (r_luau_callhook_typedef)unprotect(x(0x0138D0E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_callhook_CCV* r_luau_callhook_typedef)(int a1, int a2, int a3);
	r_luau_callhook_typedef r_luau_callhook = (r_luau_callhook_typedef)unprotect(x(Adresses::luau_callhook_addr));



	*/
	std::uint32_t luau_callhook_addr = 0x0138D0E0;
#define luau_callhook_CCV __cdecl

	/*

	* luau_calltm_Decompiled_Args: int a1, int a2, int a3
	* luau_calltm_TypeDef: int
	* luau_calltm_Aob: 55 8B EC 53 57 8B 7D 8 B8
	* luau_calltm_ProtectedByRetcheck: false
	* luau_calltm_IsApartOfLua: true
	* luau_calltm_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_calltm_typedef)(int a1, int a2, int a3);
	r_luau_calltm_typedef r_luau_calltm = (r_luau_calltm_typedef)(x(0x0138CFC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_calltm_CCV* r_luau_calltm_typedef)(int a1, int a2, int a3);
	r_luau_calltm_typedef r_luau_calltm = (r_luau_calltm_typedef)(x(Adresses::luau_calltm_addr));



	*/
	std::uint32_t luau_calltm_addr = 0x0138CFC0;
#define luau_calltm_CCV __cdecl

	/*

	* luau_deserialize_Decompiled_Args: int a1, int a2, const char* a3, int a4
	* luau_deserialize_TypeDef: int
	* luau_deserialize_Aob: 55 8B EC 6A FF 68 20 2C 1D 2 64
	* luau_deserialize_ProtectedByRetcheck: false
	* luau_deserialize_IsApartOfLua: true
	* luau_deserialize_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_deserialize_typedef)(int a1, int a2, const char* a3, int a4);
	r_luau_deserialize_typedef r_luau_deserialize = (r_luau_deserialize_typedef)(x(0x0138D270));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_deserialize_CCV* r_luau_deserialize_typedef)(int a1, int a2, const char* a3, int a4);
	r_luau_deserialize_typedef r_luau_deserialize = (r_luau_deserialize_typedef)(x(Adresses::luau_deserialize_addr));



	*/
	std::uint32_t luau_deserialize_addr = 0x0138D270;
#define luau_deserialize_CCV __cdecl

	/*

	* luau_execute_Decompiled_Args: int a1, int a2, int a3, int a4, int a5
	* luau_execute_TypeDef: int
	* luau_execute_Aob: 55 8B EC 51 80 3D CC FE E9 2 0 74
	* luau_execute_ProtectedByRetcheck: false
	* luau_execute_IsApartOfLua: true
	* luau_execute_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_execute_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_luau_execute_typedef r_luau_execute = (r_luau_execute_typedef)(x(0x013AFA10));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_execute_CCV* r_luau_execute_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_luau_execute_typedef r_luau_execute = (r_luau_execute_typedef)(x(Adresses::luau_execute_addr));



	*/
	std::uint32_t luau_execute_addr = 0x013AFA10;
#define luau_execute_CCV __cdecl

	/*

	* luau_loopforg_Decompiled_Args: int a1, int a2
	* luau_loopforg_TypeDef: int
	* luau_loopforg_Aob: 55 8B EC 56 8B 75 C 57
	* luau_loopforg_ProtectedByRetcheck: false
	* luau_loopforg_IsApartOfLua: true
	* luau_loopforg_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_loopforg_typedef)(int a1, int a2);
	r_luau_loopforg_typedef r_luau_loopforg = (r_luau_loopforg_typedef)(x(0x0138DC90));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_loopforg_CCV* r_luau_loopforg_typedef)(int a1, int a2);
	r_luau_loopforg_typedef r_luau_loopforg = (r_luau_loopforg_typedef)(x(Adresses::luau_loopforg_addr));



	*/
	std::uint32_t luau_loopforg_addr = 0x0138DC90;
#define luau_loopforg_CCV __cdecl

	/*

	* luau_prepareforn_Decompiled_Args: int a1, int a2, int a3
	* luau_prepareforn_TypeDef: int
	* luau_prepareforn_Aob: 55 8B EC 56 8B 75 14 83 7E C 4 74
	* luau_prepareforn_ProtectedByRetcheck: false
	* luau_prepareforn_IsApartOfLua: true
	* luau_prepareforn_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_prepareforn_typedef)(int a1, int a2, int a3);
	r_luau_prepareforn_typedef r_luau_prepareforn = (r_luau_prepareforn_typedef)(x(0x0138DEB0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_prepareforn_CCV* r_luau_prepareforn_typedef)(int a1, int a2, int a3);
	r_luau_prepareforn_typedef r_luau_prepareforn = (r_luau_prepareforn_typedef)(x(Adresses::luau_prepareforn_addr));



	*/
	std::uint32_t luau_prepareforn_addr = 0x0138DEB0;
#define luau_prepareforn_CCV __cdecl

	/*

	* luau_resolveimport_Decompiled_Args: int a1, int a2, int a3, int a4, int a5
	* luau_resolveimport_TypeDef: int
	* luau_resolveimport_Aob: 55 8B EC 8B 4D 14 83 EC 10 8B C1 C1 E8 1E 53 57 85 C0 74 D 8B
	* luau_resolveimport_ProtectedByRetcheck: false
	* luau_resolveimport_IsApartOfLua: true
	* luau_resolveimport_Mask: .....................
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_resolveimport_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_luau_resolveimport_typedef r_luau_resolveimport = (r_luau_resolveimport_typedef)(x(0x0138DF50));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_resolveimport_CCV* r_luau_resolveimport_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_luau_resolveimport_typedef r_luau_resolveimport = (r_luau_resolveimport_typedef)(x(Adresses::luau_resolveimport_addr));



	*/
	std::uint32_t luau_resolveimport_addr = 0x0138DF50;
#define luau_resolveimport_CCV __cdecl

	/*

	* luau_tryfunctm_Decompiled_Args: int a1, int a2
	* luau_tryfunctm_TypeDef: int
	* luau_tryfunctm_Aob: 55 8B EC 53 56 8B 75 8 57
	* luau_tryfunctm_ProtectedByRetcheck: false
	* luau_tryfunctm_IsApartOfLua: true
	* luau_tryfunctm_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luau_tryfunctm_typedef)(int a1, int a2);
	r_luau_tryfunctm_typedef r_luau_tryfunctm = (r_luau_tryfunctm_typedef)(x(0x0138E0F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luau_tryfunctm_CCV* r_luau_tryfunctm_typedef)(int a1, int a2);
	r_luau_tryfunctm_typedef r_luau_tryfunctm = (r_luau_tryfunctm_typedef)(x(Adresses::luau_tryfunctm_addr));



	*/
	std::uint32_t luau_tryfunctm_addr = 0x0138E0F0;
#define luau_tryfunctm_CCV __cdecl

	/*

	* luav_concat_Decompiled_Args: int a1, const char* a2, const char* a3
	* luav_concat_TypeDef: int
	* luav_concat_Aob: 55 8B EC 8B 55 8 83
	* luav_concat_ProtectedByRetcheck: false
	* luav_concat_IsApartOfLua: true
	* luav_concat_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_concat_typedef)(int a1, const char* a2, const char* a3);
	r_luav_concat_typedef r_luav_concat = (r_luav_concat_typedef)(x(0x01399360));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_concat_CCV* r_luav_concat_typedef)(int a1, const char* a2, const char* a3);
	r_luav_concat_typedef r_luav_concat = (r_luav_concat_typedef)(x(Adresses::luav_concat_addr));



	*/
	std::uint32_t luav_concat_addr = 0x01399360;
#define luav_concat_CCV __cdecl

	/*

	* luav_doarith_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luav_doarith_TypeDef: int
	* luav_doarith_Aob: 53 8B DC 83 EC 8 83
	* luav_doarith_ProtectedByRetcheck: false
	* luav_doarith_IsApartOfLua: true
	* luav_doarith_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_doarith_typedef)(std::uint32_t Rbx_LuaState);
	r_luav_doarith_typedef r_luav_doarith = (r_luav_doarith_typedef)(x(0x013996A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_doarith_CCV* r_luav_doarith_typedef)(std::uint32_t Rbx_LuaState);
	r_luav_doarith_typedef r_luav_doarith = (r_luav_doarith_typedef)(x(Adresses::luav_doarith_addr));



	*/
	std::uint32_t luav_doarith_addr = 0x013996A0;
#define luav_doarith_CCV __cdecl

	/*

	* luav_dolen_Decompiled_Args: int a1, int a2, int a3
	* luav_dolen_TypeDef: int
	* luav_dolen_Aob: 55 8B EC 56 8B 75 10 8B 46 C 83
	* luav_dolen_ProtectedByRetcheck: false
	* luav_dolen_IsApartOfLua: true
	* luav_dolen_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_dolen_typedef)(int a1, int a2, int a3);
	r_luav_dolen_typedef r_luav_dolen = (r_luav_dolen_typedef)(x(0x013998D0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_dolen_CCV* r_luav_dolen_typedef)(int a1, int a2, int a3);
	r_luav_dolen_typedef r_luav_dolen = (r_luav_dolen_typedef)(x(Adresses::luav_dolen_addr));



	*/
	std::uint32_t luav_dolen_addr = 0x013998D0;
#define luav_dolen_CCV __cdecl

	/*

	* luav_execute_Decompiled_Args: int a1
	* luav_execute_TypeDef: bool
	* luav_execute_Aob: 55 8B EC 83 E4 F0 81 EC C8 0 0 0 56
	* luav_execute_ProtectedByRetcheck: false
	* luav_execute_IsApartOfLua: true
	* luav_execute_Mask: .............
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_luav_execute_typedef)(int a1);
	r_luav_execute_typedef r_luav_execute = (r_luav_execute_typedef)(x(0x01385860));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(luav_execute_CCV* r_luav_execute_typedef)(int a1);
	r_luav_execute_typedef r_luav_execute = (r_luav_execute_typedef)(x(Adresses::luav_execute_addr));



	*/
	std::uint32_t luav_execute_addr = 0x01385860;
#define luav_execute_CCV __cdecl

	/*

	* luav_gettable_Decompiled_Args: int a1, int a2, int a3, int a4, int a5
	* luav_gettable_TypeDef: int
	* luav_gettable_Aob: 55 8B EC 51 53 56 8B 75 C 57
	* luav_gettable_ProtectedByRetcheck: false
	* luav_gettable_IsApartOfLua: true
	* luav_gettable_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_gettable_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_luav_gettable_typedef r_luav_gettable = (r_luav_gettable_typedef)(x(0x01399B00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_gettable_CCV* r_luav_gettable_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_luav_gettable_typedef r_luav_gettable = (r_luav_gettable_typedef)(x(Adresses::luav_gettable_addr));



	*/
	std::uint32_t luav_gettable_addr = 0x01399B00;
#define luav_gettable_CCV __cdecl

	/*

	* luav_gettable_opt_Decompiled_Args: std::uint32_t Rbx_LuaState
	* luav_gettable_opt_TypeDef: int
	* luav_gettable_opt_Aob: 55 8B EC 6A 1 FF 75
	* luav_gettable_opt_ProtectedByRetcheck: false
	* luav_gettable_opt_IsApartOfLua: true
	* luav_gettable_opt_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_gettable_opt_typedef)(std::uint32_t Rbx_LuaState);
	r_luav_gettable_opt_typedef r_luav_gettable_opt = (r_luav_gettable_opt_typedef)(x(0x01399AE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_gettable_opt_CCV* r_luav_gettable_opt_typedef)(std::uint32_t Rbx_LuaState);
	r_luav_gettable_opt_typedef r_luav_gettable_opt = (r_luav_gettable_opt_typedef)(x(Adresses::luav_gettable_opt_addr));



	*/
	std::uint32_t luav_gettable_opt_addr = 0x01399AE0;
#define luav_gettable_opt_CCV __cdecl

	/*

	* luav_lessequal_Decompiled_Args: int a1, int a2, int a3
	* luav_lessequal_TypeDef: int
	* luav_lessequal_Aob: 55 8B EC 8B 4D 8 56
	* luav_lessequal_ProtectedByRetcheck: false
	* luav_lessequal_IsApartOfLua: true
	* luav_lessequal_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_lessequal_typedef)(int a1, int a2, int a3);
	r_luav_lessequal_typedef r_luav_lessequal = (r_luav_lessequal_typedef)(x(0x01399C10));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_lessequal_CCV* r_luav_lessequal_typedef)(int a1, int a2, int a3);
	r_luav_lessequal_typedef r_luav_lessequal = (r_luav_lessequal_typedef)(x(Adresses::luav_lessequal_addr));



	*/
	std::uint32_t luav_lessequal_addr = 0x01399C10;
#define luav_lessequal_CCV __cdecl

	/*

	* luav_lessthan_Decompiled_Args: int a1, int a2, int a3
	* luav_lessthan_TypeDef: int
	* luav_lessthan_Aob: 55 8B EC 8B 4D 8 56
	* luav_lessthan_ProtectedByRetcheck: false
	* luav_lessthan_IsApartOfLua: true
	* luav_lessthan_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_lessthan_typedef)(int a1, int a2, int a3);
	r_luav_lessthan_typedef r_luav_lessthan = (r_luav_lessthan_typedef)(x(0x01399CB0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_lessthan_CCV* r_luav_lessthan_typedef)(int a1, int a2, int a3);
	r_luav_lessthan_typedef r_luav_lessthan = (r_luav_lessthan_typedef)(x(Adresses::luav_lessthan_addr));



	*/
	std::uint32_t luav_lessthan_addr = 0x01399CB0;
#define luav_lessthan_CCV __cdecl

	/*

	* luav_settable_Decompiled_Args: int a1, int a2, int a3, int a4
	* luav_settable_TypeDef: int
	* luav_settable_Aob: 55 8B EC 83 EC 18 53 8B 5D C 56
	* luav_settable_ProtectedByRetcheck: false
	* luav_settable_IsApartOfLua: true
	* luav_settable_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_settable_typedef)(int a1, int a2, int a3, int a4);
	r_luav_settable_typedef r_luav_settable = (r_luav_settable_typedef)(x(0x01399D50));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_settable_CCV* r_luav_settable_typedef)(int a1, int a2, int a3, int a4);
	r_luav_settable_typedef r_luav_settable = (r_luav_settable_typedef)(x(Adresses::luav_settable_addr));



	*/
	std::uint32_t luav_settable_addr = 0x01399D50;
#define luav_settable_CCV __cdecl

	/*

	* luav_tonumber_Decompiled_Args: int a1, int a2
	* luav_tonumber_TypeDef: int
	* luav_tonumber_Aob: 55 8B EC 8B 45 8 83
	* luav_tonumber_ProtectedByRetcheck: false
	* luav_tonumber_IsApartOfLua: true
	* luav_tonumber_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_tonumber_typedef)(int a1, int a2);
	r_luav_tonumber_typedef r_luav_tonumber = (r_luav_tonumber_typedef)(x(0x01399EF0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_tonumber_CCV* r_luav_tonumber_typedef)(int a1, int a2);
	r_luav_tonumber_typedef r_luav_tonumber = (r_luav_tonumber_typedef)(x(Adresses::luav_tonumber_addr));



	*/
	std::uint32_t luav_tonumber_addr = 0x01399EF0;
#define luav_tonumber_CCV __cdecl

	/*

	* luav_tostring_Decompiled_Args: const char* a1, int a2
	* luav_tostring_TypeDef: int
	* luav_tostring_Aob: 55 8B EC 83 EC 20 56 8B 75 C 83
	* luav_tostring_ProtectedByRetcheck: false
	* luav_tostring_IsApartOfLua: true
	* luav_tostring_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luav_tostring_typedef)(const char* a1, int a2);
	r_luav_tostring_typedef r_luav_tostring = (r_luav_tostring_typedef)(x(0x01399F50));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luav_tostring_CCV* r_luav_tostring_typedef)(const char* a1, int a2);
	r_luav_tostring_typedef r_luav_tostring = (r_luav_tostring_typedef)(x(Adresses::luav_tostring_addr));



	*/
	std::uint32_t luav_tostring_addr = 0x01399F50;
#define luav_tostring_CCV __cdecl

	/*

	* luavm_compile_Decompiled_Args: int a1
	* luavm_compile_TypeDef: int
	* luavm_compile_Aob: 55 8B EC 56 8B 75 8 8B
	* luavm_compile_ProtectedByRetcheck: false
	* luavm_compile_IsApartOfLua: false
	* luavm_compile_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luavm_compile_typedef)(int a1);
	r_luavm_compile_typedef r_luavm_compile = (r_luavm_compile_typedef)(x(0x006C5AF0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luavm_compile_CCV* r_luavm_compile_typedef)(int a1);
	r_luavm_compile_typedef r_luavm_compile = (r_luavm_compile_typedef)(x(Adresses::luavm_compile_addr));



	*/
	std::uint32_t luavm_compile_addr = 0x006C5AF0;
#define luavm_compile_CCV __cdecl

	/*

	* luavm_load_Decompiled_Args: int a1, int a2, int a3
	* luavm_load_TypeDef: int
	* luavm_load_Aob: 55 8B EC 6A FF 68 B0 7 10 2 64
	* luavm_load_ProtectedByRetcheck: false
	* luavm_load_IsApartOfLua: false
	* luavm_load_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luavm_load_typedef)(int a1, int a2, int a3);
	r_luavm_load_typedef r_luavm_load = (r_luavm_load_typedef)(x(0x006C6030));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luavm_load_CCV* r_luavm_load_typedef)(int a1, int a2, int a3);
	r_luavm_load_typedef r_luavm_load = (r_luavm_load_typedef)(x(Adresses::luavm_load_addr));



	*/
	std::uint32_t luavm_load_addr = 0x006C6030;
#define luavm_load_CCV __cdecl

	/*

	* luax_init_Decompiled_Args: const char* a1
	* luax_init_TypeDef: bool
	* luax_init_Aob: 55 8B EC 53 56 57 8B 7D 8 BE
	* luax_init_ProtectedByRetcheck: false
	* luax_init_IsApartOfLua: true
	* luax_init_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_luax_init_typedef)(const char* a1);
	r_luax_init_typedef r_luax_init = (r_luax_init_typedef)(x(0x0139A070));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(luax_init_CCV* r_luax_init_typedef)(const char* a1);
	r_luax_init_typedef r_luax_init = (r_luax_init_typedef)(x(Adresses::luax_init_addr));



	*/
	std::uint32_t luax_init_addr = 0x0139A070;
#define luax_init_CCV __cdecl

	/*

	* luaz_openspace_Decompiled_Args: int a1, int a2
	* luaz_openspace_TypeDef: int
	* luaz_openspace_Aob: 55 8B EC 8B 45 10 56 8B 75 C 8B
	* luaz_openspace_ProtectedByRetcheck: false
	* luaz_openspace_IsApartOfLua: true
	* luaz_openspace_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_luaz_openspace_typedef)(int a1, int a2);
	r_luaz_openspace_typedef r_luaz_openspace = (r_luaz_openspace_typedef)(x(0x0139BD80));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(luaz_openspace_CCV* r_luaz_openspace_typedef)(int a1, int a2);
	r_luaz_openspace_typedef r_luaz_openspace = (r_luaz_openspace_typedef)(x(Adresses::luaz_openspace_addr));



	*/
	std::uint32_t luaz_openspace_addr = 0x0139BD80;
#define luaz_openspace_CCV __cdecl

	/*

	* mainposition_Decompiled_Args: int a1, int a2
	* mainposition_TypeDef: int
	* mainposition_Aob: 55 8B EC 83 EC C 56
	* mainposition_ProtectedByRetcheck: false
	* mainposition_IsApartOfLua: true
	* mainposition_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_mainposition_typedef)(int a1, int a2);
	r_mainposition_typedef r_mainposition = (r_mainposition_typedef)(x(0x01398720));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(mainposition_CCV* r_mainposition_typedef)(int a1, int a2);
	r_mainposition_typedef r_mainposition = (r_mainposition_typedef)(x(Adresses::mainposition_addr));



	*/
	std::uint32_t mainposition_addr = 0x01398720;
#define mainposition_CCV __cdecl

	/*

	* markmt_Decompiled_Args: int a1
	* markmt_TypeDef: int
	* markmt_Aob: 55 8B EC 51 53 56 57 8B 7D 8 BB
	* markmt_ProtectedByRetcheck: false
	* markmt_IsApartOfLua: true
	* markmt_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_markmt_typedef)(int a1);
	r_markmt_typedef r_markmt = (r_markmt_typedef)(x(0x01396BD0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(markmt_CCV* r_markmt_typedef)(int a1);
	r_markmt_typedef r_markmt = (r_markmt_typedef)(x(Adresses::markmt_addr));



	*/
	std::uint32_t markmt_addr = 0x01396BD0;
#define markmt_CCV __cdecl

	/*

	* markroot_Decompiled_Args: int a1
	* markroot_TypeDef: int
	* markroot_Aob: 55 8B EC 56 57 8B 7D 8 8B
	* markroot_ProtectedByRetcheck: false
	* markroot_IsApartOfLua: true
	* markroot_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_markroot_typedef)(int a1);
	r_markroot_typedef r_markroot = (r_markroot_typedef)(x(0x01396CC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(markroot_CCV* r_markroot_typedef)(int a1);
	r_markroot_typedef r_markroot = (r_markroot_typedef)(x(Adresses::markroot_addr));



	*/
	std::uint32_t markroot_addr = 0x01396CC0;
#define markroot_CCV __cdecl

	/*

	* match_Decompiled_Args: int a1, int a2, int a3
	* match_TypeDef: int
	* match_Aob: 55 8B EC 83 EC 1C 53 8B 5D 8 56
	* match_ProtectedByRetcheck: false
	* match_IsApartOfLua: true
	* match_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_match_typedef)(int a1, int a2, int a3);
	r_match_typedef r_match = (r_match_typedef)(x(0x01393BE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(match_CCV* r_match_typedef)(int a1, int a2, int a3);
	r_match_typedef r_match = (r_match_typedef)(x(Adresses::match_addr));



	*/
	std::uint32_t match_addr = 0x01393BE0;
#define match_CCV __cdecl

	/*

	* newkey_Decompiled_Args: int a1, int a2, int a3
	* newkey_TypeDef: int
	* newkey_Aob: 55 8B EC 83 EC 10 53 8B 5D C 56
	* newkey_ProtectedByRetcheck: false
	* newkey_IsApartOfLua: true
	* newkey_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_newkey_typedef)(int a1, int a2, int a3);
	r_newkey_typedef r_newkey = (r_newkey_typedef)(x(0x01398850));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(newkey_CCV* r_newkey_typedef)(int a1, int a2, int a3);
	r_newkey_typedef r_newkey = (r_newkey_typedef)(x(Adresses::newkey_addr));



	*/
	std::uint32_t newkey_addr = 0x01398850;
#define newkey_CCV __cdecl

	/*

	* newlstr_Decompiled_Args: int a1, int a2
	* newlstr_TypeDef: int
	* newlstr_Aob: 55 8B EC 83 EC 14 53 56 8B 75 8 57
	* newlstr_ProtectedByRetcheck: false
	* newlstr_IsApartOfLua: true
	* newlstr_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_newlstr_typedef)(int a1, int a2);
	r_newlstr_typedef r_newlstr = (r_newlstr_typedef)(x(0x01397C00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(newlstr_CCV* r_newlstr_typedef)(int a1, int a2);
	r_newlstr_typedef r_newlstr = (r_newlstr_typedef)(x(Adresses::newlstr_addr));



	*/
	std::uint32_t newlstr_addr = 0x01397C00;
#define newlstr_CCV __thiscall

	/*

	* print_Decompiled_Args: int a1
	* print_TypeDef: int
	* print_Aob: 55 8B EC 6A FF 68 D8 95 E 2 64
	* print_ProtectedByRetcheck: false
	* print_IsApartOfLua: false
	* print_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_print_typedef)(int a1);
	r_print_typedef r_print = (r_print_typedef)(x(0x0065C080));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(print_CCV* r_print_typedef)(int a1);
	r_print_typedef r_print = (r_print_typedef)(x(Adresses::print_addr));



	*/
	std::uint32_t print_addr = 0x0065C080;
#define print_CCV __cdecl

	/*

	* push_captures_Decompiled_Args: int a1, int a2, int a3
	* push_captures_TypeDef: int
	* push_captures_Aob: 55 8B EC 53 56 8B 75 8 57
	* push_captures_ProtectedByRetcheck: false
	* push_captures_IsApartOfLua: true
	* push_captures_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_push_captures_typedef)(int a1, int a2, int a3);
	r_push_captures_typedef r_push_captures = (r_push_captures_typedef)(x(0x01394570));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(push_captures_CCV* r_push_captures_typedef)(int a1, int a2, int a3);
	r_push_captures_typedef r_push_captures = (r_push_captures_typedef)(x(Adresses::push_captures_addr));



	*/
	std::uint32_t push_captures_addr = 0x01394570;
#define push_captures_CCV __cdecl

	/*

	* rbx_get_property_Decompiled_Args: int a1, int a2
	* rbx_get_property_TypeDef: int
	* rbx_get_property_Aob: 55 8B EC FF 75 8 E8
	* rbx_get_property_ProtectedByRetcheck: false
	* rbx_get_property_IsApartOfLua: false
	* rbx_get_property_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_get_property_typedef)(int a1, int a2);
	r_rbx_get_property_typedef r_rbx_get_property = (r_rbx_get_property_typedef)(x(0x0195D8E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_get_property_CCV* r_rbx_get_property_typedef)(int a1, int a2);
	r_rbx_get_property_typedef r_rbx_get_property = (r_rbx_get_property_typedef)(x(Adresses::rbx_get_property_addr));



	*/
	std::uint32_t rbx_get_property_addr = 0x0195D8E0;
#define rbx_get_property_CCV __cdecl

	/*

	* rbx_setsimulationradius_Decompiled_Args: int a1, int a2
	* rbx_setsimulationradius_TypeDef: int
	* rbx_setsimulationradius_Aob: 55 8B EC 83 E4 F8 51 56 8B 75 8 85
	* rbx_setsimulationradius_ProtectedByRetcheck: false
	* rbx_setsimulationradius_IsApartOfLua: false
	* rbx_setsimulationradius_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__stdcall* r_rbx_setsimulationradius_typedef)(int a1, int a2);
	r_rbx_setsimulationradius_typedef r_rbx_setsimulationradius = (r_rbx_setsimulationradius_typedef)(x(0x0122AB20));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_setsimulationradius_CCV* r_rbx_setsimulationradius_typedef)(int a1, int a2);
	r_rbx_setsimulationradius_typedef r_rbx_setsimulationradius = (r_rbx_setsimulationradius_typedef)(x(Adresses::rbx_setsimulationradius_addr));



	*/
	std::uint32_t rbx_setsimulationradius_addr = 0x0122AB20;
#define rbx_setsimulationradius_CCV __stdcall

	/*

	* rbx_corescript_getluavmcorescriptfilepath_Decompiled_Args: int a1, int a2, const char* a3
	* rbx_corescript_getluavmcorescriptfilepath_TypeDef: int
	* rbx_corescript_getluavmcorescriptfilepath_Aob: 55 8B EC 6A FF 68 F8 12 15 2 64
	* rbx_corescript_getluavmcorescriptfilepath_ProtectedByRetcheck: false
	* rbx_corescript_getluavmcorescriptfilepath_IsApartOfLua: false
	* rbx_corescript_getluavmcorescriptfilepath_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_corescript_getluavmcorescriptfilepath_typedef)(int a1, int a2, const char* a3);
	r_rbx_corescript_getluavmcorescriptfilepath_typedef r_rbx_corescript_getluavmcorescriptfilepath = (r_rbx_corescript_getluavmcorescriptfilepath_typedef)(x(0x00AC1610));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_corescript_getluavmcorescriptfilepath_CCV* r_rbx_corescript_getluavmcorescriptfilepath_typedef)(int a1, int a2, const char* a3);
	r_rbx_corescript_getluavmcorescriptfilepath_typedef r_rbx_corescript_getluavmcorescriptfilepath = (r_rbx_corescript_getluavmcorescriptfilepath_typedef)(x(Adresses::rbx_corescript_getluavmcorescriptfilepath_addr));



	*/
	std::uint32_t rbx_corescript_getluavmcorescriptfilepath_addr = 0x00AC1610;
#define rbx_corescript_getluavmcorescriptfilepath_CCV __cdecl

	/*

	* rbx_datastore_runtransformfunction_Decompiled_Args: int a1
	* rbx_datastore_runtransformfunction_TypeDef: int
	* rbx_datastore_runtransformfunction_Aob: 55 8B EC 6A FF 68 C3 5E 17 2 64
	* rbx_datastore_runtransformfunction_ProtectedByRetcheck: false
	* rbx_datastore_runtransformfunction_IsApartOfLua: false
	* rbx_datastore_runtransformfunction_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_datastore_runtransformfunction_typedef)(int a1);
	r_rbx_datastore_runtransformfunction_typedef r_rbx_datastore_runtransformfunction = (r_rbx_datastore_runtransformfunction_typedef)(x(0x00CCA520));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_datastore_runtransformfunction_CCV* r_rbx_datastore_runtransformfunction_typedef)(int a1);
	r_rbx_datastore_runtransformfunction_typedef r_rbx_datastore_runtransformfunction = (r_rbx_datastore_runtransformfunction_typedef)(x(Adresses::rbx_datastore_runtransformfunction_addr));



	*/
	std::uint32_t rbx_datastore_runtransformfunction_addr = 0x00CCA520;
#define rbx_datastore_runtransformfunction_CCV __thiscall

	/*

	* rbx_datamodel_dodatamodelsetup_Decompiled_Args: int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9
	* rbx_datamodel_dodatamodelsetup_TypeDef: int
	* rbx_datamodel_dodatamodelsetup_Aob: 55 8B EC 6A FF 68 C1 43 10 2 64
	* rbx_datamodel_dodatamodelsetup_ProtectedByRetcheck: false
	* rbx_datamodel_dodatamodelsetup_IsApartOfLua: false
	* rbx_datamodel_dodatamodelsetup_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_datamodel_dodatamodelsetup_typedef)(int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9);
	r_rbx_datamodel_dodatamodelsetup_typedef r_rbx_datamodel_dodatamodelsetup = (r_rbx_datamodel_dodatamodelsetup_typedef)(x(0x006E79F0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_datamodel_dodatamodelsetup_CCV* r_rbx_datamodel_dodatamodelsetup_typedef)(int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9);
	r_rbx_datamodel_dodatamodelsetup_typedef r_rbx_datamodel_dodatamodelsetup = (r_rbx_datamodel_dodatamodelsetup_typedef)(x(Adresses::rbx_datamodel_dodatamodelsetup_addr));



	*/
	std::uint32_t rbx_datamodel_dodatamodelsetup_addr = 0x006E79F0;
#define rbx_datamodel_dodatamodelsetup_CCV __cdecl

	/*

	* rbx_debugvalueconverte_userdatatostring_Decompiled_Args: int a1, int a2
	* rbx_debugvalueconverte_userdatatostring_TypeDef: int
	* rbx_debugvalueconverte_userdatatostring_Aob: 55 8B EC 6A FF 68 70 48 16 2 64
	* rbx_debugvalueconverte_userdatatostring_ProtectedByRetcheck: false
	* rbx_debugvalueconverte_userdatatostring_IsApartOfLua: false
	* rbx_debugvalueconverte_userdatatostring_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_debugvalueconverte_userdatatostring_typedef)(int a1, int a2);
	r_rbx_debugvalueconverte_userdatatostring_typedef r_rbx_debugvalueconverte_userdatatostring = (r_rbx_debugvalueconverte_userdatatostring_typedef)(x(0x00C08420));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_debugvalueconverte_userdatatostring_CCV* r_rbx_debugvalueconverte_userdatatostring_typedef)(int a1, int a2);
	r_rbx_debugvalueconverte_userdatatostring_typedef r_rbx_debugvalueconverte_userdatatostring = (r_rbx_debugvalueconverte_userdatatostring_typedef)(x(Adresses::rbx_debugvalueconverte_userdatatostring_addr));



	*/
	std::uint32_t rbx_debugvalueconverte_userdatatostring_addr = 0x00C08420;
#define rbx_debugvalueconverte_userdatatostring_CCV __cdecl

	/*

	* rbx_deu_uploadcrasheventfile_Decompiled_Args: int a1, int a2
	* rbx_deu_uploadcrasheventfile_TypeDef: int
	* rbx_deu_uploadcrasheventfile_Aob: 55 8B EC 6A FF 68 75 44 F 2 64
	* rbx_deu_uploadcrasheventfile_ProtectedByRetcheck: false
	* rbx_deu_uploadcrasheventfile_IsApartOfLua: false
	* rbx_deu_uploadcrasheventfile_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_deu_uploadcrasheventfile_typedef)(int a1, int a2);
	r_rbx_deu_uploadcrasheventfile_typedef r_rbx_deu_uploadcrasheventfile = (r_rbx_deu_uploadcrasheventfile_typedef)(x(0x006282B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_deu_uploadcrasheventfile_CCV* r_rbx_deu_uploadcrasheventfile_typedef)(int a1, int a2);
	r_rbx_deu_uploadcrasheventfile_typedef r_rbx_deu_uploadcrasheventfile = (r_rbx_deu_uploadcrasheventfile_typedef)(x(Adresses::rbx_deu_uploadcrasheventfile_addr));



	*/
	std::uint32_t rbx_deu_uploadcrasheventfile_addr = 0x006282B0;
#define rbx_deu_uploadcrasheventfile_CCV __cdecl

	/*

	* rbx_lm_reporterror_Decompiled_Args: int a1
	* rbx_lm_reporterror_TypeDef: int
	* rbx_lm_reporterror_Aob: 55 8B EC F B7
	* rbx_lm_reporterror_ProtectedByRetcheck: false
	* rbx_lm_reporterror_IsApartOfLua: false
	* rbx_lm_reporterror_Mask: .....
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_lm_reporterror_typedef)(int a1);
	r_rbx_lm_reporterror_typedef r_rbx_lm_reporterror = (r_rbx_lm_reporterror_typedef)(x(0x0062CCA0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_lm_reporterror_CCV* r_rbx_lm_reporterror_typedef)(int a1);
	r_rbx_lm_reporterror_typedef r_rbx_lm_reporterror = (r_rbx_lm_reporterror_typedef)(x(Adresses::rbx_lm_reporterror_addr));



	*/
	std::uint32_t rbx_lm_reporterror_addr = 0x0062CCA0;
#define rbx_lm_reporterror_CCV __cdecl

	/*

	* rbx_lua_dumpthreadrefcounts_Decompiled_Args: int
	* rbx_lua_dumpthreadrefcounts_TypeDef: int
	* rbx_lua_dumpthreadrefcounts_Aob: 55 8B EC 51 80 3D D4 9F B2 2 0 74
	* rbx_lua_dumpthreadrefcounts_ProtectedByRetcheck: false
	* rbx_lua_dumpthreadrefcounts_IsApartOfLua: false
	* rbx_lua_dumpthreadrefcounts_Mask: ............
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_lua_dumpthreadrefcounts_typedef)(int);
	r_rbx_lua_dumpthreadrefcounts_typedef r_rbx_lua_dumpthreadrefcounts = (r_rbx_lua_dumpthreadrefcounts_typedef)(x(0x00831CA0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_lua_dumpthreadrefcounts_CCV* r_rbx_lua_dumpthreadrefcounts_typedef)(int);
	r_rbx_lua_dumpthreadrefcounts_typedef r_rbx_lua_dumpthreadrefcounts = (r_rbx_lua_dumpthreadrefcounts_typedef)(x(Adresses::rbx_lua_dumpthreadrefcounts_addr));



	*/
	std::uint32_t rbx_lua_dumpthreadrefcounts_addr = 0x00831CA0;
#define rbx_lua_dumpthreadrefcounts_CCV __cdecl

	/*

	* rbx_network_replicator_processdeserializedpacket_Decompiled_Args: int a1
	* rbx_network_replicator_processdeserializedpacket_TypeDef: int
	* rbx_network_replicator_processdeserializedpacket_Aob: 55 8B EC 6A FF 68 7B 49 1B 2 64
	* rbx_network_replicator_processdeserializedpacket_ProtectedByRetcheck: false
	* rbx_network_replicator_processdeserializedpacket_IsApartOfLua: false
	* rbx_network_replicator_processdeserializedpacket_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_network_replicator_processdeserializedpacket_typedef)(int a1);
	r_rbx_network_replicator_processdeserializedpacket_typedef r_rbx_network_replicator_processdeserializedpacket = (r_rbx_network_replicator_processdeserializedpacket_typedef)(x(0x0116D4B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_network_replicator_processdeserializedpacket_CCV* r_rbx_network_replicator_processdeserializedpacket_typedef)(int a1);
	r_rbx_network_replicator_processdeserializedpacket_typedef r_rbx_network_replicator_processdeserializedpacket = (r_rbx_network_replicator_processdeserializedpacket_typedef)(x(Adresses::rbx_network_replicator_processdeserializedpacket_addr));



	*/
	std::uint32_t rbx_network_replicator_processdeserializedpacket_addr = 0x0116D4B0;
#define rbx_network_replicator_processdeserializedpacket_CCV __thiscall

	/*

	* rbx_network_server_registerlegalscript_Decompiled_Args: int a1
	* rbx_network_server_registerlegalscript_TypeDef: int
	* rbx_network_server_registerlegalscript_Aob: 55 8B EC 6A FF 68 D0 71 1B 2 64
	* rbx_network_server_registerlegalscript_ProtectedByRetcheck: false
	* rbx_network_server_registerlegalscript_IsApartOfLua: false
	* rbx_network_server_registerlegalscript_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_network_server_registerlegalscript_typedef)(int a1);
	r_rbx_network_server_registerlegalscript_typedef r_rbx_network_server_registerlegalscript = (r_rbx_network_server_registerlegalscript_typedef)(x(0x01198070));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_network_server_registerlegalscript_CCV* r_rbx_network_server_registerlegalscript_typedef)(int a1);
	r_rbx_network_server_registerlegalscript_typedef r_rbx_network_server_registerlegalscript = (r_rbx_network_server_registerlegalscript_typedef)(x(Adresses::rbx_network_server_registerlegalscript_addr));



	*/
	std::uint32_t rbx_network_server_registerlegalscript_addr = 0x01198070;
#define rbx_network_server_registerlegalscript_CCV __thiscall

	/*

	* rbx_rcr_processexception_Decompiled_Args: int a1
	* rbx_rcr_processexception_TypeDef: int
	* rbx_rcr_processexception_Aob: 55 8B EC 53 56 68 BC BE 52 2 6A
	* rbx_rcr_processexception_ProtectedByRetcheck: false
	* rbx_rcr_processexception_IsApartOfLua: false
	* rbx_rcr_processexception_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_rcr_processexception_typedef)(int a1);
	r_rbx_rcr_processexception_typedef r_rbx_rcr_processexception = (r_rbx_rcr_processexception_typedef)(x(0x0062CB50));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_rcr_processexception_CCV* r_rbx_rcr_processexception_typedef)(int a1);
	r_rbx_rcr_processexception_typedef r_rbx_rcr_processexception = (r_rbx_rcr_processexception_typedef)(x(Adresses::rbx_rcr_processexception_addr));



	*/
	std::uint32_t rbx_rcr_processexception_addr = 0x0062CB50;
#define rbx_rcr_processexception_CCV __thiscall

	/*

	* rbx_sc_closestate_Decompiled_Args: int a1
	* rbx_sc_closestate_TypeDef: int
	* rbx_sc_closestate_Aob: 55 8B EC 6A FF 68 F0 14 11 2 64
	* rbx_sc_closestate_ProtectedByRetcheck: false
	* rbx_sc_closestate_IsApartOfLua: false
	* rbx_sc_closestate_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_sc_closestate_typedef)(int a1);
	r_rbx_sc_closestate_typedef r_rbx_sc_closestate = (r_rbx_sc_closestate_typedef)(x(0x00784C10));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_sc_closestate_CCV* r_rbx_sc_closestate_typedef)(int a1);
	r_rbx_sc_closestate_typedef r_rbx_sc_closestate = (r_rbx_sc_closestate_typedef)(x(Adresses::rbx_sc_closestate_addr));



	*/
	std::uint32_t rbx_sc_closestate_addr = 0x00784C10;
#define rbx_sc_closestate_CCV __cdecl

	/*

	* rbx_lua_bridge_on_index_Decompiled_Args: int a1, int a2
	* rbx_lua_bridge_on_index_TypeDef: int
	* rbx_lua_bridge_on_index_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* rbx_lua_bridge_on_index_ProtectedByRetcheck: false
	* rbx_lua_bridge_on_index_IsApartOfLua: false
	* rbx_lua_bridge_on_index_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_rbx_lua_bridge_on_index_typedef)(int a1, int a2);
	r_rbx_lua_bridge_on_index_typedef r_rbx_lua_bridge_on_index = (r_rbx_lua_bridge_on_index_typedef)(x(0x0084DE40));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_lua_bridge_on_index_CCV* r_rbx_lua_bridge_on_index_typedef)(int a1, int a2);
	r_rbx_lua_bridge_on_index_typedef r_rbx_lua_bridge_on_index = (r_rbx_lua_bridge_on_index_typedef)(x(Adresses::rbx_lua_bridge_on_index_addr));



	*/
	std::uint32_t rbx_lua_bridge_on_index_addr = 0x0084DE40;
#define rbx_lua_bridge_on_index_CCV __fastcall

	/*

	* rbx_security_context_current_Decompiled_Args: std::uint32_t Rbx_LuaState
	* rbx_security_context_current_TypeDef: int
	* rbx_security_context_current_Aob: 8B D 84 FE F0
	* rbx_security_context_current_ProtectedByRetcheck: false
	* rbx_security_context_current_IsApartOfLua: true
	* rbx_security_context_current_Mask: .....
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_security_context_current_typedef)(std::uint32_t Rbx_LuaState);
	r_rbx_security_context_current_typedef r_rbx_security_context_current = (r_rbx_security_context_current_typedef)(x(0x0137FF70));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_security_context_current_CCV* r_rbx_security_context_current_typedef)(std::uint32_t Rbx_LuaState);
	r_rbx_security_context_current_typedef r_rbx_security_context_current = (r_rbx_security_context_current_typedef)(x(Adresses::rbx_security_context_current_addr));



	*/
	std::uint32_t rbx_security_context_current_addr = 0x0137FF70;
#define rbx_security_context_current_CCV __cdecl

	/*

	* rbx_sc_crash_Decompiled_Args: int a1
	* rbx_sc_crash_TypeDef: int
	* rbx_sc_crash_Aob: 55 8B EC 51 56 57 8B 7D 8 BE
	* rbx_sc_crash_ProtectedByRetcheck: false
	* rbx_sc_crash_IsApartOfLua: false
	* rbx_sc_crash_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_sc_crash_typedef)(int a1);
	r_rbx_sc_crash_typedef r_rbx_sc_crash = (r_rbx_sc_crash_typedef)(x(0x007856B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_sc_crash_CCV* r_rbx_sc_crash_typedef)(int a1);
	r_rbx_sc_crash_typedef r_rbx_sc_crash = (r_rbx_sc_crash_typedef)(x(Adresses::rbx_sc_crash_addr));



	*/
	std::uint32_t rbx_sc_crash_addr = 0x007856B0;
#define rbx_sc_crash_CCV __cdecl

	/*

	* rbx_sc_interrupt_Decompiled_Args: int a1
	* rbx_sc_interrupt_TypeDef: int
	* rbx_sc_interrupt_Aob: 55 8B EC 83 EC 10 56 8B 75 8 57
	* rbx_sc_interrupt_ProtectedByRetcheck: false
	* rbx_sc_interrupt_IsApartOfLua: false
	* rbx_sc_interrupt_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_sc_interrupt_typedef)(int a1);
	r_rbx_sc_interrupt_typedef r_rbx_sc_interrupt = (r_rbx_sc_interrupt_typedef)(x(0x0078A760));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_sc_interrupt_CCV* r_rbx_sc_interrupt_typedef)(int a1);
	r_rbx_sc_interrupt_typedef r_rbx_sc_interrupt = (r_rbx_sc_interrupt_typedef)(x(Adresses::rbx_sc_interrupt_addr));



	*/
	std::uint32_t rbx_sc_interrupt_addr = 0x0078A760;
#define rbx_sc_interrupt_CCV __cdecl

	/*

	* rbx_sc_loadstring_Decompiled_Args: int a1
	* rbx_sc_loadstring_TypeDef: int
	* rbx_sc_loadstring_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* rbx_sc_loadstring_ProtectedByRetcheck: false
	* rbx_sc_loadstring_IsApartOfLua: false
	* rbx_sc_loadstring_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_sc_loadstring_typedef)(int a1);
	r_rbx_sc_loadstring_typedef r_rbx_sc_loadstring = (r_rbx_sc_loadstring_typedef)(x(0x0078B050));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_sc_loadstring_CCV* r_rbx_sc_loadstring_typedef)(int a1);
	r_rbx_sc_loadstring_typedef r_rbx_sc_loadstring = (r_rbx_sc_loadstring_typedef)(x(Adresses::rbx_sc_loadstring_addr));



	*/
	std::uint32_t rbx_sc_loadstring_addr = 0x0078B050;
#define rbx_sc_loadstring_CCV __cdecl

	/*

	* rbx_sc_scriptcontext_Decompiled_Args: int a1
	* rbx_sc_scriptcontext_TypeDef: int
	* rbx_sc_scriptcontext_Aob: 55 8B EC 6A FF 68 D1 D 11 2 64
	* rbx_sc_scriptcontext_ProtectedByRetcheck: false
	* rbx_sc_scriptcontext_IsApartOfLua: false
	* rbx_sc_scriptcontext_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_sc_scriptcontext_typedef)(int a1);
	r_rbx_sc_scriptcontext_typedef r_rbx_sc_scriptcontext = (r_rbx_sc_scriptcontext_typedef)(x(0x0077E030));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_sc_scriptcontext_CCV* r_rbx_sc_scriptcontext_typedef)(int a1);
	r_rbx_sc_scriptcontext_typedef r_rbx_sc_scriptcontext = (r_rbx_sc_scriptcontext_typedef)(x(Adresses::rbx_sc_scriptcontext_addr));



	*/
	std::uint32_t rbx_sc_scriptcontext_addr = 0x0077E030;
#define rbx_sc_scriptcontext_CCV __thiscall

	/*

	* rbx_sc_startscript_Decompiled_Args: int a1
	* rbx_sc_startscript_TypeDef: int
	* rbx_sc_startscript_Aob: 55 8B EC 6A FF 68 30 28 11 2 64
	* rbx_sc_startscript_ProtectedByRetcheck: false
	* rbx_sc_startscript_IsApartOfLua: false
	* rbx_sc_startscript_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_sc_startscript_typedef)(int a1);
	r_rbx_sc_startscript_typedef r_rbx_sc_startscript = (r_rbx_sc_startscript_typedef)(x(0x00796200));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_sc_startscript_CCV* r_rbx_sc_startscript_typedef)(int a1);
	r_rbx_sc_startscript_typedef r_rbx_sc_startscript = (r_rbx_sc_startscript_typedef)(x(Adresses::rbx_sc_startscript_addr));



	*/
	std::uint32_t rbx_sc_startscript_addr = 0x00796200;
#define rbx_sc_startscript_CCV __thiscall

	/*

	* rbx_sc_watchdog_Decompiled_Args: int a1
	* rbx_sc_watchdog_TypeDef: int
	* rbx_sc_watchdog_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* rbx_sc_watchdog_ProtectedByRetcheck: false
	* rbx_sc_watchdog_IsApartOfLua: false
	* rbx_sc_watchdog_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_sc_watchdog_typedef)(int a1);
	r_rbx_sc_watchdog_typedef r_rbx_sc_watchdog = (r_rbx_sc_watchdog_typedef)(x(0x00797D30));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_sc_watchdog_CCV* r_rbx_sc_watchdog_typedef)(int a1);
	r_rbx_sc_watchdog_typedef r_rbx_sc_watchdog = (r_rbx_sc_watchdog_typedef)(x(Adresses::rbx_sc_watchdog_addr));



	*/
	std::uint32_t rbx_sc_watchdog_addr = 0x00797D30;
#define rbx_sc_watchdog_CCV __thiscall

	/*

	* rbx_scriptcontext_getcontext_Decompiled_Args: int a1
	* rbx_scriptcontext_getcontext_TypeDef: int
	* rbx_scriptcontext_getcontext_Aob: 55 8B EC 8B 45 8 85
	* rbx_scriptcontext_getcontext_ProtectedByRetcheck: false
	* rbx_scriptcontext_getcontext_IsApartOfLua: false
	* rbx_scriptcontext_getcontext_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_scriptcontext_getcontext_typedef)(int a1);
	r_rbx_scriptcontext_getcontext_typedef r_rbx_scriptcontext_getcontext = (r_rbx_scriptcontext_getcontext_typedef)(x(0x00788D70));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_scriptcontext_getcontext_CCV* r_rbx_scriptcontext_getcontext_typedef)(int a1);
	r_rbx_scriptcontext_getcontext_typedef r_rbx_scriptcontext_getcontext = (r_rbx_scriptcontext_getcontext_typedef)(x(Adresses::rbx_scriptcontext_getcontext_addr));



	*/
	std::uint32_t rbx_scriptcontext_getcontext_addr = 0x00788D70;
#define rbx_scriptcontext_getcontext_CCV __cdecl

	/*

	* rbx_scriptcontext_getglobalstate_Decompiled_Args: int a1
	* rbx_scriptcontext_getglobalstate_TypeDef: int
	* rbx_scriptcontext_getglobalstate_Aob: 55 8B EC 8B 4D 8 83
	* rbx_scriptcontext_getglobalstate_ProtectedByRetcheck: false
	* rbx_scriptcontext_getglobalstate_IsApartOfLua: false
	* rbx_scriptcontext_getglobalstate_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_scriptcontext_getglobalstate_typedef)(int a1);
	r_rbx_scriptcontext_getglobalstate_typedef r_rbx_scriptcontext_getglobalstate = (r_rbx_scriptcontext_getglobalstate_typedef)(x(0x00788FE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_scriptcontext_getglobalstate_CCV* r_rbx_scriptcontext_getglobalstate_typedef)(int a1);
	r_rbx_scriptcontext_getglobalstate_typedef r_rbx_scriptcontext_getglobalstate = (r_rbx_scriptcontext_getglobalstate_typedef)(x(Adresses::rbx_scriptcontext_getglobalstate_addr));



	*/
	std::uint32_t rbx_scriptcontext_getglobalstate_addr = 0x00788FE0;
#define rbx_scriptcontext_getglobalstate_CCV __cdecl

	/*

	* rbx_modulescript_vmstate_Decompiled_Args: int a1
	* rbx_modulescript_vmstate_TypeDef: int
	* rbx_modulescript_vmstate_Aob: 55 8B EC 8D 45 8 81
	* rbx_modulescript_vmstate_ProtectedByRetcheck: false
	* rbx_modulescript_vmstate_IsApartOfLua: false
	* rbx_modulescript_vmstate_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__stdcall* r_rbx_modulescript_vmstate_typedef)(int a1);
	r_rbx_modulescript_vmstate_typedef r_rbx_modulescript_vmstate = (r_rbx_modulescript_vmstate_typedef)(x(0x00C183B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_modulescript_vmstate_CCV* r_rbx_modulescript_vmstate_typedef)(int a1);
	r_rbx_modulescript_vmstate_typedef r_rbx_modulescript_vmstate = (r_rbx_modulescript_vmstate_typedef)(x(Adresses::rbx_modulescript_vmstate_addr));



	*/
	std::uint32_t rbx_modulescript_vmstate_addr = 0x00C183B0;
#define rbx_modulescript_vmstate_CCV __stdcall

	/*

	* rbx_modulescript_pervmstate_setcompletedsuccess_Decompiled_Args: int a1, int a2
	* rbx_modulescript_pervmstate_setcompletedsuccess_TypeDef: int
	* rbx_modulescript_pervmstate_setcompletedsuccess_Aob: 55 8B EC 83 79 14 0 C7
	* rbx_modulescript_pervmstate_setcompletedsuccess_ProtectedByRetcheck: false
	* rbx_modulescript_pervmstate_setcompletedsuccess_IsApartOfLua: false
	* rbx_modulescript_pervmstate_setcompletedsuccess_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__stdcall* r_rbx_modulescript_pervmstate_setcompletedsuccess_typedef)(int a1, int a2);
	r_rbx_modulescript_pervmstate_setcompletedsuccess_typedef r_rbx_modulescript_pervmstate_setcompletedsuccess = (r_rbx_modulescript_pervmstate_setcompletedsuccess_typedef)(x(0x00C18080));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_modulescript_pervmstate_setcompletedsuccess_CCV* r_rbx_modulescript_pervmstate_setcompletedsuccess_typedef)(int a1, int a2);
	r_rbx_modulescript_pervmstate_setcompletedsuccess_typedef r_rbx_modulescript_pervmstate_setcompletedsuccess = (r_rbx_modulescript_pervmstate_setcompletedsuccess_typedef)(x(Adresses::rbx_modulescript_pervmstate_setcompletedsuccess_addr));



	*/
	std::uint32_t rbx_modulescript_pervmstate_setcompletedsuccess_addr = 0x00C18080;
#define rbx_modulescript_pervmstate_setcompletedsuccess_CCV __stdcall

	/*

	* rbx_modulescript_pervmstate_getandclearyieldedimporters_Decompiled_Args: int a1, int a2
	* rbx_modulescript_pervmstate_getandclearyieldedimporters_TypeDef: int
	* rbx_modulescript_pervmstate_getandclearyieldedimporters_Aob: 55 8B EC 8B 55 8 53
	* rbx_modulescript_pervmstate_getandclearyieldedimporters_ProtectedByRetcheck: false
	* rbx_modulescript_pervmstate_getandclearyieldedimporters_IsApartOfLua: false
	* rbx_modulescript_pervmstate_getandclearyieldedimporters_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_modulescript_pervmstate_getandclearyieldedimporters_typedef)(int a1, int a2);
	r_rbx_modulescript_pervmstate_getandclearyieldedimporters_typedef r_rbx_modulescript_pervmstate_getandclearyieldedimporters = (r_rbx_modulescript_pervmstate_getandclearyieldedimporters_typedef)(x(0x00C17DE0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_modulescript_pervmstate_getandclearyieldedimporters_CCV* r_rbx_modulescript_pervmstate_getandclearyieldedimporters_typedef)(int a1, int a2);
	r_rbx_modulescript_pervmstate_getandclearyieldedimporters_typedef r_rbx_modulescript_pervmstate_getandclearyieldedimporters = (r_rbx_modulescript_pervmstate_getandclearyieldedimporters_typedef)(x(Adresses::rbx_modulescript_pervmstate_getandclearyieldedimporters_addr));



	*/
	std::uint32_t rbx_modulescript_pervmstate_getandclearyieldedimporters_addr = 0x00C17DE0;
#define rbx_modulescript_pervmstate_getandclearyieldedimporters_CCV __thiscall

	/*

	* rbx_scriptcontext_requiremodulescriptsuccesscontinuation_Decompiled_Args: int a1, int a2, int a3
	* rbx_scriptcontext_requiremodulescriptsuccesscontinuation_TypeDef: int
	* rbx_scriptcontext_requiremodulescriptsuccesscontinuation_Aob: 55 8B EC 6A FF 68 40 24 11 2 64
	* rbx_scriptcontext_requiremodulescriptsuccesscontinuation_ProtectedByRetcheck: false
	* rbx_scriptcontext_requiremodulescriptsuccesscontinuation_IsApartOfLua: false
	* rbx_scriptcontext_requiremodulescriptsuccesscontinuation_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation_typedef)(int a1, int a2, int a3);
	r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation_typedef r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation = (r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation_typedef)(x(0x00793210));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_scriptcontext_requiremodulescriptsuccesscontinuation_CCV* r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation_typedef)(int a1, int a2, int a3);
	r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation_typedef r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation = (r_rbx_scriptcontext_requiremodulescriptsuccesscontinuation_typedef)(x(Adresses::rbx_scriptcontext_requiremodulescriptsuccesscontinuation_addr));



	*/
	std::uint32_t rbx_scriptcontext_requiremodulescriptsuccesscontinuation_addr = 0x00793210;
#define rbx_scriptcontext_requiremodulescriptsuccesscontinuation_CCV __cdecl

	/*

	* rbx_setthreadname_Decompiled_Args: int
	* rbx_setthreadname_TypeDef: int
	* rbx_setthreadname_Aob: 55 8B EC 80 3D F4 7A B3 2 0 74
	* rbx_setthreadname_ProtectedByRetcheck: false
	* rbx_setthreadname_IsApartOfLua: false
	* rbx_setthreadname_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_setthreadname_typedef)(int);
	r_rbx_setthreadname_typedef r_rbx_setthreadname = (r_rbx_setthreadname_typedef)(x(0x00FF4150));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_setthreadname_CCV* r_rbx_setthreadname_typedef)(int);
	r_rbx_setthreadname_typedef r_rbx_setthreadname = (r_rbx_setthreadname_typedef)(x(Adresses::rbx_setthreadname_addr));



	*/
	std::uint32_t rbx_setthreadname_addr = 0x00FF4150;
#define rbx_setthreadname_CCV __cdecl

	/*

	* rbx_serializer_loadinstances_Decompiled_Args: int
	* rbx_serializer_loadinstances_TypeDef: int
	* rbx_serializer_loadinstances_Aob: 55 8B EC 6A 0 FF 75
	* rbx_serializer_loadinstances_ProtectedByRetcheck: false
	* rbx_serializer_loadinstances_IsApartOfLua: false
	* rbx_serializer_loadinstances_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_serializer_loadinstances_typedef)(int);
	r_rbx_serializer_loadinstances_typedef r_rbx_serializer_loadinstances = (r_rbx_serializer_loadinstances_typedef)(x(0x00860770));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_serializer_loadinstances_CCV* r_rbx_serializer_loadinstances_typedef)(int);
	r_rbx_serializer_loadinstances_typedef r_rbx_serializer_loadinstances = (r_rbx_serializer_loadinstances_typedef)(x(Adresses::rbx_serializer_loadinstances_addr));



	*/
	std::uint32_t rbx_serializer_loadinstances_addr = 0x00860770;
#define rbx_serializer_loadinstances_CCV __cdecl

	/*

	* rbx_serializer_loadinstances_long_Decompiled_Args: int a1, int a2
	* rbx_serializer_loadinstances_long_TypeDef: int
	* rbx_serializer_loadinstances_long_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* rbx_serializer_loadinstances_long_ProtectedByRetcheck: false
	* rbx_serializer_loadinstances_long_IsApartOfLua: false
	* rbx_serializer_loadinstances_long_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbx_serializer_loadinstances_long_typedef)(int a1, int a2);
	r_rbx_serializer_loadinstances_long_typedef r_rbx_serializer_loadinstances_long = (r_rbx_serializer_loadinstances_long_typedef)(x(0x00860790));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_serializer_loadinstances_long_CCV* r_rbx_serializer_loadinstances_long_typedef)(int a1, int a2);
	r_rbx_serializer_loadinstances_long_typedef r_rbx_serializer_loadinstances_long = (r_rbx_serializer_loadinstances_long_typedef)(x(Adresses::rbx_serializer_loadinstances_long_addr));



	*/
	std::uint32_t rbx_serializer_loadinstances_long_addr = 0x00860790;
#define rbx_serializer_loadinstances_long_CCV __cdecl

	/*

	* rbx_standardout_printwithlink_Decompiled_Args: int a1, int a2, int a3, int a4, int a5
	* rbx_standardout_printwithlink_TypeDef: int
	* rbx_standardout_printwithlink_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* rbx_standardout_printwithlink_ProtectedByRetcheck: false
	* rbx_standardout_printwithlink_IsApartOfLua: false
	* rbx_standardout_printwithlink_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_rbx_standardout_printwithlink_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_rbx_standardout_printwithlink_typedef r_rbx_standardout_printwithlink = (r_rbx_standardout_printwithlink_typedef)(x(0x0065BD60));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_standardout_printwithlink_CCV* r_rbx_standardout_printwithlink_typedef)(int a1, int a2, int a3, int a4, int a5);
	r_rbx_standardout_printwithlink_typedef r_rbx_standardout_printwithlink = (r_rbx_standardout_printwithlink_typedef)(x(Adresses::rbx_standardout_printwithlink_addr));



	*/
	std::uint32_t rbx_standardout_printwithlink_addr = 0x0065BD60;
#define rbx_standardout_printwithlink_CCV __fastcall

	/*

	* rbx_ts_startscript_Decompiled_Args: int a1
	* rbx_ts_startscript_TypeDef: int
	* rbx_ts_startscript_Aob: 55 8B EC 6A FF 68 36 82 15 2 64
	* rbx_ts_startscript_ProtectedByRetcheck: false
	* rbx_ts_startscript_IsApartOfLua: false
	* rbx_ts_startscript_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_ts_startscript_typedef)(int a1);
	r_rbx_ts_startscript_typedef r_rbx_ts_startscript = (r_rbx_ts_startscript_typedef)(x(0x00B0ABA0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_ts_startscript_CCV* r_rbx_ts_startscript_typedef)(int a1);
	r_rbx_ts_startscript_typedef r_rbx_ts_startscript = (r_rbx_ts_startscript_typedef)(x(Adresses::rbx_ts_startscript_addr));



	*/
	std::uint32_t rbx_ts_startscript_addr = 0x00B0ABA0;
#define rbx_ts_startscript_CCV __thiscall

	/*

	* rbx_ws_getrealphysicsfps_Decompiled_Args: int a1
	* rbx_ws_getrealphysicsfps_TypeDef: int
	* rbx_ws_getrealphysicsfps_Aob: 55 8B EC 6A FF 68 70 E5 12 2 64
	* rbx_ws_getrealphysicsfps_ProtectedByRetcheck: false
	* rbx_ws_getrealphysicsfps_IsApartOfLua: false
	* rbx_ws_getrealphysicsfps_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_rbx_ws_getrealphysicsfps_typedef)(int a1);
	r_rbx_ws_getrealphysicsfps_typedef r_rbx_ws_getrealphysicsfps = (r_rbx_ws_getrealphysicsfps_typedef)(x(0x00913260));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbx_ws_getrealphysicsfps_CCV* r_rbx_ws_getrealphysicsfps_typedef)(int a1);
	r_rbx_ws_getrealphysicsfps_typedef r_rbx_ws_getrealphysicsfps = (r_rbx_ws_getrealphysicsfps_typedef)(x(Adresses::rbx_ws_getrealphysicsfps_addr));



	*/
	std::uint32_t rbx_ws_getrealphysicsfps_addr = 0x00913260;
#define rbx_ws_getrealphysicsfps_CCV __thiscall

	/*

	* rbxcrash_Decompiled_Args: int a1, int a2
	* rbxcrash_TypeDef: int
	* rbxcrash_Aob: 55 8B EC 51 A1 10 D0 F0 2 56
	* rbxcrash_ProtectedByRetcheck: false
	* rbxcrash_IsApartOfLua: false
	* rbxcrash_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rbxcrash_typedef)(int a1, int a2);
	r_rbxcrash_typedef r_rbxcrash = (r_rbxcrash_typedef)(x(0x015802A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rbxcrash_CCV* r_rbxcrash_typedef)(int a1, int a2);
	r_rbxcrash_typedef r_rbxcrash = (r_rbxcrash_typedef)(x(Adresses::rbxcrash_addr));



	*/
	std::uint32_t rbxcrash_addr = 0x015802A0;
#define rbxcrash_CCV __cdecl

	/*

	* reallymarkobject_Decompiled_Args: int a1, int a2
	* reallymarkobject_TypeDef: int
	* reallymarkobject_Aob: 55 8B EC 56 8B 75 C F B6
	* reallymarkobject_ProtectedByRetcheck: false
	* reallymarkobject_IsApartOfLua: true
	* reallymarkobject_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_reallymarkobject_typedef)(int a1, int a2);
	r_reallymarkobject_typedef r_reallymarkobject = (r_reallymarkobject_typedef)(x(0x013973C0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(reallymarkobject_CCV* r_reallymarkobject_typedef)(int a1, int a2);
	r_reallymarkobject_typedef r_reallymarkobject = (r_reallymarkobject_typedef)(x(Adresses::reallymarkobject_addr));



	*/
	std::uint32_t reallymarkobject_addr = 0x013973C0;
#define reallymarkobject_CCV __cdecl

	/*

	* rehash_Decompiled_Args: std::uint32_t Rbx_LuaState
	* rehash_TypeDef: int
	* rehash_Aob: 53 8B DC 83 EC 8 83
	* rehash_ProtectedByRetcheck: false
	* rehash_IsApartOfLua: true
	* rehash_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_rehash_typedef)(std::uint32_t Rbx_LuaState);
	r_rehash_typedef r_rehash = (r_rehash_typedef)(x(0x01398A00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(rehash_CCV* r_rehash_typedef)(std::uint32_t Rbx_LuaState);
	r_rehash_typedef r_rehash = (r_rehash_typedef)(x(Adresses::rehash_addr));



	*/
	std::uint32_t rehash_addr = 0x01398A00;
#define rehash_CCV __cdecl

	/*

	* reportError_Decompiled_Args: const char* a1
	* reportError_TypeDef: int
	* reportError_Aob: 55 8B EC 56 6A 0 6A FF
	* reportError_ProtectedByRetcheck: false
	* reportError_IsApartOfLua: false
	* reportError_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__stdcall* r_reportError_typedef)(const char* a1);
	r_reportError_typedef r_reportError = (r_reportError_typedef)(x(0x0078A080));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(reportError_CCV* r_reportError_typedef)(const char* a1);
	r_reportError_typedef r_reportError = (r_reportError_typedef)(x(Adresses::reportError_addr));



	*/
	std::uint32_t reportError_addr = 0x0078A080;
#define reportError_CCV __stdcall

	/*

	* resize_Decompiled_Args: int a1, int a2, int a3, int a4
	* resize_TypeDef: int
	* resize_Aob: 55 8B EC 83 EC 38 56 8B 75 C 8B
	* resize_ProtectedByRetcheck: false
	* resize_IsApartOfLua: true
	* resize_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_resize_typedef)(int a1, int a2, int a3, int a4);
	r_resize_typedef r_resize = (r_resize_typedef)(x(0x01398C80));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(resize_CCV* r_resize_typedef)(int a1, int a2, int a3, int a4);
	r_resize_typedef r_resize = (r_resize_typedef)(x(Adresses::resize_addr));



	*/
	std::uint32_t resize_addr = 0x01398C80;
#define resize_CCV __cdecl

	/*

	* resume_Decompiled_Args: int a1, int a2
	* resume_TypeDef: int
	* resume_Aob: 55 8B EC 56 8B 75 8 80
	* resume_ProtectedByRetcheck: false
	* resume_IsApartOfLua: true
	* resume_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_resume_typedef)(int a1, int a2);
	r_resume_typedef r_resume = (r_resume_typedef)(x(0x01385130));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(resume_CCV* r_resume_typedef)(int a1, int a2);
	r_resume_typedef r_resume = (r_resume_typedef)(x(Adresses::resume_addr));



	*/
	std::uint32_t resume_addr = 0x01385130;
#define resume_CCV __cdecl

	/*

	* resume_error_Decompiled_Args: int a1, const char* a2
	* resume_error_TypeDef: int
	* resume_error_Aob: 55 8B EC 8B 55 C 8B
	* resume_error_ProtectedByRetcheck: true
	* resume_error_IsApartOfLua: true
	* resume_error_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_resume_error_typedef)(int a1, const char* a2);
	r_resume_error_typedef r_resume_error = (r_resume_error_typedef)unprotect(x(0x01385210));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(resume_error_CCV* r_resume_error_typedef)(int a1, const char* a2);
	r_resume_error_typedef r_resume_error = (r_resume_error_typedef)unprotect(x(Adresses::resume_error_addr));



	*/
	std::uint32_t resume_error_addr = 0x01385210;
#define resume_error_CCV __cdecl

	/*

	* retcheck_Decompiled_Args: int a1, int a2
	* retcheck_TypeDef: int
	* retcheck_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* retcheck_ProtectedByRetcheck: false
	* retcheck_IsApartOfLua: false
	* retcheck_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__fastcall* r_retcheck_typedef)(int a1, int a2);
	r_retcheck_typedef r_retcheck = (r_retcheck_typedef)(x(0x006C6430));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(retcheck_CCV* r_retcheck_typedef)(int a1, int a2);
	r_retcheck_typedef r_retcheck = (r_retcheck_typedef)(x(Adresses::retcheck_addr));



	*/
	std::uint32_t retcheck_addr = 0x006C6430;
#define retcheck_CCV __fastcall

	/*

	* robloxextraspace__robloxextraspace_Decompiled_Args: int a1
	* robloxextraspace__robloxextraspace_TypeDef: int
	* robloxextraspace__robloxextraspace_Aob: 55 8B EC 56 8B F1 8B 46 C 8B
	* robloxextraspace__robloxextraspace_ProtectedByRetcheck: false
	* robloxextraspace__robloxextraspace_IsApartOfLua: false
	* robloxextraspace__robloxextraspace_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__thiscall* r_robloxextraspace__robloxextraspace_typedef)(int a1);
	r_robloxextraspace__robloxextraspace_typedef r_robloxextraspace__robloxextraspace = (r_robloxextraspace__robloxextraspace_typedef)(x(0x007821E0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(robloxextraspace__robloxextraspace_CCV* r_robloxextraspace__robloxextraspace_typedef)(int a1);
	r_robloxextraspace__robloxextraspace_typedef r_robloxextraspace__robloxextraspace = (r_robloxextraspace__robloxextraspace_typedef)(x(Adresses::robloxextraspace__robloxextraspace_addr));



	*/
	std::uint32_t robloxextraspace__robloxextraspace_addr = 0x007821E0;
#define robloxextraspace__robloxextraspace_CCV __thiscall

	/*

	* sandboxthread_Decompiled_Args: int a1, int a2, int a3
	* sandboxthread_TypeDef: int
	* sandboxthread_Aob: 55 8B EC 80 3D 88 93 B2 2 0 56
	* sandboxthread_ProtectedByRetcheck: false
	* sandboxthread_IsApartOfLua: false
	* sandboxthread_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_sandboxthread_typedef)(int a1, int a2, int a3);
	r_sandboxthread_typedef r_sandboxthread = (r_sandboxthread_typedef)(x(0x00795480));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(sandboxthread_CCV* r_sandboxthread_typedef)(int a1, int a2, int a3);
	r_sandboxthread_typedef r_sandboxthread = (r_sandboxthread_typedef)(x(Adresses::sandboxthread_addr));



	*/
	std::uint32_t sandboxthread_addr = 0x00795480;
#define sandboxthread_CCV __cdecl

	/*

	* setarrayvector_Decompiled_Args: int a1, int a2
	* setarrayvector_TypeDef: int
	* setarrayvector_Aob: 55 8B EC 53 8B 5D 10 56 57 81 FB FF FF FF F 77
	* setarrayvector_ProtectedByRetcheck: false
	* setarrayvector_IsApartOfLua: true
	* setarrayvector_Mask: ................
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_setarrayvector_typedef)(int a1, int a2);
	r_setarrayvector_typedef r_setarrayvector = (r_setarrayvector_typedef)(x(0x01398E80));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(setarrayvector_CCV* r_setarrayvector_typedef)(int a1, int a2);
	r_setarrayvector_typedef r_setarrayvector = (r_setarrayvector_typedef)(x(Adresses::setarrayvector_addr));



	*/
	std::uint32_t setarrayvector_addr = 0x01398E80;
#define setarrayvector_CCV __cdecl

	/*

	* setnodevector_Decompiled_Args: int a1, int a2
	* setnodevector_TypeDef: bool
	* setnodevector_Aob: 55 8B EC 53 56 8B 75 10 57 85 F6 75 10 8B 7D C 33
	* setnodevector_ProtectedByRetcheck: false
	* setnodevector_IsApartOfLua: true
	* setnodevector_Mask: .................
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_setnodevector_typedef)(int a1, int a2);
	r_setnodevector_typedef r_setnodevector = (r_setnodevector_typedef)(x(0x01398F00));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(setnodevector_CCV* r_setnodevector_typedef)(int a1, int a2);
	r_setnodevector_typedef r_setnodevector = (r_setnodevector_typedef)(x(Adresses::setnodevector_addr));



	*/
	std::uint32_t setnodevector_addr = 0x01398F00;
#define setnodevector_CCV __cdecl

	/*

	* singlestep_Decompiled_Args: int a1
	* singlestep_TypeDef: int
	* singlestep_Aob: 55 8B EC 53 8B 5D 8 56
	* singlestep_ProtectedByRetcheck: false
	* singlestep_IsApartOfLua: true
	* singlestep_Mask: ........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_singlestep_typedef)(int a1);
	r_singlestep_typedef r_singlestep = (r_singlestep_typedef)(x(0x013974A0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(singlestep_CCV* r_singlestep_typedef)(int a1);
	r_singlestep_typedef r_singlestep = (r_singlestep_typedef)(x(Adresses::singlestep_addr));



	*/
	std::uint32_t singlestep_addr = 0x013974A0;
#define singlestep_CCV __cdecl

	/*

	* spawn_Decompiled_Args: int a1
	* spawn_TypeDef: int
	* spawn_Aob: 55 8B EC 6A FF 68 9F 26 11 2 64
	* spawn_ProtectedByRetcheck: false
	* spawn_IsApartOfLua: false
	* spawn_Mask: ...........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_spawn_typedef)(int a1);
	r_spawn_typedef r_spawn = (r_spawn_typedef)(x(0x00795810));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(spawn_CCV* r_spawn_typedef)(int a1);
	r_spawn_typedef r_spawn = (r_spawn_typedef)(x(Adresses::spawn_addr));



	*/
	std::uint32_t spawn_addr = 0x00795810;
#define spawn_CCV __cdecl

	/*

	* stack_init_Decompiled_Args: int a1
	* stack_init_TypeDef: int
	* stack_init_Aob: 55 8B EC 57 68 C0 0 0 0 FF
	* stack_init_ProtectedByRetcheck: false
	* stack_init_IsApartOfLua: true
	* stack_init_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_stack_init_typedef)(int a1);
	r_stack_init_typedef r_stack_init = (r_stack_init_typedef)(x(0x0138E6B0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(stack_init_CCV* r_stack_init_typedef)(int a1);
	r_stack_init_typedef r_stack_init = (r_stack_init_typedef)(x(Adresses::stack_init_addr));



	*/
	std::uint32_t stack_init_addr = 0x0138E6B0;
#define stack_init_CCV __cdecl

	/*

	* str_find_aux_Decompiled_Args: int a1, const char* a2
	* str_find_aux_TypeDef: int
	* str_find_aux_Aob: 55 8B EC 81 EC 34 1 0 0 53
	* str_find_aux_ProtectedByRetcheck: false
	* str_find_aux_IsApartOfLua: true
	* str_find_aux_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_str_find_aux_typedef)(int a1, const char* a2);
	r_str_find_aux_typedef r_str_find_aux = (r_str_find_aux_typedef)(x(0x01394720));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(str_find_aux_CCV* r_str_find_aux_typedef)(int a1, const char* a2);
	r_str_find_aux_typedef r_str_find_aux = (r_str_find_aux_typedef)(x(Adresses::str_find_aux_addr));



	*/
	std::uint32_t str_find_aux_addr = 0x01394720;
#define str_find_aux_CCV __cdecl

	/*

	* tag_error_Decompiled_Args: std::uint32_t Rbx_LuaState
	* tag_error_TypeDef: int
	* tag_error_Aob: 55 8B EC FF 75 10 FF 75 8 E8
	* tag_error_ProtectedByRetcheck: false
	* tag_error_IsApartOfLua: true
	* tag_error_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_tag_error_typedef)(std::uint32_t Rbx_LuaState);
	r_tag_error_typedef r_tag_error = (r_tag_error_typedef)(x(0x01384780));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(tag_error_CCV* r_tag_error_typedef)(std::uint32_t Rbx_LuaState);
	r_tag_error_typedef r_tag_error = (r_tag_error_typedef)(x(Adresses::tag_error_addr));



	*/
	std::uint32_t tag_error_addr = 0x01384780;
#define tag_error_CCV __cdecl

	/*

	* trustCheck_Decompiled_Args: const char* a1, const char* a2
	* trustCheck_TypeDef: bool
	* trustCheck_Aob: 55 8B EC 64 A1 0 0 0 0 6A
	* trustCheck_ProtectedByRetcheck: false
	* trustCheck_IsApartOfLua: false
	* trustCheck_Mask: ..........
	* Usage (Not 100% Accurate):

	typedef bool(__cdecl* r_trustCheck_typedef)(const char* a1, const char* a2);
	r_trustCheck_typedef r_trustCheck = (r_trustCheck_typedef)(x(0x014FC290));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef bool(trustCheck_CCV* r_trustCheck_typedef)(const char* a1, const char* a2);
	r_trustCheck_typedef r_trustCheck = (r_trustCheck_typedef)(x(Adresses::trustCheck_addr));



	*/
	std::uint32_t trustCheck_addr = 0x014FC290;
#define trustCheck_CCV __cdecl

	/*

	* unbound_search_Decompiled_Args: int a1, int a2
	* unbound_search_TypeDef: int
	* unbound_search_Aob: 55 8B EC 83 EC 8 F 28 1D
	* unbound_search_ProtectedByRetcheck: false
	* unbound_search_IsApartOfLua: true
	* unbound_search_Mask: .........
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_unbound_search_typedef)(int a1, int a2);
	r_unbound_search_typedef r_unbound_search = (r_unbound_search_typedef)(x(0x01398FC0));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(unbound_search_CCV* r_unbound_search_typedef)(int a1, int a2);
	r_unbound_search_typedef r_unbound_search = (r_unbound_search_typedef)(x(Adresses::unbound_search_addr));



	*/
	std::uint32_t unbound_search_addr = 0x01398FC0;
#define unbound_search_CCV __cdecl

	/*

	* vm_invoker_Decompiled_Args: std::uint32_t Rbx_LuaState
	* vm_invoker_TypeDef: int
	* vm_invoker_Aob: 55 8B EC 8B 45 8 80
	* vm_invoker_ProtectedByRetcheck: false
	* vm_invoker_IsApartOfLua: true
	* vm_invoker_Mask: .......
	* Usage (Not 100% Accurate):

	typedef int(__cdecl* r_vm_invoker_typedef)(std::uint32_t Rbx_LuaState);
	r_vm_invoker_typedef r_vm_invoker = (r_vm_invoker_typedef)(x(0x0138D250));

	* (Compatable Format) Usage (Not 100% Accurate):

	typedef int(vm_invoker_CCV* r_vm_invoker_typedef)(std::uint32_t Rbx_LuaState);
	r_vm_invoker_typedef r_vm_invoker = (r_vm_invoker_typedef)(x(Adresses::vm_invoker_addr));



	*/
	std::uint32_t vm_invoker_addr = 0x0138D250;
#define vm_invoker_CCV __cdecl

	std::uint32_t callcheck = 0x005F4760;

	std::uint32_t retcheckflag1 = 0x02998144;

	std::uint32_t retcheckflag2 = 0x02998234;

	std::uint32_t xorconst = 0x028FFE50;

	std::uint32_t luao_nilobject = 0x02178EB0;

	std::uint32_t luat_typenames = 0x0217A1C0;

	std::uint32_t kname = 0x026D9980;

	std::uint32_t op_index = 0x01389060;

	std::uint32_t op_case_table = 0x01388F44;

	std::uint32_t retcheck_jmp = 0x006C6450;

	std::uint32_t trustcheck_bypass1 = 0x025AA818;

	std::uint32_t scriptcontext = 0x01FADDAC;

	// Roblox Offsets 
	int getnamecallmethod_offset = 100;
	int networked_dm = 1184;
	int Identity1 = 24;
	int Identity2 = 108;
	// Luastate Offsets 
	int ls_readonly = 8;
	int ls_base_ci = 36;
	int ls_ci = 24;
	int ls_saved_pc = 16;
	int ls_top = 16;
	int ls_base = 8;

	// Globalstate Offsets 
	int ls_gs_totalbytes = 76;
	int ls_gs_rootgc = 28;
	int ls_gs_mainthread = 368;

	// Closure Offsets 
	int ls_closure_isc = 6;

	// Types 
#define R_LUA_TNONE   -1
#define R_LUA_TUSERDATA   8
#define R_LUA_TFUNCTION   7
#define R_LUA_TSTRING   5
#define R_LUA_TBOOLEAN   1
#define R_LUA_TNUMBER   4
#define R_LUA_TTABLE   6
#define R_LUA_TNIL   0
#define R_LUA_TTHREAD   9
#define R_LUA_TVECTOR   3
#define R_LUA_TLIGHTUSERDATA   2
#define R_LUA_TPROTO   10
#define R_LUA_TUPVALUE   11

// Protos 
	int proto_code = 36;
	int proto_sizecode = 72;
	int proto_p = 20;
	int proto_sizep = 68;
	int proto_k = 16;
	int proto_sizek = 76;
	int proto_lineinfo = 32;
	int proto_sizelineinfo = 56;
	int proto_locvars = 12;
	int proto_sizelocvars = 64;
	int proto_upvalues = 28;
	int proto_sizeupvalues = 56;
	int proto_is_vararg = 20;
	int proto_source = 44;
	int proto_nups = 7;
	int proto_numparams = 237;
	int proto_maxstacksize = 83;
	DWORD RBX_GlobalState(DWORD LS) { return (LS + 8) ^ *(DWORD*)(LS + 8); }
	// Dumped With Mellonyt aka Mellon Addy Dumper My Discord: mellonyt#1488 
	// Addys Dumped: 302
	// No Duplicate Addys :D 
}
